...
export default function CalendarPage() {
  ...
  if (state === 'loading') return <p className="text-fg/50 text-sm">Loading your compliance calendar…</p>;
  if (state === 'auth') {
    return (
  ...
  return (
    <div>
      ...
      <p className="text-fg/60 text-sm mb-8 max-w-2xl">
          ...
          {obligations.map((o, i) => (
            <div key={i} className={`border border-[var(--color-line)] border-l-2 ${SEV_CLASS[o.severity || 'low']} px-5 py-4 flex items-baseline justify-between gap-4`}>
              <div className="min-w-0">
                <div className="font-mono text-[0.62rem] uppercase tracking-wider text-fg/45 mb-1">{String(o.regime || '').toUpperCase()}</div>
                <div className="font-serif text-lg text-ivory">{o.title}</div>
                {o.detail && <p className="text-sm text-fg/65 mt-1 leading-relaxed">{o.detail}</p>}
                {o.citation && <div className="font-mono text-[0.66rem] text-fg/35 mt-1">{o.citation}</div>}
              </div>
              <div className="text-right shrink-0">
                <div className={`font-mono text-lg ${SEV_TEXT[o.severity || 'low']}`}>{dayLabel(o.daysUntil)}</div>
                <div className="font-mono text-[0.66rem] text-fg/45">{o.dueDate}</div>
              </div>
            </div>
          ))}
        </div>
