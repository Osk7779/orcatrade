...
const VERDICT: Record<string, { label: string; cls: string }> = {
  ...
  minor_issues: { label: 'Minor issues', cls: 'text-fg/70 border-l-fg/30' },
  consistent: { label: 'Consistent', cls: 'text-emerald-300 border-l-emerald-500' },
};

const SEV_TEXT: Record<Severity, string> = {
  critical: 'text-red-400', high: 'text-amber-400', medium: 'text-fg/70', low: 'text-fg/55', info: 'text-sky-400',
};
    ...
    <div className="max-w-3xl">
      ...
      <form onSubmit={audit} className="flex flex-col gap-3 mb-6">
        <div className="flex gap-3 flex-wrap">
          <select value={docType} onChange={(e) => setDocType(e.target.value)} className="bg-fg/[0.04] border border-[var(--color-line)] px-3 py-2 text-sm rounded-sm">
            {DOC_TYPES.map((d) => <option key={d.id} value={d.id}>{d.label}</option>)}
          </select>
          <select value={planId} onChange={(e) => setPlanId(e.target.value)} className="bg-fg/[0.04] border border-[var(--color-line)] px-3 py-2 text-sm rounded-sm">
        ...
        <textarea
          ...
          className="bg-fg/[0.04] border border-[var(--color-line)] px-3 py-2 text-sm font-mono rounded-sm focus:outline-none focus:border-fg/30"
        />
      ...
      {result && v && (
        <div className={`border border-[var(--color-line)] border-l-2 ${v.cls} px-5 py-4`}>
          ...
          {result.extraction && (
            <div className="text-fg/45 text-xs mb-3 font-mono">
            ...
            <ul className="flex flex-col gap-2">
              {result.findings.map((f: AuditFinding, i) => (
                <li key={i} className="text-sm">
                  <span className={`font-mono text-[0.62rem] uppercase mr-2 ${SEV_TEXT[f.severity] || 'text-fg/50'}`}>{f.severity}</span>
                  <span className="text-fg/80">{f.message}</span>
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-fg/60 text-sm">No issues found against the provided inputs.</p>
          )}
          {result.advisory && <p className="text-fg/40 text-xs mt-4 leading-relaxed">{result.advisory}</p>}
        </div>
      )}
    </div>
  );
