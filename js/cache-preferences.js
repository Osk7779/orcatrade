(function () {
  ...
  function widgetMarkup(preference, firstVisit) {
    if (firstVisit) {
      return '' +
  ...
  function ensureWidgetStyles() {
    ...
    style.textContent = ''
      /* Persistent widget sits below the sticky header (z-1000) so it
         can never block the nav. Modal mode (first-visit consent gate)
         is allowed to sit at 990 — under the header but over everything
         else, so the user can still see the brand while choosing. */
      + '.ot-cache-widget{position:fixed;left:1rem;bottom:1.25rem;z-index:40;font-family:Geist,Arial,sans-serif;}'
      + '.ot-cache-widget--modal{inset:0;right:auto;bottom:auto;z-index:990;display:flex;align-items:center;justify-content:center;padding:1rem;}'
      + '.ot-cache-overlay{position:fixed;inset:0;background:rgba(0,0,0,0.58);backdrop-filter:blur(6px);}'
      ...
      + '.ot-cache-widget-panel{display:none;width:min(340px,calc(100vw - 7rem));margin-top:0.6rem;padding:1rem 1rem 0.95rem;background:rgba(8,12,20,0.96);border:1px solid rgba(255,255,255,0.14);color:#ececec;box-shadow:0 18px 40px rgba(0,0,0,0.45);backdrop-filter:blur(16px);}'
      + '.ot-cache-widget-panel--modal{display:block;position:relative;width:min(420px,calc(100vw - 2rem));margin-top:0;padding:1.15rem 1.1rem 1rem;z-index:1;}'
      ...
      + '.ot-cache-widget-actions button{border:1px solid rgba(255,255,255,0.16);background:transparent;color:#ececec;padding:0.55rem 0.75rem;font-size:0.74rem;letter-spacing:0.08em;text-transform:uppercase;cursor:pointer;-webkit-tap-highlight-color:rgba(255,255,255,0.08);touch-action:manipulation;}'
      ...
      /* Mobile: hide the persistent toggle once a preference is saved.
         The first-visit modal still shows (consent gate); changing the
         preference later is a desktop-only affordance. Keeps the chat
         composer and bottom navigation surface uncluttered. */
      + '@media (max-width:640px){.ot-cache-widget{left:1rem;right:auto;bottom:1rem;}.ot-cache-widget-toggle{justify-content:center;}.ot-cache-widget-panel{width:min(280px,calc(100vw - 6rem));}.ot-cache-widget--modal{left:0;right:0;}}'
      + '@media (max-width:640px){.ot-cache-widget:not(.ot-cache-widget--modal){display:none;}}';
  ...
  function renderWidget() {
    ...
    var firstVisit = !hasUserChoice();
    var root = document.createElement('div');
    root.className = 'ot-cache-widget' + (firstVisit ? ' ot-cache-widget--modal' : '');
    root.id = 'ot-cache-widget';
    root.innerHTML = widgetMarkup(getPreference(), firstVisit);
