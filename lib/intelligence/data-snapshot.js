...
//   v1 → v2: added tradeDefence.measures[].rateUnit (slice 3b) so a pinned
//            recompute classifies specific (EUR/unit) vs ad-valorem duties
//            identically to the live calculator.
...
//
// opts.pinnedTaric (apex P1.1 — TARIC duty pinning per quote):
//   When the caller has the actual live TARIC rates that were used during
//   quote computation (extracted from customs-quote.js's liveRateMeta), pass
//   them in so they get pinned into the snapshot. Recompute against the
//   stored snapshot then uses the pinned rate as the source of truth, not a
//   fresh upstream fetch — making EVERY euro reproducible. Drift detection
//   (lib/intelligence/taric-drift.js) compares the pinned rate against a
//   live re-fetch and surfaces the delta.
//
//   Shape: [{ hsCode, originCountry, rate, source, asOf }, …]
function captureDataSnapshot(opts = {}) {
  const pinnedTaricInput = Array.isArray(opts.pinnedTaric) ? opts.pinnedTaric : [];
  // Normalise + sort for stable id. Only the rate-bearing fields are
  // captured — source/asOf are metadata that doesn't drive a number.
  const pinnedTaric = pinnedTaricInput
    .filter((p) => p && p.hsCode && Number.isFinite(p.rate))
    .map((p) => ({
      hsCode: String(p.hsCode),
      originCountry: p.originCountry ? String(p.originCountry).toUpperCase() : null,
      rate: Number(p.rate),
      source: p.source || null,
      asOf: p.asOf || null,
    }))
    .sort((a, b) => {
      const k = `${a.hsCode}|${a.originCountry || ''}`;
      const j = `${b.hsCode}|${b.originCountry || ''}`;
  ...
  return {
      ...
      measures: Array.isArray(tradeDefence && tradeDefence.MEASURES)
        ? tradeDefence.MEASURES
            .map((m) => ({
              ...
              // Specific (EUR/unit) measures are excluded from ad-valorem
              // totalling — capture the unit so a pinned recompute matches.
    ...
    // TARIC duty rates — when live-fetched at quote time, the caller passes
    // them via opts.pinnedTaric and they're pinned here for recompute.
    // `mode` distinguishes "the snapshot itself is authoritative (offline)"
    // from "the snapshot has pinned live rates (live with pinning)" from
    // "no rates pinned at this layer — recompute must re-fetch (legacy)".
    taric: {
      mode: process.env.ORCATRADE_DISABLE_LIVE_TARIC
        ? 'snapshot'
        : (pinnedTaric.length > 0 ? 'pinned' : 'live'),
      pinned: pinnedTaric,
    },
  };
}
...
// opts forwarded to captureDataSnapshot — e.g. { pinnedTaric: [...] } per
// apex P1.1.
function currentDataSnapshot(opts = {}) {
  const snapshot = captureDataSnapshot(opts);
  return {
...
function diffDataSnapshots(oldSnap, newSnap) {
  ...
  // TARIC mode (live vs snapshot vs pinned)
  cmp('taric.mode', 'TARIC mode', (a.taric || {}).mode, (b.taric || {}).mode);

  // TARIC pinned rates — per (hsCode, originCountry) pair (apex P1.1).
  // A rate change here moves a customer-facing duty number directly, so
  // the diff surfaces it as a first-class change with the full context.
  const pinA = ((a.taric || {}).pinned || []);
  const pinB = ((b.taric || {}).pinned || []);
  const pinKey = (p) => `${p.hsCode}|${p.originCountry || ''}`;
  const pinAByKey = new Map(pinA.map((p) => [pinKey(p), p]));
  const pinBByKey = new Map(pinB.map((p) => [pinKey(p), p]));
  for (const key of new Set([...pinAByKey.keys(), ...pinBByKey.keys()])) {
    const pA = pinAByKey.get(key); const pB = pinBByKey.get(key);
    cmp(`taric.pinned.${key}.rate`, `TARIC duty rate for ${key}`,
      pA ? pA.rate : undefined, pB ? pB.rate : undefined);
  }
