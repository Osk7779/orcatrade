...
function comparePaymentInstruments({ amountEur, supplierCountry, supplierRelationshipMonths, importerRiskAppetite }) {
  ...
  const instruments = INSTRUMENT_KEYS.map(key => {
    ...
    const variableCost = amountEur * inst.costPercent;
    const totalCostEur = round(variableCost + inst.fixedFeeEur, 0);
    return {
      key,
      label: inst.label,
      family: inst.family,
...
function estimateLcCost({ amountEur, durationMonths, confirmed, expectedDiscrepancies }) {
  ...
  const issuanceFee = round(amountEur * LC_COST_TABLE.issuanceQuarterPercent * quarters, 0);
  const confirmationFee = confirmed ? round(amountEur * LC_COST_TABLE.confirmationQuarterPercent * quarters, 0) : 0;
...
function estimateFxHedgingCost({ amountEur, currencyPair, durationDays }) {
  ...
  const hedgingCostEur = round(amountEur * proRated, 0);
  const unhedgedRiskOneSigmaEur = round(amountEur * (fx.annualisedVolatilityPercent / 100) * Math.sqrt(days / 365), 0);
...
function calculateWorkingCapitalCycle({ dioDays, dsoDays, dpoDays, costOfCapitalAnnual }) {
  ...
  const sample = 100000;
  const cycleCostOnSampleEur = round(sample * cocAnnual * (cycleDays / 365), 0);
...
function assessTradeCreditCover({ buyerCountry, buyerSizeBracket, exposureEur }) {
  ...
  const annualPremiumEur = Math.max(
    ...
    round(exposureEur * baseRate * countryLoading * sizeFactor, 0),
  );
  ...
  return {
    ...
    breakdown: [
      { label: `Base rate (${(baseRate * 100).toFixed(3)}% per €1)`, value: round(exposureEur * baseRate, 0) },
