...
//
// Universal envelope for finance-quote outputs. Every finance function
// (comparePaymentInstruments, estimateLcCost, estimateFxHedgingCost,
// calculateWorkingCapitalCycle, assessTradeCreditCover) accepts an
// amount (in euros) and is bounded by the validateInput cap of €50M.
// Tier-A requires the amount to fall in [€1, €50M] (1-cent to 50M-EUR
// in integer cents per ADR 0004).
//
// Function-specific axes (currencyPair, instrumentKey, durationMonths)
// flow into the snapshot but don't gate eligibility in v1 — adding per-
// function COVERAGE manifests is a follow-up. The honest posture:
// today's PRICING_SNAPSHOT is our internal benchmark (Polish + DACH
// bank rate cards 2026 Q1), so source_kind = 'mirror' and TA-2 fails.
// Until we wire ECB FX live + named-bank-quote-API feeds, no finance
// quote satisfies Tier-A. Same shape as customs-quote without live
// TARIC — degrades safely AWAY from eligibility.
const TIER_A_COVERAGE = Object.freeze({
  calculatorName: 'finance-quote',
  version: 1,
  axes: Object.freeze({
    amountCents: { type: 'integer-range', min: 100, max: 50_000_000_00 },
  }),
});
...
//
// Same shape as customs-quote.buildTierAInput (PR #89) and sourcing-
// quote.buildTierAInput (PR #99). PR #141 added the primary-source
// path: when comparePaymentInstrumentsAsync or estimateLcCostAsync
// successfully reached ECB for a quote, the result carries
// liveFxMeta and we emit it as a primary_regulator snapshot,
// dropping the rate-card mirror (PR #132/#139 pattern).
...
function buildTierAInput(quoteResult) {
  ...
  // liveFxMeta — if the async path ran and ECB returned a rate, the
  // hedging cost number drew on the primary regulator. This is the
  // only path to TA-2 passing today.
  const fx = quoteResult.liveFxMeta;
  const hasPrimary = Boolean(fx && fx.asOf && fx.source === 'ecb-fx');
  if (hasPrimary) {
    snapshots.push({
      id: `ecb-fx:${fx.currency}@${fx.asOfDate}`,
      source_kind: 'primary_regulator',
      as_of_iso: toIsoSafe(fx.asOf),
    });
  } else {
    // Sync path (or async fell back): honest mirror declaration.
    snapshots.push({
      id: `finance-quote:pricing@${PRICING_SNAPSHOT.asOf}`,
      source_kind: 'mirror',
      as_of_iso: toIsoStartOfDay(PRICING_SNAPSHOT.asOf),
    });
  }

  const amountEur = (quoteResult.inputs && Number(quoteResult.inputs.amountEur)) || 0;
  ...
  return {
    ...
    escalations: [],
    overrides: [],
    coverageInput: { amountCents },
    calculatorCoverage: TIER_A_COVERAGE,
  };
}

// ── comparePaymentInstrumentsAsync — ECB-backed primary-source path ──
//
// Mirror of customs-quote.calculateQuoteAsync + sourcing-quote.
// recommendCountryAsync. Falls back to sync comparePaymentInstruments
// when no fxCurrency supplied OR ECB returns null OR no fxDate
// supplied (calculator stays deterministic per ADR 0020 — the caller
// supplies the date).
//
// The instrument comparison numbers don't currently change — the
// rate-card still drives cost ranking. What changes is the AUDIT
// POSTURE: an ECB-backed quote cites a primary regulator,
// satisfying TA-2.
async function comparePaymentInstrumentsAsync(input, opts = {}) {
  const sync = comparePaymentInstruments(input);
  if (!sync.ok) return sync;

  const wantEcb = opts.useEcb !== false && opts.fxCurrency && opts.fxDate;
  if (!wantEcb) return sync;

  const ecb = require('./ecb-fx-client');

  let rate = null;
  try {
    rate = await ecb.lookupSpotRate(opts.fxCurrency, opts.fxDate, {
  ...
  if (!rate || !rate.asOf || rate.source !== 'ecb-fx') return sync;
  ...
  return {
    ...
    liveFxMeta: {
      ...
      asOf: rate.asOf,
      source: rate.source,
      fromCache: rate.fromCache === true,
      stale: rate.stale === true,
      unitsPerEur: rate.unitsPerEur,
      verifyUrl: ecb.ecbVerifyUrl(opts.fxCurrency),
    },
  };
}

// Same shape as customs-quote.js's helpers — tolerate the two
...
module.exports = {
  ...
  comparePaymentInstrumentsAsync,
  estimateLcCost,
