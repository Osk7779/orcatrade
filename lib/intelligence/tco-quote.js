...
//
// Money: annual figures scale per-shipment values across up to 52 shipments,
// so they run large — computed in integer cents (lib/intelligence/money.js,
// half-even rounding, no float drift) and displayed at whole-euro granularity.
...
function scaleAnnual(perShipment, shipmentsPerYear, waccPct, daysInInventory) {
  const annualCustomsCents = M.mulRate(M.fromEuro(perShipment.customsValueEur), shipmentsPerYear);
  const annualDutyCents = M.mulRate(M.fromEuro(perShipment.dutyEur), shipmentsPerYear);
  const annualVatCents = M.mulRate(M.fromEuro(perShipment.vatEur), shipmentsPerYear);
  const annualBrokerageCents = M.mulRate(M.fromEuro(perShipment.brokerageEur), shipmentsPerYear);
  const annualTransportCents = M.mulRate(M.fromEuro(perShipment.transportEur), shipmentsPerYear);
  ...
  const avgInventoryCents = M.mulRate(annualCustomsCents, daysInInventory / 365);
  ...
  const inventoryCarryingCents = M.mulRate(avgInventoryCents, waccPct / 100);

  // Net annual cost (excluding VAT cash-flow, which is recoverable):
  const annualNetCents = M.sum([annualDutyCents, annualBrokerageCents, annualTransportCents, inventoryCarryingCents]);
  // Cash-flow annual cost (including VAT, which sits on balance sheet):
  const annualCashFlowCents = M.add(annualNetCents, annualVatCents);

  const e = (cents) => Math.round(M.toEuro(cents));
  return {
    annualCustomsValueEur: e(annualCustomsCents),
    annualDutyEur: e(annualDutyCents),
    annualVatEur: e(annualVatCents),
    annualBrokerageEur: e(annualBrokerageCents),
    annualTransportEur: e(annualTransportCents),
    avgInventoryValueEur: e(avgInventoryCents),
    inventoryCarryingCostEur: e(inventoryCarryingCents),
    annualNetCost: e(annualNetCents),
    annualCashFlowCost: e(annualCashFlowCents),
  };
}
...
function calculateTco({
  ...
  const bondedDeferralValueEur = Math.round(M.toEuro(M.mulRate(M.fromEuro(annualDeferableTaxes), (waccPct / 100) * (daysInInventory / 365))));
