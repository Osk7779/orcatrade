...
function buildReportLineage(sourceReport, normalizedOrderData, asOfDate) {
...
function buildEvidenceSnapshot(normalizedOrderData, checkedRegulations, generatedAt) {
...
function enforceComplianceLogic(report, orderData = {}) {
  ...
  const reportLineage = buildReportLineage(sourceReport, normalizedOrderData, asOfDate);
  const reportOwnership = buildReportOwnership(normalizedOrderData);
  const evidenceSnapshot = buildEvidenceSnapshot(normalizedOrderData, checkedRegulations, reportLineage.generatedAt);
