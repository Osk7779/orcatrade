...
function pickFirstDefined(source, keys) {
  ...
  for (const key of keys) {
    ...
    if (typeof value === 'string' && !cleanString(value)) continue;
    return value;
  }
...
function parseBooleanFlag(value) {
  ...
  if (['true', 'yes', 'y', '1', 'on', 'ready', 'available', 'submitted', 'complete'].includes(text)) {
    return true;
  }

  if (['false', 'no', 'n', '0', 'off', 'missing', 'not ready', 'not available', 'not submitted', 'incomplete'].includes(text)) {
    return false;
  }
...
function parseGlobalTurnover(orderData = {}) {
  return parseMoneyAmount(
    pickFirstDefined(orderData, ['globalTurnover', 'companyTurnover', 'turnover', 'annualTurnover'])
  );
}
...
function evaluateCbam(orderData = {}, asOfDate) {
  ...
  const goodsCode = cleanString(pickFirstDefined(orderData, ['cnCode', 'hsCode']));
  const authorisedDeclarant = parseBooleanFlag(
    pickFirstDefined(orderData, ['authorisedDeclarant', 'authorizedDeclarant', 'declarantReady'])
  );
  const supplierEmissionsData = parseBooleanFlag(
    pickFirstDefined(orderData, ['supplierEmissionsData', 'emissionsDataAvailable', 'emissionsEvidenceAvailable'])
  );
  ...
  if (isCbamCategoryText(categoryText)) {
    const missingFacts = [];
    const evidenceSignals = ['Goods description intersects Annex I sector keywords.', `Current assessment date: ${asOfDate}.`];

    if (!goodsCode) missingFacts.push('CN / HS classification');
    if (authorisedDeclarant === null) missingFacts.push('authorised CBAM declarant status');
    if (supplierEmissionsData === null) missingFacts.push('supplier emissions data status');

    if (goodsCode) evidenceSignals.push(`Goods code supplied: ${goodsCode}.`);
    if (authorisedDeclarant !== null) {
      evidenceSignals.push(`Authorised declarant status supplied: ${authorisedDeclarant ? 'yes' : 'no'}.`);
    }
    if (supplierEmissionsData !== null) {
      evidenceSignals.push(`Supplier emissions data status supplied: ${supplierEmissionsData ? 'yes' : 'no'}.`);
    }
...
function evaluateEudr(orderData = {}, asOfDate) {
  ...
  const geolocationAvailable = parseBooleanFlag(
    pickFirstDefined(orderData, ['geolocationAvailable', 'geolocationEvidence', 'geolocationData', 'polygonDataAvailable'])
  );
  const dueDiligenceStatement = parseBooleanFlag(
    pickFirstDefined(orderData, ['dueDiligenceStatement', 'dueDiligenceReady', 'dueDiligenceStatementReady'])
  );
  ...
  if (geolocationAvailable === null) {
    missingFacts.push('plot-level geolocation evidence status');
  }
  if (dueDiligenceStatement === null) {
    missingFacts.push('due-diligence statement status');
  }

  const evidenceSignals = [
    'Goods description intersects EUDR-covered commodities or derived products.',
    `Current assessment date: ${asOfDate}.`,
    `Application date used: ${futureApplicabilityDate}.`,
  ];

  if (geolocationAvailable !== null) {
    evidenceSignals.push(`Geolocation evidence status supplied: ${geolocationAvailable ? 'yes' : 'no'}.`);
  }
  if (dueDiligenceStatement !== null) {
    evidenceSignals.push(`Due-diligence statement status supplied: ${dueDiligenceStatement ? 'yes' : 'no'}.`);
  }
