...
// a primary source.
//
// PR #132 closed the first primary-source gate: when liveRateMeta is
// present, the rate-card mirror is DROPPED from the snapshot list.
// Rationale: TA-2 fails as soon as ANY snapshot is not primary_
// regulator. Before this PR, BOTH snapshots were always emitted, and
// the mirror's source_kind:'mirror' caused TA-2 to fail even when
// the duty number came from the live primary regulator. Since the
// live rate OVERRODE the rate-card's number (via mfnRateOverride in
// calculateQuoteAsync), the mirror snapshot is operationally
// redundant — it would be misleading to claim it's "part of the
// input" for the duty number that ended up in the quote.
...
function buildTierAInput(quoteResult) {
  ...
  // duty number on this quote came from the primary regulator. This
  // snapshot, if present, is the ONLY one we emit — see the PR #132
  // rationale above.
  const live = quoteResult.duty && quoteResult.duty.liveRateMeta;
  const hasPrimaryRegulator = Boolean(live && live.asOf);
  if (hasPrimaryRegulator) {
    snapshots.push({
    ...
    // Sync path (or async path that fell back to the rate card): the
    // duty number came from the mirror — declare it honestly. TA-2
    // will fail on this snapshot, which is the correct conservative
    // posture.
    snapshots.push({
