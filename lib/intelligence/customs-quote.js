...
function resolveDutyRate({ hsCode, originCountry, claimPreferential, mfnRateOverride, mfnRateOverrideSource }) {
...
async function calculateQuoteAsync(input, opts = {}) {
  ...
  const wantLive = opts.useLiveTaric !== false && normHs && normOrigin && normHs.length >= 8;
  ...
  let liveRate = null;
  try {
    liveRate = await taric.lookupHsRate(normHs, normOrigin, {
  ...
  // Re-run the sync calc but with the live rate as the MFN override. We
  // can't reuse calculateQuote directly because it doesn't accept an
  // override; the cleanest approach is to inline the small amount of
  // top-level orchestration here.
  const validation = validateInput(input);
  if (!validation.ok) return { ok: false, errors: validation.errors };
  ...
  const standard = calculateStandardClearance({
    customsValueEur, dutyRate: duty.rate, vat, linesCount,
  });
  const bonded = calculateBondedWarehouse({
    customsValueEur, dutyRate: duty.rate, vat, linesCount, bondedDays, bondedVolumeCbm, releaseStrategy,
  });
  const dutyEur = customsValueEur * duty.rate;
  const vatEur = (customsValueEur + dutyEur) * vat.rate;
  const recommendation = recommendRoute({ standard, bonded, releaseStrategy, dutyEur, vatEur, customsValueEur });
  ...
  return {
    ...
    vat: {
      rate: vat.rate, ratePercent: round(vat.rate * 100, 2), country: vat.name,
    },
    quotes: [standard, bonded],
    recommendation,
    bondedEducation: {
    ...
    nextSteps: [
      `Live ${liveRate.sourceLabel || 'TARIC sanity-check'} rate applied: ${(liveRate.rate * 100).toFixed(1)}%${liveRate.fromCache ? ' (from cache)' : ''}. Cross-check on the official EU TARIC portal: ${taric.taricVerifyUrl(normHs, normOrigin)}`,
      'Request a Binding Tariff Information (BTI) ruling from your national customs authority for high-volume SKUs — locks in the classification for 3 years across the EU.',
      'If re-export is on the table, ask about T1 transit + bonded chain — we operate this through partner warehouses in Hamburg, Rotterdam, and Gdańsk.',
    ],
    pricingSnapshot: PRICING_SNAPSHOT,
