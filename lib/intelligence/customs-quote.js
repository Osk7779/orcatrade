...
function resolveDutyRate({ hsCode, originCountry, claimPreferential }) {
  ...
  if (overlay) {
    ...
    if (typeof adj === 'number') {
      rate += adj;
      breakdown.push({ label: `Origin overlay (${overlay.name}, chapter ${chapter}) — anti-dumping / safeguard`, value: adj });
    }
    if (claimPreferential && overlay.preferential) {
      const discount = overlay.preferentialDiscount || 0;
      const reduction = -1 * rate * discount;
      rate += reduction;
      if (rate < 0) rate = 0;
      breakdown.push({ label: `Preferential origin claim (${overlay.name}) — ${Math.round(discount * 100)}% reduction on MFN; valid origin proof required`, value: reduction });
    }
  }
  return {
    ok: true,
