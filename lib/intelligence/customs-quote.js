...
function calculateStandardClearance({ customsValueEur, dutyRate, vat, linesCount }) {
  ...
  // effectiveLandedCostEur excludes VAT — it's the true P&L cost for a
  // VAT-registered importer (VAT is recovered on the next return). landedCostEur
  // and totalEur stay as-is for backwards compatibility and for non-VAT-registered
  // importers (rare; see cautions).
  const effectiveLandedCostEur = customsValueEur + dutyEur + brokerageEur + ENTRY_SUMMARY_DECLARATION_EUR;
  return {
    ...
    effectiveLandedCostEur: round(effectiveLandedCostEur),
    vatRecoverableEur: round(vatEur),
    breakdown: [
...
function calculateBondedWarehouse({ customsValueEur, dutyRate, vat, linesCount, bondedDays, bondedVolumeCbm, releaseStrategy }) {
  ...
  const effectiveLandedCostEur = customsValueEur + dutyDueEur + bondedOpsCostEur;
  ...
  return {
    ...
    landedCostEur: round(landedCostEur),
    effectiveLandedCostEur: round(effectiveLandedCostEur),
    vatRecoverableEur: round(vatDueEur),
...
async function calculateQuoteAsync(input, opts = {}) {
  ...
  if (!wantLive) return calculateQuote(input);
