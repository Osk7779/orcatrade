...
function calculateValueAddedServiceCost({ services, monthlyOrders, avgUnitsPerOrder, avgPalletsHeld, returnsRate, skuCount }) {
  ...
  for (const svc of services || []) {
    ...
    if (def.perPalletEur) {
      ...
      monthlyCost = (def.perSkuEur * (skuCount || 0)) / SETUP_AMORTISATION_MONTHS;
      labelDetail = `€${def.perSkuEur}/SKU × ${skuCount || 0} SKUs (one-off, amortised over ${SETUP_AMORTISATION_MONTHS} months)`;
    }
    breakdown.push({ key: svc, label: def.name, detail: labelDetail, monthlyCostEur: round(monthlyCost) });
    total += monthlyCost;
  }
...
function calculateHubMonthly({ hub, monthlyOrders, avgUnitsPerOrder, avgLinesPerOrder, avgPalletsHeld, avgOrderWeightKg, primaryDestination, valueAddedServiceCost, vasBreakdown, vasInputs }) {
  ...
  const storageEur = hub.storagePerPalletPerMonthEur * avgPalletsHeld;
  const monthlyInboundPallets = Math.max(1, Math.round(avgPalletsHeld / 6));
  const inboundEur = hub.inboundReceiptPerPalletEur * monthlyInboundPallets;
  const pickPackEur = monthlyOrders * (
    hub.pickBaseEur +
    hub.pickPerLineEur * avgLinesPerOrder +
    hub.pickPerUnitEur * avgUnitsPerOrder +
    hub.packagingMaterialsPerOrderEur
  );
  const outboundShippingEur = monthlyOrders * (outboundRate.base + outboundRate.perKg * avgOrderWeightKg);
  const setupAmortisedEur = hub.setupFeeEur / SETUP_AMORTISATION_MONTHS;
  const totalMonthlyEur = storageEur + inboundEur + pickPackEur + outboundShippingEur + setupAmortisedEur + valueAddedServiceCost;
  const costPerOrderEur = totalMonthlyEur / monthlyOrders;

  const breakdown = [
    {
      label: `Storage (€${hub.storagePerPalletPerMonthEur}/pallet/month × ${avgPalletsHeld} pallets)`,
      monthlyCostEur: round(storageEur),
    },
    {
      label: `Inbound receipt (€${hub.inboundReceiptPerPalletEur}/pallet × ~${monthlyInboundPallets} pallets/month)`,
      monthlyCostEur: round(inboundEur),
    },
    {
      ...
      monthlyCostEur: round(pickPackEur),
    },
    {
  ...
  return {
    ...
    inboundEur: round(inboundEur),
    pickPackEur: round(pickPackEur),
    outboundShippingEur: round(outboundShippingEur),
    setupAmortisedEur: round(setupAmortisedEur),
    valueAddedServiceCostEur: round(valueAddedServiceCost),
    totalMonthlyEur: round(totalMonthlyEur),
    costPerOrderEur: round(costPerOrderEur, 2),
