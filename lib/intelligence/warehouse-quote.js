...
function buildTierAInput(quoteResult) {
  ...
  if (quoteResult.livePpiMeta && quoteResult.livePpiMeta.area && quoteResult.livePpiMeta.asOfPeriod) {
    snapshots = [{
      ...
      // Eurostat publishes warehousing PPI quarterly with a ~75-day
      // publication lag. A 120-day freshness window covers one full
      // release cycle + buffer; outside that, the snapshot is genuinely
      // stale and TA-1 should reject it.
      freshness_days: 120,
    }];
  } else {
    snapshots = [{
