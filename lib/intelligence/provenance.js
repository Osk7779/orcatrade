...
function currentProvenance() {
  return {
    ...
    // Content-addressed fingerprint of the actual volatile data values behind
    // this result (reproducibility-v2). The human-readable dataAsOf dates above
    // are kept for back-compat; this is the machine coordinate that makes the
    // (inputs, calculatorVersion, dataSnapshotId) tuple reproducible.
    dataSnapshotId: dataSnapshot.currentDataSnapshot().id,
