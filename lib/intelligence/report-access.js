...
function buildTokenPayload(reportId, expiresAtMs) {
  return {
    v: 1,
    p: TOKEN_PURPOSE,
...
function createReportAccessToken(reportId, options = {}) {
  ...
  const payload = buildTokenPayload(normalizedReportId, expiresAtMs);
...
function buildRetrievalPath(reportId, accessToken) {
  const normalizedReportId = cleanString(reportId);
  const normalizedToken = cleanString(accessToken);
  if (!normalizedReportId || !normalizedToken) return '';

  return `/api/report?reportId=${encodeURIComponent(normalizedReportId)}&accessToken=${encodeURIComponent(normalizedToken)}`;
}
...
function attachReportAccess(report, options = {}) {
  ...
  return {
    ...
    reportAccess: {
      ...
      retrievalPath: buildRetrievalPath(baseReport.reportId, access.token),
    },
  };
}
