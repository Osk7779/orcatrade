...
const REGIMES = [
...

  {
    ...
    // RoHS applies to electrical and electronic equipment (EEE) only.
    // Non-electronic eyewear — spectacle frames (9003) and finished
    // spectacles/sunglasses/goggles (9004) — has no EEE and is out of scope.
    excludePrefixes: ['9003', '9004'],
...

  {
    ...
    // Same EEE-scope carve-out as RoHS: non-electronic eyewear is not WEEE.
    excludePrefixes: ['9003', '9004'],
...

  {
    ...
    // The Toy Safety Directive covers products designed/intended for play by
    // children under 14. Within chapter 95 that's the toy/game/festive
    // headings (9503-9505), NOT adult sporting & leisure goods: general
    // physical-exercise / sports equipment (9506), fishing tackle (9507),
    // and fairground amusements (9508) are out of scope.
    excludePrefixes: ['9506', '9507', '9508'],
...
function findApplicableRegimes({ hsCode, productCategory }) {
  ...
  for (const regime of REGIMES) {
    ...
    // Carve-outs: a broad chapter-level trigger can over-reach onto
    // headings that the regime's own legal scope excludes (e.g. RoHS is
    // EEE-only, so non-electronic eyewear in chapter 90 is out of scope).
    // If the code matches a more-specific exclusion prefix, suppress the
    // hit — prevents trust-eroding false positives without re-mapping the
    // whole chapter (which would risk false negatives on the genuinely
    // in-scope headings).
    if (hit && hsCode && Array.isArray(regime.excludePrefixes)) {
      if (regime.excludePrefixes.some(p => hsHasPrefix(hsCode, p))) {
        hit = false;
        matchedTrigger = null;
      }
    }
