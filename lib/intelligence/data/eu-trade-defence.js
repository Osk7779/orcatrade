...
//
// SCOPE & LIMITS
// - Curated ~30 measures covering common SME-scale imports. NOT exhaustive.
...
function findMeasures({ hsCode, originCountry }) {
  if (!hsCode || !originCountry) return [];
  const origin = String(originCountry).toUpperCase();
  const matches = [];
  for (const measure of MEASURES) {
    if (!measure.origins.includes(origin)) continue;
