...
// Sprint 96 — the commitment, per request. The attainment blocks
// aggregate; this puts the promise ON the individual request: due
// time before quoting, verdict after — INCLUDING honest misses
// ("quoted in 52h — outside the 48h commitment"), shown to the
// customer. One derivation, server-side; renderers show the sign
// (ADR 0021).
//
// States:
//   pending — unquoted, inside the budget (hoursRemaining > 0)
//   overdue — unquoted, past the budget (hoursRemaining < 0)
//   met     — quoted, turnaround ≤ target
//   missed  — quoted, turnaround > target (the honest one)
//   null    — no commitment applies: terminal without a quote,
//             or unscoreable stamps (clock skew is dropped, never
//             counted as instant — the attainment rule).
const SLA_PRE_QUOTE_STATUSES = Object.freeze(['submitted', 'processing', 'awaiting_review']);

/**
 * @param {{ createdAt?: string, quotedAt?: string | null, status?: string, targetHours?: number, nowMs?: number, archivedAt?: string | null }} args
 */
function computeSlaCommitment({ createdAt, quotedAt, status, targetHours, nowMs, archivedAt } = {}) {
  // Sprint 105 — an archived request has been withdrawn from the
  // worklist: the cohorts, alerts, and breach sweeps all exclude
  // it, so the banner's 'the team has been alerted' would be
  // FALSE there. No commitment speaks where the machinery does
  // not stand behind it. (A quoted-then-archived row keeps its
  // verdict — the promise was already settled before withdrawal.)
  if (archivedAt && !quotedAt) return null;
  const created = Date.parse(String(createdAt || ''));
  if (!Number.isFinite(created)) return null;
  const target = (Number.isInteger(targetHours) && targetHours > 0)
    ? targetHours
    : SLA_QUOTE_TURNAROUND_TARGET_HOURS;
  const dueAtMs = created + target * 3_600_000;
  if (quotedAt) {
    const quoted = Date.parse(String(quotedAt));
    if (!Number.isFinite(quoted) || quoted < created) return null;
    const quotedInHours = round1((quoted - created) / 3_600_000);
    return {
      targetHours: target,
      state: quotedInHours <= target ? 'met' : 'missed',
      quotedInHours,
      dueAt: new Date(dueAtMs).toISOString(),
    };
  }
  if (!SLA_PRE_QUOTE_STATUSES.includes(String(status))) return null;
  const now = Number.isFinite(nowMs) ? Number(nowMs) : Date.now();
  const hoursRemaining = round1((dueAtMs - now) / 3_600_000);
  return {
    targetHours: target,
    state: hoursRemaining < 0 ? 'overdue' : 'pending',
    hoursRemaining,
    dueAt: new Date(dueAtMs).toISOString(),
  };
}
...
module.exports = {
  ...
  SLA_WINDOW_DAYS,
  SLA_RISK_FRACTION,
  SLA_PRE_QUOTE_STATUSES,
  slaRiskThresholdHours,
  turnaroundHours,
  computeSlaAttainment,
  computeSlaCommitment,
};
