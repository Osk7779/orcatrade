...
// EU-27 member states. Goods of EU origin moving between EU member
// states aren't "imports into the customs territory of the Union"
// (Reg (EU) 2023/956 Art. 2(1) — CBAM applies to goods "imported into
// the customs territory of the Union"). Intra-EU movements are
// internal market trade, not imports, so CBAM doesn't apply.
const EU27 = new Set([
...
// EEA EFTA states + Switzerland are excluded by Reg (EU) 2023/956
// Art. 2(3) because their ETS is integrated with the EU ETS.
const EEA_EFTA_EXEMPT = new Set(['IS', 'LI', 'NO', 'CH']);
...
function determineCbamApplicability({ productCategory, productDescription, originCountry, hsCode }) {
  ...
  const isEuOrigin = EU27.has(origin);
  const isEeaEftaOrigin = EEA_EFTA_EXEMPT.has(origin);
  ...
  // Intra-EU origin: not an import into the customs territory of the
  // Union at all. CBAM Art. 2(1) bounds the regulation to imports
  // FROM outside the customs territory, so DE→PL etc. is out of scope.
  // This was a calculator gap caught by PR #32's slice-3c tests
  // (which had to use Iceland to get an applies:false case where DE
  // would have logically been correct).
  if (isEuOrigin) {
    return {
      applies: false,
      reason: 'Goods of EU origin moving between EU member states are not imports into the customs territory of the Union — CBAM applies only to imports from outside the EU customs territory.',
      categoryKey,
      citation: 'Regulation (EU) 2023/956, Art. 2(1)',
      confidence: 'green',
    };
  }
