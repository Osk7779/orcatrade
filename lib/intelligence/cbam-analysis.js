...
function calculateCertificateExposure({ tonnesGoods, categoryKey, etsPriceEur = ETS_PRICE_SNAPSHOT.priceEurPerTonne }) {
  ...
  return {
    ...
    certificateCostEur: {
      central: Math.round(certificateCost),
      low: Math.round(lowTonnes * ETS_PRICE_SNAPSHOT.scenarioRange.lowEur),
      high: Math.round(highTonnes * ETS_PRICE_SNAPSHOT.scenarioRange.highEur),
    },
    calc: [
...
function calculatePenaltyExposure({ tonnesEmissions, isAuthorisedDeclarant = true }) {
  ...
  if (isAuthorisedDeclarant) {
    return {
      ...
      penaltyEur: Math.round(tonnesEmissions * baseRateEur),
  ...
  return {
    ...
    penaltyEur: Math.round(tonnesEmissions * baseRateEur * 4),
    cumulativeWithCertificateObligation: true,
