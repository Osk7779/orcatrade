...
function validateInput({ customsValueEur, quoteCurrency, paymentTermsDays }) {
  const errors = [];
  const v = Number(customsValueEur);
  if (!Number.isFinite(v) || v <= 0) errors.push('customsValueEur must be > 0');
  if (!fx.isSupported(quoteCurrency)) errors.push(`quoteCurrency must be one of: ${fx.SUPPORTED_CURRENCIES.join(', ')}`);
...
function assessFxRisk({ customsValueEur, quoteCurrency, paymentTermsDays = 60 }) {
  const validation = validateInput({ customsValueEur, quoteCurrency, paymentTermsDays });
  ...
  const equivalentForeign = fx.convertFromEur(valueEur, currency);
  const vol90dPct = fx.VOLATILITY_90D_PCT[currency] || 5.0;

  // Risk scenario: foreign strengthens 5% — cost in EUR rises 5%
  const adverseMovePct = 5.0;
  const riskEur = Math.round(valueEur * (adverseMovePct / 100));

  // Sized to vol-90d adverse case (1-sigma move over 90 days)
  const sigmaRiskEur = Math.round(valueEur * (vol90dPct / 100));

  // Hedge cost via forward: (basis points / 30 days) × days/30
  const hedgeBpPer30d = fx.HEDGE_COST_BP_PER_30D[currency] || 30;
  const hedgeBp = hedgeBpPer30d * (days / 30);
  const hedgeCostEur = Math.round(valueEur * hedgeBp / 10000);
