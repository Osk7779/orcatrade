...
function calculateQuote(input) {
  ...
  const premiumEur = Math.max(Math.round(calculatedPremium * 100) / 100, MIN_PREMIUM_EUR);
  const minPremiumApplied = calculatedPremium < MIN_PREMIUM_EUR;
  const commissionEur = Math.round(premiumEur * COMMISSION_PCT * 100) / 100;
  const netToInsurerEur = Math.round((premiumEur - commissionEur) * 100) / 100;

  // Indicative retail premium (what an SME would pay buying directly without group leverage)
  const retailPremiumEur = Math.round(premiumEur * 1.35 * 100) / 100;
  const savingsVsRetailEur = Math.round((retailPremiumEur - premiumEur) * 100) / 100;
