...
function sanitizeFactoryResults(raw, input = {}) {
  ...
  const fallbackFactories = Array.from({ length: exactFactorySearch ? 1 : 6 }, (_, index) => buildFallbackFactory(filters, index));
  const rawCandidates = raw && Array.isArray(raw.factories) ? raw.factories : [];
  const candidates = exactFactorySearch
    ? [pickExactFactoryCandidate(rawCandidates, requestedFactoryName)]
    : rawCandidates;
