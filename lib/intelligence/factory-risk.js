...
function looksLikeSpecificFactoryName(query, filters) {
  ...
  if (lower.includes('factory') || lower.includes('manufacturer') || lower.includes('supplier') || lower.includes(' in ')) {
    return false;
  }
  if (filters.categoryConstraint && lower.includes(filters.categoryConstraint.toLowerCase())) return false;
  if (filters.countryConstraint && lower.includes(filters.countryConstraint.toLowerCase())) return false;
...
function formatFactoryName(name, city, suffix) {
  ...
  if (/(co\.|ltd\.|limited|industries|manufacturing|group|works|systems)$/i.test(cleaned)) return cleaned;
...
function buildFactoryName(filters, city, speciality, index) {
  ...
  if (index === 0 && looksLikeSpecificFactoryName(filters.query, filters)) {
    return formatFactoryName(filters.query, city, suffix);
  }
...
function sanitiseFactory(candidate, fallback, filters, index) {
  ...
  const regulation = regulationStatusForCategory(`${filters.category} ${speciality}`, complianceScore, riskScore);
  const name = cleanString(factory.name) || fallback.name;
