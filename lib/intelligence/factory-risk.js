...
const KNOWN_CITY_TOKENS = new Set(
  Object.values(COUNTRY_CITIES)
    .flat()
    .flatMap(city => tokenizeFactoryText(city).map(token => token.toLowerCase()))
);
...
function normaliseLooseFactoryText(text) {
...
function buildMeaningfulMarketQueryTokens(filters, requestedFactoryName, options = {}) {
  const sourceText = cleanString(requestedFactoryName || filters.query);
  if (!sourceText) return [];

  const category = filters.categoryConstraint || filters.category;
  const categoryKeywordSet = new Set(getCategoryKeywords(category).map(token => singularizeToken(token)));
  const blockedTokens = new Set([
    ...Array.from(GENERIC_FACTORY_SEARCH_TERMS),
    ...tokenizeFactoryText(filters.country).map(token => token.toLowerCase()),
  ]);

  const tokens = tokenizeFactoryText(sourceText)
    .map(token => cleanString(token).toLowerCase())
    .filter(token => {
      if (token.length <= 1 || blockedTokens.has(token)) return false;
      if (!options.excludeCategoryKeywords) return true;
      return !categoryKeywordSet.has(singularizeToken(token));
    });
...
function countRelevantQueryTokenMatches(tokens, ...texts) {
  if (!Array.isArray(tokens) || !tokens.length) return 0;
  const haystack = texts.map(text => normaliseLooseFactoryText(text)).filter(Boolean).join(' ');
  if (!haystack) return 0;

  return tokens.filter((token) => {
    return buildTokenVariants(token).some(variant => haystack.includes(variant));
  }).length;
}

function hasDirectoryFactoryNameSignal(text) {
  const normalized = normaliseLooseFactoryText(text);
  if (!normalized) return false;

  return DIRECTORY_FACTORIES.some((factory) => {
    const candidate = normaliseLooseFactoryText(factory.name);
    return candidate.includes(normalized) || normalized.includes(candidate);
  });
}

function matchesCategorySearchPhrase(text, filters) {
  const normalized = normaliseLooseFactoryText(text);
  if (!normalized) return false;

  const category = filters.categoryConstraint || filters.category;
  const phrases = [
    ...getCategorySpecialities(category),
    ...getCategoryKeywords(category),
  ];

  return phrases.some((phrase) => {
    const candidate = normaliseLooseFactoryText(phrase);
    if (!candidate) return false;
    if (candidate === normalized) return true;
    if (normalized.includes(candidate) && candidate.split(' ').length > 1) return true;
    if (candidate.includes(normalized) && normalized.split(' ').length > 1) return true;
    return false;
  });
}
...
function looksLikeSpecificFactoryName(query, filters) {
  ...
  if (hasDirectoryFactoryNameSignal(text)) {
    return true;
  }
  ...
  const hasCategoryPhraseMatch = matchesCategorySearchPhrase(text, filters);
  ...
  if (hasCaseSignal && nonGenericTokens.length >= 2 && !hasCategoryPhraseMatch) {
    return true;
  }
...
function looksLikeImplicitFactoryName(query, filters) {
  const text = extractRequestedFactoryName(query, filters);
  if (!text || looksLikeSpecificFactoryName(query, filters)) return false;

  const tokens = tokenizeFactoryText(text).filter(token => !GENERIC_FACTORY_SEARCH_TERMS.has(token));
  if (tokens.length < 2 || tokens.length > 6) {
    return false;
  }

  if (matchesCategorySearchPhrase(text, filters)) {
    return false;
  }

  const categoryKeywords = new Set(
    getCategoryKeywords(filters.categoryConstraint || filters.category)
      .map(keyword => singularizeToken(keyword))
  );
  const nonCategoryTokens = tokens.filter(token => !categoryKeywords.has(singularizeToken(token)));
  const hasKnownCityLead = KNOWN_CITY_TOKENS.has(tokens[0]);
  const tailToken = tokens[tokens.length - 1] || '';
  const hasCompanyLikeTail = /^(packaging|plastics?|textiles?|metals?|metalworks|furniture|electronics|industries|manufacturing|factory|supplier|group|works|tech|technology|trading|export)$/i.test(tailToken);

  if (hasKnownCityLead && nonCategoryTokens.length >= 1) {
    return true;
  }
  if (hasCompanyLikeTail && nonCategoryTokens.length >= 1) {
    return true;
  }
  if (tokens.length >= 3 && nonCategoryTokens.length >= 2) {
    return true;
  }
...
function extractKnownCityFromText(text, country) {
  const normalizedText = normaliseLooseFactoryText(text);
  if (!normalizedText) return '';

  const cityPool = COUNTRY_CITIES[country] || COUNTRY_CITIES.China;
  return cityPool.find(city => normalizedText.includes(normaliseLooseFactoryText(city))) || '';
}
...
function findVerifiedExactFactoryCandidate(candidates, requestedFactoryName) {
  if (!Array.isArray(candidates) || !candidates.length || !requestedFactoryName) return null;
  return candidates.find(candidate => candidateMatchesRequestedFactory(candidate && candidate.name, requestedFactoryName)) || null;
}

function scoreDirectoryFactoryMatch(factory, filters, requestedFactoryName) {
  const queryTokens = buildMeaningfulMarketQueryTokens(filters, requestedFactoryName);
  const specificQueryTokens = buildMeaningfulMarketQueryTokens(filters, requestedFactoryName, {
    excludeCategoryKeywords: true,
  });
  ...
  const matchedQueryTokens = countRelevantQueryTokenMatches(queryTokens, factory.name, factory.speciality);
  const matchedSpecificQueryTokens = countRelevantQueryTokenMatches(specificQueryTokens, factory.name, factory.speciality);

  if (filters.countryConstraint && cleanString(factory.country) !== filters.country) {
    return -1;
  }

  if (!requestedFactoryName) {
    if (specificQueryTokens.length && matchedSpecificQueryTokens === 0) {
      return -1;
    }
    if (!specificQueryTokens.length && queryTokens.length) {
      const minimumMatches = queryTokens.length > 1 ? 2 : 1;
      if (matchedQueryTokens < minimumMatches) {
        return -1;
      }
    }
  }
  ...
  queryTokens.forEach((token) => {
    ...
    if (variants.some(variant => nameText.includes(variant))) score += 10;
    if (variants.some(variant => specialityText.includes(variant))) score += 8;
    if (variants.some(variant => categoryText.includes(variant))) score += 3;
  });
...
function sanitiseFactory(candidate, fallback, filters, index) {
  ...
  const candidateCity = cleanString(factory.city);
  const candidateCountry = cleanString(factory.country);
  const requestedFactoryName = extractRequestedFactoryName(filters.query, filters);
  const exactFactorySearch = requestedFactoryName && looksLikeSpecificFactoryName(filters.query, filters);
  const queryCity = requestedFactoryName ? extractKnownCityFromText(requestedFactoryName, filters.country) : '';
  const city = queryCity || (candidateCity && (
    cityPool.includes(candidateCity) ||
    candidateCountry === filters.country ||
    candidateCountry === (filters.countryConstraint || filters.country)
  ) ? candidateCity : fallback.city);
...
function sanitizeFactoryResults(raw, input = {}, options = {}) {
  const filters = normalizeFactorySearch(input);
  const strictDirectoryOnly = Boolean(options.strictDirectoryOnly);
  const requestedFactoryName = extractRequestedFactoryName(filters.query, filters);
  ...
  const implicitFactorySearch = Boolean(
    requestedFactoryName &&
    !exactFactorySearch &&
    !rawCandidates.length &&
    !directoryCandidates.length &&
    looksLikeImplicitFactoryName(filters.query, filters)
  );

  if (implicitFactorySearch) {
    const fallbackFactory = buildFallbackFactory(filters, 0, searchProfile);
    return {
      factories: [{
        ...fallbackFactory,
        name: requestedFactoryName,
        city: extractKnownCityFromText(requestedFactoryName, filters.country) || fallbackFactory.city,
      }],
      queryMode: 'exact_factory',
      requestedFactoryName,
      directoryMatchCount: 0,
      resultMode: 'provisional_exact_lookup',
    };
  }

  const verifiedRawExactCandidate = exactFactorySearch
    ? findVerifiedExactFactoryCandidate(rawCandidates, requestedFactoryName)
    : null;
  const verifiedDirectoryExactCandidate = exactFactorySearch
    ? findVerifiedExactFactoryCandidate(directoryCandidates, requestedFactoryName)
    : null;
  const candidates = exactFactorySearch
    ? [verifiedRawExactCandidate || verifiedDirectoryExactCandidate]
    : (rawCandidates.length ? rawCandidates : directoryCandidates);
  const failClosedMarketScan = Boolean(strictDirectoryOnly && !exactFactorySearch && !rawCandidates.length);
  const targetFactories = failClosedMarketScan
    ? fallbackFactories.slice(0, directoryCandidates.length)
    : fallbackFactories;
  const usedNames = new Set();

  const factories = targetFactories.map((fallback, index) => {
  ...
  return {
    ...
    resultMode: exactFactorySearch
      ? (verifiedDirectoryExactCandidate ? 'verified_directory_match' : 'provisional_exact_lookup')
      : (
        failClosedMarketScan
          ? (directoryCandidates.length ? 'directory_only_market_scan' : 'no_verified_market_matches')
          : (directoryCandidates.length ? 'hybrid_market_scan' : 'synthetic_market_scan')
      ),
  };
}
