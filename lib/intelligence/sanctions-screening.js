...
// Shown when screening the bundled SAMPLE (real lists not loaded).
const ADVISORY_SAMPLE =
...
// Shown when screening the real loaded consolidated lists. Still never an
// all-clear — lists lag and matching is fuzzy — but accurate about what ran.
const ADVISORY_AUTHORITATIVE =
  'Screened against the loaded consolidated lists (EU / UK OFSI / US OFAC / UN ' +
  'as configured). A non-match is INDICATIVE, not a legal clearance: lists are ' +
  'updated frequently and name-matching is fuzzy, so confirm against the ' +
  'official source before transacting. Not legal advice.';

function advisoryFor(authoritative) {
  return authoritative ? ADVISORY_AUTHORITATIVE : ADVISORY_SAMPLE;
}
...
function screen({ name, list, threshold = DEFAULT_THRESHOLD } = {}) {
  ...
  if (!queryNorm) {
    return { query: String(name || ''), status: 'invalid', reason: 'empty or unscreenable name', matches: [], advisory: advisoryFor(authoritative) };
  }
  ...
  return {
    ...
    authoritative,
    // 'potential_match' = escalate. A non-match is 'no_match' against the real
    // loaded lists, 'no_sample_match' against the sample — NEITHER is a
    // clearance (see advisory); only 'potential_match' is actionable here.
    status: matches.length ? 'potential_match' : (authoritative ? 'no_match' : 'no_sample_match'),
    matchCount: matches.length,
    matches,
    advisory: advisoryFor(authoritative),
  };
}
