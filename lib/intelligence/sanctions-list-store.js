...
// Describe what's currently being screened against: the loaded consolidated
// sources + counts + freshness when real data is present, else the bundled
// sample. Drives the "list status" line in the UI. Never throws.
function sampleMeta() {
  return {
    authoritative: false,
    source: SAMPLE.source,
    totalCount: (SAMPLE.entries || []).length,
    sources: [],
  };
}

async function listMeta() {
  if (!isAvailable()) return sampleMeta();
  try {
    const total = await totalCount();
    if (!total) return sampleMeta();
    const rows = await db.query('SELECT source, entry_count, refreshed_at FROM sanctions_refresh ORDER BY source');
    return {
      authoritative: true,
      source: 'CONSOLIDATED',
      totalCount: total,
      sources: (rows || []).map(r => ({ source: r.source, count: r.entry_count, refreshedAt: r.refreshed_at })),
    };
  } catch (err) {
    log.error('listMeta failed; reporting sample', { err: err.message });
    return sampleMeta();
  }
}
