...
// be scored (no snapshot estimate, zero/negative values, missing
// actual). Estimate arrives as legacy EUR-float snapshot data —
// converted to integer cents AT THE BOUNDARY (fromEuro semantics),
// after which everything is integer arithmetic.
//
// @param {{ landedCents?: number, snapshot?: any }} row
function scoreRow(row) {
  ...
  const estimateCents = Math.round(estimateEur * 100);
