...
// warehouse-quote buildTierAInput. Reads result.inputs.weightKg and
// surfaces it as coverageInput.weightKg.
//
// PR #145: when calculateQuoteAsync has attached `liveFreightMeta` (a
// Eurostat water-transport PPI snapshot, NACE H50), we DROP the
// OrcaTrade PRICING_SNAPSHOT mirror entry and substitute a
// primary_regulator snapshot whose id encodes the Eurostat period +
// area. This is the same PR #132 / #139 / #141 / #143 pattern. TA-2
// then succeeds and the Tier-A pill lights up.
...
function buildTierAInput(quoteResult) {
  ...
  let snapshots;
  if (quoteResult.liveFreightMeta && quoteResult.liveFreightMeta.area && quoteResult.liveFreightMeta.asOfPeriod) {
    snapshots = [{
      id: `eurostat-freight-ppi:${quoteResult.liveFreightMeta.area}@${quoteResult.liveFreightMeta.asOfPeriod}`,
      source_kind: 'primary_regulator',
      as_of_iso: quoteResult.liveFreightMeta.asOf || toIsoStartOfDay(PRICING_SNAPSHOT.asOf),
      // Quarterly Eurostat cadence — same 120-day window as
      // warehouse-quote (PR #144).
      freshness_days: 120,
    }];
  } else {
    snapshots = [{
...
// ── Async surface backed by Eurostat water-transport PPI ─────────────
//
// Mirrors the PR #143 warehouse calculateQuoteAsync shape: wraps the
// sync calculator, asks the Eurostat freight client for a primary-
// source snapshot of the water-transport PPI (NACE H50), and attaches
// `liveFreightMeta` on success.
//
// Falls back to the sync surface when:
//   - opts.useEurostat === false
//   - opts.freightArea is absent/invalid
...
async function calculateQuoteAsync(input, opts = {}) {
  ...
  if (!opts.freightArea) return baseResult;

  const freight = require('./eurostat-freight-client');
  const snapshot = await freight.lookupFreightPpi(opts.freightArea, {
  ...
  return {
    ...
    liveFreightMeta: {
      ...
      verifyUrl: freight.eurostatVerifyUrl(),
    },
  };
}
