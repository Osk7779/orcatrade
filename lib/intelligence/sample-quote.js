...
//
// Money: fees computed in integer cents (lib/intelligence/money.js, half-even
// rounding); the total is the sum of the displayed lines, so it reconciles.
...
function calculateQuote(input) {
  ...
  const consolidationFeeEur = M.toEuro(M.add(M.fromEuro(BASE_CONSOLIDATION_FEE_EUR), M.mulRate(M.fromEuro(PER_SUPPLIER_FEE_EUR), supplierCount)));
  const baseShippingEur = M.toEuro(M.mulRate(M.fromEuro(band.eur), region.multiplier));
  const expressSurchargeEur = isExpress ? EXPRESS_SURCHARGE_EUR : 0;
  const rushSurchargeEur = isRush ? RUSH_TURNAROUND_FEE_EUR : 0;

  // Total is the sum of the displayed line euros, so the breakdown reconciles.
  const totalEur = M.toEuro(M.sum([M.fromEuro(consolidationFeeEur), M.fromEuro(baseShippingEur), M.fromEuro(expressSurchargeEur), M.fromEuro(rushSurchargeEur)]));
