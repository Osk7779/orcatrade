...
/**
 * @typedef {{ filename: string, content: string }} EmailAttachment
 *   filename — what the recipient's mail client shows; should include
 *     the extension so the OS picks the right opener.
 *   content — base64-encoded file contents. Resend accepts inline
 *     base64; we never proxy attachments via URL because that would
 *     leak the asset to anyone who intercepts the bounce.
 */

/**
 * @param {{ to: string, subject: string, text: string, html?: string | null, from?: string | null, attachments?: EmailAttachment[] | null }} args
 */
async function send({ to, subject, text, html = null, from = null, attachments = null }) {
  if (!isConfigured()) {
  ...
  if (Array.isArray(attachments) && attachments.length > 0) {
    // Filter to entries that have both fields populated — a malformed
    // attachment would otherwise make Resend reject the whole send.
    const valid = attachments.filter(
      (a) => a && typeof a.filename === 'string' && a.filename
        && typeof a.content === 'string' && a.content,
    );
    if (valid.length > 0) payload.attachments = valid;
  }
