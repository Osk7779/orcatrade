...
// Fallback sender when RESEND_FROM is unset. Must be on a Resend-verified
// domain: resend.dev senders only deliver to the Resend account owner.
const DEFAULT_FROM = 'OrcaTrade <noreply@orcatrade.pl>';

// The one team mailbox every internal notification (inquiries, leads,
// founding applications, digests, compliance summaries) is delivered to.
// The old per-person @orcatrade.pl Workspace mailboxes no longer exist and
// hard-bounced, which got them onto Resend's suppression list — so every
// internal email was silently dropped. One address, overridable per env.
const TEAM_INBOX_DEFAULT = 'orcatrade@orcatradegroup.com';

function teamInbox() {
  return (process.env.ORCATRADE_TEAM_INBOX || '').trim() || TEAM_INBOX_DEFAULT;
}
