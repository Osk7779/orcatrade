...
module.exports = async (req, res) => {
  ...
  // Fire-and-forget view increment + audit. We don't want a slow
  // KV write to delay the redirect, but we also don't want to lose
  // the count entirely. KV reads are fast in practice; if this ever
  // becomes a bottleneck, move to a deferred queue.
  ...
  // Redirect to the wizard with the encoded inputs.
  const encoded = encodeShareInputs(record.inputs);
  const target = '/start/?p=' + encoded + '&from=share';
  res.statusCode = 302;
