...
// Which email stream to unsubscribe from. Backwards-compatible: no `stream`
// param (every email sent before this sprint) → planRevisionEmails. Any value
// must be a known pref key, else we fall back to planRevisionEmails rather than
// silently flipping nothing.
function readStream(req) {
  let raw = '';
  if (req.query && req.query.stream) raw = String(req.query.stream);
  else {
    const qs = (req.url || '').split('?')[1] || '';
    raw = new URLSearchParams(qs).get('stream') || '';
  }
  return notificationPrefs.PREF_KEYS.includes(raw) ? raw : 'planRevisionEmails';
}

// Friendly copy per stream for the confirmation page + audit event.
const STREAM_COPY = {
  planRevisionEmails: {
    ...
    event: 'plan_revision_emails_unsubscribed',
  },
  weeklyDigestEmails: {
    title: "You're unsubscribed from the weekly digest",
    message: "We won't send you the weekly portfolio digest any more. Your account and other features are unchanged. You can re-enable this any time from your preferences page.",
    event: 'weekly_digest_emails_unsubscribed',
  },
  complianceDeadlineEmails: {
    title: "You're unsubscribed from compliance deadline reminders",
    message: "We won't remind you about upcoming CBAM/EUDR deadlines on your saved plans any more. Your account and other features are unchanged. You can re-enable this any time from your preferences page.",
    event: 'compliance_deadline_emails_unsubscribed',
  },
  monitoringAlerts: {
    title: "You're unsubscribed from monitoring alerts",
    message: "We won't email you the monitoring digest about cost drift, FX exposure, or sanctions updates on your saved plans any more. The alerts still appear on your monitoring alerts page. You can re-enable the email any time from your preferences page.",
    event: 'monitoring_alerts_unsubscribed',
  },
};
...
module.exports = async (req, res) => {
  ...
  try {
    await notificationPrefs.setPrefs(email, { [stream]: false });
  } catch (err) {
  ...
  try {
    await events.record(STREAM_COPY[stream].event, { email, stream });
  } catch (_) {}
  ...
  return res.end(renderPage({
    status: 'ok',
    title: STREAM_COPY[stream].title,
    message: STREAM_COPY[stream].message,
    ctaHref: SITE_ORIGIN + '/account/preferences/',
