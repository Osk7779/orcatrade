...
async function handlePatch(req, res, ctx) {
  ...
  // change nor a reset, refuse — the audit event would carry
  // nothing meaningful and the reason (if any) would attach to
  // an empty operation.
  if (Object.keys(knobPatch).length === 0 && resetResult.keys.length === 0) {
    return jsonResponse(res, 400, {
      error: 'PATCH must include at least one knob change or reset',
    });
  }
