...
const { verifyQuoteStudioAccess } = require('../quote-studio-auth');
const { consumeRateLimit } = require('../intelligence/runtime-store');
const { requestAnthropicMessage, extractAnthropicText } = require('../ai/model-runtime');
...
module.exports = async (req, res) => {
  ...
  // Internal team tool — gate on the dedicated Quote Studio allowlist (or the
  // admin session / token). Applies to every method, including the probe.
  const verdict = await verifyQuoteStudioAccess(req);

  // GET → lightweight auth probe so the page can show a sign-in gate on load
  // without leaking the tool to people who aren't on the team.
  if (req.method === 'GET') {
    if (!verdict.ok) return res.status(verdict.statusCode).json({ ok: false, authed: false, error: verdict.error });
    return res.status(200).json({ ok: true, authed: true, email: verdict.mode === 'session' ? verdict.email : null });
  }
