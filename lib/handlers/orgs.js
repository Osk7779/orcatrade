...
module.exports = async (req, res) => {
  ...
  // Admin paths (Sprint BG-3.3 phase 1 + BG-3.6) — both bypass the
  // user-session gate. Token via X-Admin-Token header OR ?token=…
  //
  ...
  const isAdminRoute = (orgId === 'admin' && !action)
    || (orgId && action === 'tier');
  if (isAdminRoute) {
    const expected = adminToken();
    if (!expected) {
      return jsonResponse(res, 503, { error: 'Admin endpoint not configured (ORCATRADE_LEADS_TOKEN unset)' });
    }
