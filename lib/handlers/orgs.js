...
//   POST /api/orgs/<id>/role        → change a member's role (ORG_MEMBERS_MANAGE)
...
// Resolve the actor's role in the org and gate on an RBAC permission. Returns
// the actor's role on success; on failure sends a 403 and returns null. Single
// choke-point so every org action enforces the same matrix (lib/rbac.js).
async function requirePermission(req, res, user, orgId, permission) {
  const role = await orgs.getMemberRole(orgId, user.email);
  if (!role || !rbac.can(role, permission)) {
    jsonResponse(res, 403, { error: 'You do not have permission to perform this action', requiredPermission: permission });
    return null;
  }
  return role;
}
...
async function handleCreate(req, res, user) {
  ...
  // ADR 0005 — audit-log writes propagate; dispatcher returns 5xx on failure.
  // Phase 1 reorders to audit-FIRST + explicit 503.
  await events.record('org_created', { email: user.email, orgId: record.id, orgName: record.name });
...
async function handleGet(req, res, user, orgId) {
  ...
  const myRole = await orgs.getMemberRole(orgId, user.email);
  if (!myRole) return jsonResponse(res, 403, { error: 'not a member of this org' });
  const members = await orgs.listMembers(orgId);
  // Hand the UI everything it needs to render the right controls: the viewer's
  // own role, whether they can manage members, and which roles they may assign.
  return jsonResponse(res, 200, {
    ok: true,
    org,
    members,
    myRole,
    canManageMembers: rbac.can(myRole, rbac.PERMISSIONS.ORG_MEMBERS_MANAGE),
    assignableRoles: rbac.assignableRoles(myRole),
  });
}
...
async function handleInvite(req, res, user, orgId) {
  ...
  // Legacy 'member' from older clients normalises to its canonical role (viewer).
  const role = rbac.canonicalRole(body.role || 'viewer');
  ...
  // Must hold the member-management permission on this org (owner/admin).
  const actorRole = await requirePermission(req, res, user, orgId, rbac.PERMISSIONS.ORG_MEMBERS_MANAGE);
  if (!actorRole) return; // 403 already sent
  // The invited role must be one the actor is allowed to grant (never owner).
  const grantable = rbac.assignableRoles(actorRole);
  if (!grantable.includes(role)) {
    return jsonResponse(res, 400, { error: `role must be one of: ${grantable.join(', ')}` });
  }
  try {
...
// POST /api/orgs/<id>/role  body: { email, role }
//   Change an existing member's role. Requires ORG_MEMBERS_MANAGE (owner/admin).
//   The new role must be one the actor may grant (assignableRoles; never owner —
//   ownership moves only via /transfer). Legacy 'member' input → viewer.
async function handleSetRole(req, res, user, orgId) {
  const body = req.body || {};
  const email = body.email;
  const role = rbac.canonicalRole(body.role);
  if (!email) return jsonResponse(res, 400, { error: 'email required' });
  ...
  let result;
  try {
    result = await orgs.changeMemberRole(orgId, { email, role });
  } catch (err) {
    return jsonResponse(res, 400, { error: err.message });
  }
  if (!result.changed) {
    // not-a-member / cannot-change-owner → 4xx; no-change → 200 idempotent.
    if (result.reason === 'no-change') return jsonResponse(res, 200, { ok: true, member: result.member });
    const status = result.reason === 'not-a-member' ? 404 : 403;
    return jsonResponse(res, status, { error: result.reason });
  }
  // ADR 0005 — audit-log write propagates.
  await events.record('org_member_role_changed', {
    email: user.email,            // actor
    orgId,
    targetEmail: orgs.normaliseEmail(email),
    role,
  });
  return jsonResponse(res, 200, { ok: true, member: result.member });
}
...
async function handleClearTier(req, res, orgId) {
  ...
  // ADR 0005 — audit-log write propagates.
  await events.record('org_tier_cleared', { orgId });
...
module.exports = async (req, res) => {
  ...
  // /api/orgs/<id>/scim — owner-gated SCIM token management (GET status,
  // POST mint/rotate, DELETE revoke). The IdP-facing provisioning lives at
  // /api/scim/v2/* and authenticates with the token minted here.
  if (action === 'scim') {
    if (req.method === 'GET') return handleGetScim(req, res, user, orgId);
    if (req.method === 'POST') return handleGenScim(req, res, user, orgId);
    if (req.method === 'DELETE') return handleRevokeScim(req, res, user, orgId);
    return jsonResponse(res, 405, { error: 'Method not allowed' });
  }
  if (req.method !== 'POST') {
    return jsonResponse(res, 405, { error: 'Method not allowed' });
  ...
  if (action === 'role') return handleSetRole(req, res, user, orgId);
...
async function handleSetSso(req, res, user, orgId) {
  ...
  // ADR 0005 — audit-log write propagates.
  await events.record('org_sso_configured', { email: user.email, orgId });
  return jsonResponse(res, 200, {
    ok: true,
    initiateUrl: `${SITE_ORIGIN}/api/auth/sso/initiate?org=${encodeURIComponent(orgId)}`,
  });
...
async function handleDeleteSso(req, res, user, orgId) {
  ...
  // ADR 0005 — audit-log write propagates.
  await events.record('org_sso_removed', { email: user.email, orgId });
  return jsonResponse(res, 200, { ok: true });
}

// ── Owner-gated SCIM token management (apex III1 — SCIM slice 1) ──
//
// The SCIM token lets the customer's IdP provision/deprovision org members, so
// it is owner-level trust (same gate as SSO config). The raw token is returned
// exactly once on mint; thereafter only its status is visible.
async function handleGetScim(req, res, user, orgId) {
  if (!(await requireOwner(req, res, user, orgId))) return;
  const status = await scimStore.getStatus(orgId);
  return jsonResponse(res, 200, {
    ok: true,
    ...status,
    endpoint: `${SITE_ORIGIN}/api/scim/v2`,
  });
}

async function handleGenScim(req, res, user, orgId) {
  if (!(await requireOwner(req, res, user, orgId))) return;
  const { token, createdAt } = await scimStore.generateToken(orgId);
  // ADR 0005 — audit-log write propagates.
  await events.record('org_scim_token_minted', { email: user.email, orgId });
  // Raw token returned ONCE — the UI must tell the owner to copy it now.
  return jsonResponse(res, 200, {
    ok: true,
    token,
    createdAt,
...
async function handleRevokeScim(req, res, user, orgId) {
  if (!(await requireOwner(req, res, user, orgId))) return;
  await scimStore.revoke(orgId);
  // ADR 0005 — audit-log write propagates.
  await events.record('org_scim_token_revoked', { email: user.email, orgId });
  return jsonResponse(res, 200, { ok: true });
}
...
module.exports.handleSetRole = handleSetRole;
...
module.exports.handleGetScim = handleGetScim;
module.exports.handleGenScim = handleGenScim;
module.exports.handleRevokeScim = handleRevokeScim;
