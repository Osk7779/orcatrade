...
async function handleGenScim(req, res, user, orgId) {
  ...
  try { await events.record('org_scim_token_minted', { email: user.email, orgId }); } catch (_) {}
...
async function handleRevokeScim(req, res, user, orgId) {
  ...
  try { await events.record('org_scim_token_revoked', { email: user.email, orgId }); } catch (_) {}
  return jsonResponse(res, 200, { ok: true });
}
