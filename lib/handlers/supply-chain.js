const { COUNTRY_ROUTE_CONTEXT } = require('../intelligence/catalog');
// Wrapped in lib/circuit.js per ADR 0006. Same shared 'anthropic-messages'
// circuit as the 5 agent handlers — one sustained outage trips one circuit
...
} = require('../intelligence/supply-chain');
...
Return this exact JSON shape:
{
  ...
  "recommendations": ["string", "string", "string"]
    ...
    const data = await circuit.run('anthropic-messages', async () => {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Anthropic ${response.status}: ${errorText.slice(0, 400)}`);
      }
      return await response.json();
    }, {
      // Fallback: throw → caught by the outer try/catch which returns
      // the supply-chain mock (existing graceful-degradation path).
