...
const personalTools = [
  ...
  {
    name: 'getMyOrgServiceLevels',
    description: "Report the signed-in user's ORGANISATION's measured service levels: quote-turnaround and first-response SLA attainment against the org's (possibly contract-negotiated) targets, the live SLA-at-risk snapshot (requests approaching or past the turnaround promise RIGHT NOW), and the org's quote-accuracy ledger (their reported actual outcomes vs the quotes). Every figure is deterministic and computed by the platform's calculators — cite this tool for every number you state. HONESTY TIERS: a metric with tier 'insufficient' has its figures withheld (null) because fewer than 10 scoreable data points exist — report that plainly as 'still accruing' and state the sample count; NEVER invent or estimate a percentage in its place. 'indicative' means an early sample (label it as such); 'measured' is the full instrument. Use when the user asks 'how are we doing against our SLA', 'how fast do you actually quote us', 'how accurate are your quotes for us', or 'is anything at risk of missing its deadline right now'. If the user belongs to multiple organisations this reports the first; name the organisation in your answer. Read-only.",
    input_schema: { type: 'object', properties: {}, additionalProperties: false },
  },
  {
    name: 'recallMemory',
    description: "Retrieve what you've previously remembered about THIS signed-in user — their stated preferences, supplier facts, and working context carried across conversations (e.g. 'their target margin', 'their main supplier', 'their preferred routing'). Call this EARLY in a conversation, before asking the user something they may have already told you. Returns a list of { key, value, kind } the user has had you store. Read-only.",
    input_schema: {
      type: 'object',
      properties: {
        key: { type: 'string', description: 'Optional: fetch one memory by its key (e.g. "target-margin"). Omit to list everything you remember about this user.' },
      },
      additionalProperties: false,
    },
  },
  {
    ...
    description: "Persist a durable fact, preference, or piece of context about THIS signed-in user so you (and the agent in future conversations) can recall it later. Use this ONLY for stable, reusable facts the user states about how THEY operate — 'remember our supplier is Shenzhen Acme', 'our target margin is 35%', 'we always clear through Rotterdam'. Do NOT store one-off question details, sensitive personal data, or anything the user didn't intend to be remembered. Pick a short stable key so re-saving the same fact overwrites rather than duplicates. The value is the user's data, returned only to them.",
    input_schema: {
      type: 'object',
      properties: {
        key: { type: 'string', description: 'A short stable slug for the fact, e.g. "target-margin" or "main-supplier".' },
        value: { type: 'string', description: 'The fact to remember, in plain language.' },
        kind: { type: 'string', enum: ['preference', 'fact', 'context'], description: 'Optional category. Default "fact".' },
      },
      required: ['key', 'value'],
      additionalProperties: false,
    },
  ...
  {
    name: 'forgetForUser',
    description: "Delete a previously-stored memory for the signed-in user, by its key. Use when the user says 'forget that', 'that's no longer true', or corrects a fact you'd stored. Returns whether a memory was removed.",
    input_schema: {
      type: 'object',
      properties: {
        key: { type: 'string', description: 'The key of the memory to forget (from recallMemory).' },
      },
      required: ['key'],
      additionalProperties: false,
    },
...
function buildPersonalImpls(email) {
  ...
  return {
    ...
    getMyOrgServiceLevels: async () => {
      if (!e) return { error: 'not signed in' };
      const orgs = require('../orgs');
      // Membership derives from the CLOSURED email — the model can
      // never point this at another org.
      const memberOrgs = await orgs.listOrgsForEmail(e);
      if (!Array.isArray(memberOrgs) || memberOrgs.length === 0) {
        return { error: 'no organisation on this account yet' };
      }
      const org = memberOrgs[0];
      let orgIdNumeric = typeof org.dbId === 'number' ? org.dbId : null;
      if (orgIdNumeric === null) {
      ...
      if (!Number.isFinite(orgIdNumeric)) {
        return { error: 'organisation not provisioned for measured service levels yet' };
      }
      // Single source of truth (ADR 0021): the SAME aggregation the
      // cockpit + digest use, with the FULL org config threaded so
      // attainment measures against the negotiated targets.
      ...
      if (!agg || !agg.ok) return { error: 'service levels unavailable right now' };
      const i = agg.insights || {};
      const atRisk = i.slaAtRisk || null;
      return {
        orgName: org.name || null,
        slaQuoteTurnaround: i.slaQuoteTurnaround || null,
        slaFirstResponse: i.slaFirstResponse || null,
        // Counts + thresholds only — the model narrates urgency;
        // the per-row worklist lives on the cockpit + daily alert.
        slaAtRisk: atRisk ? {
          riskThresholdHours: atRisk.riskThresholdHours,
          targetHours: atRisk.targetHours,
          atRiskCount: atRisk.atRiskCount,
          breachedCount: atRisk.breachedCount,
        } : null,
        accuracyLedger: i.accuracyLedger || null,
        tiers: "insufficient = figures withheld until 10 scoreable data points (report as 'still accruing'); indicative = early sample (< 50); measured = 50+",
      };
    },
    recallMemory: async (input) => {
      if (!e) return { count: 0, memories: [] };
      const key = input && typeof input.key === 'string' ? input.key.trim() : '';
      if (key) {
        const one = await agentMemory.recall(e, key);
        return one ? { count: 1, memories: [one] } : { count: 0, memories: [] };
      }
      const all = await agentMemory.list(e);
      return { count: all.length, memories: all };
    },
    rememberForUser: async (input) => {
      if (!e) return { error: 'not signed in' };
      const res = await agentMemory.remember(e, {
        key: input && input.key,
    ...
    forgetForUser: async (input) => {
      if (!e) return { error: 'not signed in' };
      const removed = await agentMemory.forget(e, input && input.key);
      return { removed };
    },
  };
}
