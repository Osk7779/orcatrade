...
// the team inbox (lib/email.js teamInbox) via Resend (no email sent if RESEND_API_KEY unset —
...
const log = require('../log').withContext({ handler: 'founding' });

const FOUNDING_LIMIT = 10;
const { teamInbox } = require('../email');
...
// Internal notification to the team inbox — operator sees the
...
async function sendInternalEmail({ name, email, company, role, monthlyValueEur, message, locale, waitlist }) {
  ...
  return resendSend({
    from: require('../email').resolveFrom(),
    to: teamInbox(),
    subject,
    text,
    replyTo: email,
...
// reply_to is the team inbox so any reply from the applicant lands in
...
module.exports = async (req, res) => {
  ...
  const reqLog = log.withContext({ requestId: req.requestId, action: 'apply', waitlist: overLimit });
  reqLog.info('founding application accepted', { email, locale });
  const [internal, applicant] = await Promise.all([
    sendInternalEmail({ name, email, company, role, monthlyValueEur, message, locale, waitlist: overLimit })
      .catch((err) => { reqLog.error('internal email threw', { err: err && err.message }); return { sent: false, reason: 'exception' }; }),
    sendApplicantEmail({ name, email, locale, waitlist: overLimit })
      .catch((err) => { reqLog.error('applicant email threw', { err: err && err.message }); return { sent: false, reason: 'exception' }; }),
  ]);
