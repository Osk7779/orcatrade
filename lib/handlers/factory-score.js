...
Return exactly this JSON:
{
  ...
  try {
    ...
    if (!apiRes.ok) {
      // Surface Claude's actual error text — without it, the upstream
      // 400/403/429 was opaque ("Claude API error 400") with no clue
      // whether it's a model/permission/budget issue.
      let errBody = '';
      try { errBody = await apiRes.text(); } catch (_) {}
      throw new Error('Claude API ' + apiRes.status + ': ' + errBody.slice(0, 240));
    }
