...
// Subset of event types that belong on a goods master's audit timeline.
// Mirrors the SHIPMENT_TIMELINE_EVENT_TYPES pattern in shipments.js —
// filtered server-side so the client never has to render an internal
// system event that happens to carry the same entityId.
const GOODS_TIMELINE_EVENT_TYPES = new Set([
...
// carry. Identical contract to shipments.js's redactor — keeping it
// per-handler keeps the privacy contract local to each entity type
// without coupling handlers via a shared timeline helper.
...
function pathInfoFromUrl(req) {
  ...
  // /api/goods/<externalId>/<action>   → ['goods', '<externalId>', '<action>']
...
async function handleHistory(req, res, ctx, externalId) {
  // Confirm the goods record belongs to the user's org BEFORE returning
  // any audit data — even with a guess at an externalId, a non-owner
  // must see a 404 (not "this good exists, just not yours"). Same
  // ownership check shipments.handleHistory makes.
  const fetched = await goods.getGoodsByExternalId({
    orgId: ctx.orgIdNumeric,
    externalId,
  });
  if (!fetched.ok) {
  ...
  try {
    ...
    const filtered = raw
      .filter((e) => GOODS_TIMELINE_EVENT_TYPES.has(e.type))
    ...
    return jsonResponse(res, 500, { error: 'Could not read goods history' });
  }
}
