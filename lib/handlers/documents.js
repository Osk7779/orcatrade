...
module.exports = async (req, res) => {
  ...
  const { type, data, fromPlan } = req.body || {};
  if (!type) {
    return res.status(400).json({ error: '`type` is required' });
  }

  // Sprint document-prefill-v1 — pass `fromPlan` (an Import Plan's inputs) to
  // pre-fill the document, then merge any explicit `data` over the draft so the
  // caller can fill the placeholder parties / override fields in one call.
  let docData = data;
  ...
  if (!docData || typeof docData !== 'object') {
    return res.status(400).json({ error: 'Provide `data`, or `fromPlan` to pre-fill from a plan.' });
  }
