...
const { streamAnthropicMessage } = require('../intelligence/model-runtime');
...
function normaliseInput(body = {}) {
  return {
    ...
    // `brief` is the freeform single-line input from the marketing-shell
    // ToolPage on /analysis. Treat it as a productDescription alias so
    // the brief-style entry path doesn't 400 — the rest of the handler
    // already does keyword extraction from the description.
    productDescription: cleanString(body.productDescription || body.brief),
