...
const { aggregatePortfolio, comparePortfolioSnapshots } = require('../intelligence/portfolio-aggregate');
...
// Shared fan-out: compose every line in parallel (composePlan validates
// each + returns { ok:false, errors } on bad lines, kept per-line so one
// bad SKU doesn't sink the portfolio), then aggregate. Returns
// { okPlans, lineErrors, aggregate, responseLines } or null when no line
// composed OK. Used by both the anonymous compute and the authed refresh.
async function composeAndAggregate(rawLines) {
  ...
  if (okPlans.length === 0) return { okPlans, lineErrors, aggregate: null, responseLines: [] };
  const aggregate = aggregatePortfolio(okPlans, { brokerageFee: customs.brokerageFee });
  const responseLines = okPlans.map((p) => ({
    inputs: p.inputs,
    totals: p.totals,
  ...
  return { okPlans, lineErrors, aggregate, responseLines };
}
...
async function handleCompute(req, res) {
  ...
  const { aggregate, responseLines, lineErrors } = await composeAndAggregate(rawLines);
  if (!aggregate) return res.status(400).json({ error: 'No valid lines', lineErrors });
...
// POST /api/portfolio/csv — recompute { lines } and stream the result as
// an RFC-4180 CSV download (per-SKU breakdown + totals + summary).
// Public + anonymous like the compute path; a deliberate user action, so
// the extra fan-out is fine. Reuses composeAndAggregate so the CSV always
// matches what the on-screen plan would show.
async function handleCsv(req, res) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });
  const ip = req.headers['x-forwarded-for']?.split(',')[0]?.trim() || 'unknown';
  const rate = await consumeRateLimit('portfolio-csv', ip, 12, 60 * 1000);
  if (rate.limited) return res.status(429).json({ error: 'Too many requests. Please wait a moment.' });

  const body = req.body || {};
  const rawLines = Array.isArray(body.lines) ? body.lines : null;
  if (!rawLines || rawLines.length === 0) return res.status(400).json({ error: 'lines[] required' });
  if (rawLines.length > MAX_LINES) return res.status(400).json({ error: `Too many lines (max ${MAX_LINES}).` });

  const { aggregate, responseLines } = await composeAndAggregate(rawLines);
  if (!aggregate) return res.status(400).json({ error: 'No valid lines' });

  const csv = portfolioCsv.portfolioToCsv(aggregate, responseLines);
  res.statusCode = 200;
  res.setHeader('Content-Type', 'text/csv; charset=utf-8');
  res.setHeader('Content-Disposition', `attachment; filename="${portfolioCsv.csvFilename()}"`);
...
async function handleSave(req, res) {
  ...
  try {
    ...
    // failure (Phase 1 reorders to audit-FIRST + explicit 503).
    await events.record('portfolio_saved', { lineCount: record.lines.length });
...
async function handleShare(req, res, id) {
  ...
  if (req.method === 'POST') {
    ...
    await events.record('portfolio_share_created', { code: share.code });
...
// POST /api/portfolio/item/<id>/refresh — recompute a saved portfolio's
// stored lines against today's data, diff vs the snapshot saved at save
// time, and return the fresh aggregate + drift. This is the "what changed
// since you saved this" capability — the portfolio analogue of the
// single-plan revision check.
async function handleRefresh(req, res, id) {
  ...
  const record = await savedPortfolios.getPortfolio(id, user.email);
  if (!record) return json(res, 404, { error: 'Portfolio not found' });
  const { aggregate, responseLines, lineErrors } = await composeAndAggregate(record.lines || []);
  if (!aggregate) return json(res, 400, { error: 'Saved portfolio has no recomputable lines', lineErrors });
...
async function handleSharedRead(req, res, code) {
  ...
  // View increment is read-side telemetry — non-blocking is acceptable.
  savedPortfolios.incrementShareViews(code).catch(() => {});
  // ADR 0005: opening a share creates an audit row; propagate on failure.
  await events.record('portfolio_share_opened', { code: String(code).toLowerCase() });
...
module.exports = async (req, res) => {
  ...
  if (action === 'csv') return handleCsv(req, res);
  ...
  if (action === 'item') {
    ...
    if (sub === 'refresh') return handleRefresh(req, res, id);
...
module.exports.handleRefresh = handleRefresh;
module.exports.handleCsv = handleCsv;
module.exports.composeAndAggregate = composeAndAggregate;
