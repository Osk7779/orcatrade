const { teamInbox, resolveFrom } = require('../email');

const { cleanString } = require('../intelligence/catalog');
const { createCacheKey } = require('../intelligence/cache-store');
const {
  consumeRateLimit,
  getSharedCacheValue,
  ...
  setSharedCacheValue,
} = require('../intelligence/runtime-store');
const { attachReportAccess } = require('../intelligence/report-access');
const { extractAnthropicText, requestAnthropicMessage } = require('../intelligence/model-runtime');
const { MODELS, cachedSystem } = require('../ai/models');
const { validateCompliancePayload } = require('../intelligence/compliance-validator');
const { applyRequestHeaders, buildRequestContext, emitAuditEvent } = require('../intelligence/request-runtime');
const {
  buildDeterministicFallbackReport,
  enforceComplianceLogic,
  resolveAsOfDate,
  RULE_VERSION,
} = require('../intelligence/compliance');
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const systemPromptStatic = `You are OrcaTrade Intelligence, a senior EU trade compliance engine with expert knowledge of:
...
DETERMINISTIC PRE-CHECK — YOU MUST RESPECT THIS UNLESS USER DATA CLEARLY CONTRADICTS IT`;
    ...
    // Split here so the large static head caches (shared across all reports);
    // the per-request applicability block rides in an uncached trailing block.
...
Return ONLY this JSON object with no markdown wrapping:
{
  ...
  "disclaimer": "This report is generated by OrcaTrade Intelligence based on information provided by the user. It does not constitute legal advice and should not be relied upon as such. For binding legal opinions on EU trade compliance obligations, consult a qualified EU trade law practitioner. Report ID: ${reportId}. Generated: ${timestamp}."
    ...
    const modelResponse = await requestAnthropicMessage({
      ...
      model: MODELS.AGENT,
      maxTokens: 8000,
      system: cachedSystem(systemPromptStatic, systemPromptDynamic),
