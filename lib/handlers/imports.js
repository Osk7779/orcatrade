...
//   POST   /api/imports/<externalId>/process         → run the calculator orchestrator (202 + request)
...
//   POST   /api/imports/<externalId>/restore         → un-archive (sprint 76)
//   POST   /api/imports/<externalId>/actual          → report actual landed outcome (sprint 81)
//   POST   /api/imports/bulk-archive                 → archive up to 50 (sprint 74)
//   POST   /api/imports/bulk-restore                 → un-archive up to 50 (sprint 76)
//
...
//
// /process runs the calculator orchestrator synchronously (v1:
// <1s in-process wall clock) — submitted → processing → shortlist +
// quote attached → awaiting_review, 202 with the updated request.
// (Header previously claimed a 501 stub long after the wiring
// shipped — corrected sprint 101; the record must not understate
// the product any more than it may overstate it.)
...
const IMPORT_REQUEST_TIMELINE_EVENT_TYPES = new Set([
  ...
  // Sprint 76 — restore pairs with archived on the timeline so the
  // narrative reads archive → restore, never a silent reappearance.
  'import_request_restored',
  // Sprint 81 — the accuracy loop closing belongs on the narrative
  // timeline: quoted → approved → actual reported.
  'import_request_actual_reported',
  // Sprint 97 — a breach is part of the request's story.
...
const IMPORT_REQUEST_AUDIT_EVENT_TYPES = new Set([
  ...
  'import_request_restored',
  // Sprint 81 — actual outcome reported.
  'import_request_actual_reported',
  // Sprint 97 — SLA breach recorded (system actor).
  'import_request_sla_breached',
  // Sprint 18 — messaging.
...
async function handleList(req, res, ctx) {
  ...
  const result = await importRequests.listImportRequestsForOrg({
    ...
    // Sprint 103 — server-side archived-only cut (list + export
    // share the taxonomy so the CSV mirrors the view exactly).
    archivedOnly: q.archivedOnly === '1' || q.archivedOnly === 'true',
  ...
  return jsonResponse(res, 200, {
    ok: true,
    importRequests: augmented,
    // Sprint 104 — no silent caps: the true match count rides
    // along so the UI can say 'showing X of Y' when truncated.
    totalMatching: Number.isFinite(Number(result.totalMatching))
      ? Number(result.totalMatching)
      : augmented.length,
  });
}
...
async function handleGet(req, res, ctx, externalId) {
  ...
  const augmented = {
    ...
    // before quoting, verdict after (including honest misses,
    // shown to the customer). Derived once here against the org's
    // NEGOTIATED target (fail-open to the platform default inside
    // getOperatorConfig); renderers show the sign only. Null when
    // no commitment applies (terminal without a quote).
    slaCommitment: await (async () => {
      try {
        const operatorConfig = require('../operator-config');
        const slaCalc = require('../intelligence/sla');
        const orgConfig = await operatorConfig.getOperatorConfig(ctx.orgIdNumeric);
        return slaCalc.computeSlaCommitment({
          createdAt: r.createdAt,
          quotedAt: r.quotedAt,
          status: r.status,
          targetHours: orgConfig.slaQuoteTurnaroundTargetHours,
          // Sprint 105 — archived rows are off the worklist; the
          // pending/overdue claims must not speak there.
          archivedAt: r.archivedAt,
        });
      } catch (_) {
        // Display augmentation only — never fail the detail read.
        return null;
      }
    })(),
  };
...
async function handleInsights(req, res, ctx) {
  ...
  // Sprint 88 — BUG FIX: thread the per-org knobs into the
  // aggregation. Since sprint 42 the cron ALERT paths loaded org
  // config, but this cockpit path never did — so the dashboard the
  // customer tunes their knobs against kept using the platform
  // defaults while the emails honoured the overrides. All five
  // knobs travel; getOperatorConfig fail-opens to defaults so a
  // KV blip degrades to pre-fix behaviour, never a 500.
  const operatorConfig = require('../operator-config');
  const orgConfig = await operatorConfig.getOperatorConfig(ctx.orgIdNumeric);
  const result = await importRequests.aggregateOpsInsights({
    ...
    // Sprint 89 — negotiated SLA targets (knobs 6 & 7). The org's
    // cockpit attainment measures against THEIR contract numbers;
    // the public /api/sla page and the cross-org triage risk line
    // stay on the platform targets.
...
// Sprint 81 — report the actual landed outcome (Track B phase 2).
// Org-scoped; BOTH customer and ops can report (unlike ratings —
// ops often learns the outcome first from carrier + customs
// settlement). Status gate (customer_approved) + validation live
// in the data layer; supersession is allowed and audit-preserved.
async function handleReportActual(req, res, ctx, externalId) {
  const body = (req.body && typeof req.body === 'object') ? req.body : {};
  const result = await importRequests.recordImportRequestActual({
    orgId: ctx.orgIdNumeric,
    externalId,
    actorEmailHash: ctx.emailHash,
    landedEur: body.landedEur,
    notes: typeof body.notes === 'string' ? body.notes : undefined,
  });
  if (!result.ok) {
    if (result.notFound) return jsonResponse(res, 404, { error: 'Not found' });
    if (result.conflict) return jsonResponse(res, 409, { error: result.errors[0] });
    if (/not configured/i.test(result.errors[0])) return jsonResponse(res, 503, { error: result.errors[0] });
    if (result.errors.some((e) => /required|must be|<=/.test(e))) {
      return jsonResponse(res, 400, { error: result.errors[0] });
    }
    return jsonResponse(res, 500, { error: result.errors[0] });
  }
  return jsonResponse(res, 200, {
    ...
    isSupersession: result.isSupersession || false,
  });
}

// Sprint 76 — restore an archived request. Parity RBAC with the
// DELETE archive it mirrors: ungated, org-scoped at the data
// layer. Idempotent (restoring a live row → unchanged: true).
async function handleRestore(req, res, ctx, externalId) {
  const result = await importRequests.restoreImportRequest({
    orgId: ctx.orgIdNumeric,
    externalId,
    actorEmailHash: ctx.emailHash,
  });
...
// Sprint 76 — bulk restore. Mirror of handleBulkArchive below:
// same parity-RBAC posture (ungated because the single restore
// is ungated), same 503/400 mapping, same three-outcome report.
async function handleBulkRestore(req, res, ctx) {
  const body = (req.body && typeof req.body === 'object') ? req.body : {};
  const result = await importRequests.bulkRestoreImportRequests({
    orgId: ctx.orgIdNumeric,
    externalIds: Array.isArray(body.externalIds) ? body.externalIds : [],
    actorEmailHash: ctx.emailHash,
  });
...
// Sprint 74 — bulk archive from the imports list. Deliberately NO
// ops-role gate: it wraps the same per-row path as the single
// DELETE archive, which is org-scoped and ungated — a bulk wrapper
// must not be more OR less privileged than the action it wraps
// (the sprint-20 parity discipline: bulk-review is gated because
// single review is gated; bulk-archive is ungated because single
// archive is ungated). Per-row audit events + webhook dispatch
// happen inside archiveImportRequest via events.record — the
// batch layer adds no bypass.
async function handleBulkArchive(req, res, ctx) {
  const body = (req.body && typeof req.body === 'object') ? req.body : {};
  const result = await importRequests.bulkArchiveImportRequests({
    orgId: ctx.orgIdNumeric,
    externalIds: Array.isArray(body.externalIds) ? body.externalIds : [],
    actorEmailHash: ctx.emailHash,
  });
...
module.exports = async function (req, res) {
  ...
  try {
    ...
    // Sprint 74 — /api/imports/bulk-archive. Same reserved-keyword
    // pattern (external IDs are 'ir_<16hex>' and never collide with
    // the literal "bulk-archive").
    if (externalId === 'bulk-archive' && !action) {
      if (req.method !== 'POST') return jsonResponse(res, 405, { error: 'bulk-archive requires POST' });
      return handleBulkArchive(req, res, ctx);
    }
    // Sprint 76 — /api/imports/bulk-restore. Archive's mirror.
    if (externalId === 'bulk-restore' && !action) {
      if (req.method !== 'POST') return jsonResponse(res, 405, { error: 'bulk-restore requires POST' });
      return handleBulkRestore(req, res, ctx);
    }
    ...
    // Sprint 76 — archive's mirror. POST (not DELETE) because it
    // brings the row BACK; the DELETE verb stays archive-only.
    if (action === 'restore') {
      if (req.method !== 'POST') return jsonResponse(res, 405, { error: 'restore requires POST' });
      return handleRestore(req, res, ctx, externalId);
    }
    // Sprint 81 — actual landed outcome.
    if (action === 'actual') {
      if (req.method !== 'POST') return jsonResponse(res, 405, { error: 'actual requires POST' });
      return handleReportActual(req, res, ctx, externalId);
    }
    if (action === 'history') {
