...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    const meta = await sanctions.getListMeta();
    return res.status(200).json({
      ok: true,
      authoritative: meta.authoritative,
      list: meta,
      advisory: sanctions.advisoryFor(meta.authoritative),
    });
  }
  ...
  // Screen against the real consolidated list when loaded, else the sample.
  const list = await sanctions.getActiveList();
  const result = sanctions.screen({ name, threshold, list });
