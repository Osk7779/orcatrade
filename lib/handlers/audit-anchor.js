...
// Resolve the sub-action from req.query.path (set by api/[...path].js
// from `/api/audit-anchor/<sub>`). Bare `/api/audit-anchor` → null.
function subAction(req) {
  const p = req.query && req.query.path;
  if (Array.isArray(p) && p.length > 1) return String(p[1]);
  if (typeof p === 'string') {
...
module.exports = async function handler(req, res) {
  ...
  // ── Sub-action: /api/audit-anchor/history (apex III2 follow-up) ──
  // Rolling snapshot history; populated nightly by the cron job
  // (`.github/workflows/cron.yml`) and rendered by /trust/anchors/.
  // Hard-capped at MAX_SNAPSHOTS (~3 months of daily snapshots).
  if (subAction(req) === 'history') {
    const history = require('../audit-anchor-history');
    const rawLimit = req.query && req.query.limit;
    const limit = rawLimit ? Number(rawLimit) : undefined;
    let snapshots = [];
    try { snapshots = await history.listAnchorSnapshots({ limit }); }
    catch (err) {
      log.warn('audit-anchor history read failed', { err: err && err.message });
    }
    return json(res, 200, {
      ok: true,
  ...
  return json(res, 200, {
    ...
    verification: {
      ...
      howToVerify: [
        ...
        'For a public rolling history of past anchors, GET /api/audit-anchor/history (no auth, capped at ~3 months of daily snapshots).',
      ],
