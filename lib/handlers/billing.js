...
const DUNNING_COUNTER_TTL_DAYS = 30;
const DUNNING_PREFIX = 'stripe:dunning:';

const eventKey = id => `stripe:event:${id}`;
const customerKey = email => `stripe:customer:${String(email).toLowerCase().trim()}`;
// Sprint BG-3.3 phase 2 — reverse map written at checkout.session.completed
// time, read at customer.subscription.deleted time. Lets us downgrade the
// right ORG at period-end cancellation without re-fetching the Stripe
// subscription. Also keyed for the user's email→customer reverse so we
// can find the email on cancellation when no metadata is present.
const orgByCustomerKey = customerId => `stripe:org-by-customer:${customerId}`;
const emailByCustomerKey = customerId => `stripe:email-by-customer:${customerId}`;
...
// RBAC (apex III1): subscription changes are org-level money. If the user
// belongs to an org, they must hold BILLING_MANAGE (owner / admin / finance);
// a viewer or analyst is blocked. Users with no org are billing for themselves
// and are always permitted. Returns true to proceed, false after sending 403.
async function ensureBillingPermitted(req, res, user) {
  try {
    const orgs = require('../orgs');
    const rbac = require('../rbac');
    const userOrgs = await orgs.listOrgsForEmail(user.email);
    if (Array.isArray(userOrgs) && userOrgs.length > 0) {
      const role = await orgs.getMemberRole(userOrgs[0].id, user.email);
      if (!rbac.can(role, rbac.PERMISSIONS.BILLING_MANAGE)) {
        jsonResponse(res, 403, {
          error: 'You do not have permission to manage billing for your organisation',
          requiredPermission: rbac.PERMISSIONS.BILLING_MANAGE,
        });
        return false;
      }
    }
  } catch (_) { /* org lookup failure must not block personal billing */ }
  return true;
}
...
async function handleCheckout(req, res, user) {
  if (!(await ensureBillingPermitted(req, res, user))) return;
  ...
  // Sprint BG-3.3 phase 2 — look up the subscriber's primary org so
  // the Stripe metadata can carry it through to the webhook. Primary
  // org = first in listOrgsForEmail (oldest membership). When the
  // user has no org, we leave orgId null and the webhook falls back
  // to writing per-email tier (legacy phase-0/1 behaviour). Lazy
  // require to avoid pulling lib/orgs at module-init time.
  ...
  } catch (_) { /* org lookup failure → fall back to per-email writes */ }
...
async function handleMe(req, res, user) {
  ...
  const customerId = await kv.get(customerKey(user.email));
  // Sprint BG-3.4 — surface tier provenance to the UI so a user on a
  // Team-plan org sees "via your organisation: Acme Co" rather than
  // wondering why their tier is different from what they bought on
  // Stripe. origin ∈ {'org', 'email', 'default'} comes from BG-3.3
  // phase 1's resolveTier; orgName is looked up lazily only when the
  // override actually applies.
  let orgName = null;
  if (resolved.origin === 'org' && resolved.orgId) {
    try {
      const orgs = require('../orgs');
      const org = await orgs.getOrg(resolved.orgId);
      orgName = (org && org.name) || null;
    } catch (_) { /* fall through with null orgName */ }
  }
...
function inferTierFromEvent(event) {
  ...
  // Sprint BG-3.3 phase 2 — also surface org_id (when the subscriber
  // belongs to an org at checkout time). Stripe persists subscription
  // metadata across the subscription lifecycle, so we can still
  // identify the org at period-end cancellation time even if the
  // user's memberships have changed in between.
  const obj = event && event.data && event.data.object;
  if (!obj) return { tierId: null, billingCycle: null, orgId: null };
  ...
  return { tierId, billingCycle, orgId };
}
...
async function processWebhookEvent(event) {
  ...
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      const { tierId, billingCycle, orgId } = inferTierFromEvent(event);
      if (!email || !tierId) return { handled: false, reason: 'missing-email-or-tier' };
      if (customerId) {
        // Forward map: email → customer (for portal sessions).
        await kv.set(customerKey(email), customerId, { ttlSeconds: CUSTOMER_MAP_TTL_DAYS * 24 * 60 * 60 });
        // Reverse map: customer → email (for cancellation lookup; phase 0 already wrote this elsewhere, here we make it canonical).
        await kv.set(emailByCustomerKey(customerId), email, { ttlSeconds: CUSTOMER_MAP_TTL_DAYS * 24 * 60 * 60 });
      }

      // Sprint BG-3.3 phase 2 — when the subscriber belongs to an org,
      // write the tier at the ORG level so every member of that org
      // sees the upgrade. When no orgId is present (subscriber has no
      // org), fall back to per-email writes — the legacy phase-0/1
      // path. resolveTier reads org first, falls back to email, so
      // existing per-email subscribers keep their tier.
      if (orgId) {
        await userTier.setOrgTier(orgId, {
          tierId,
        ...
        if (customerId) {
          // Reverse map for the cancellation handler — we need to know
          // which org to downgrade at period-end without re-reading
          // the subscription's metadata.
          await kv.set(orgByCustomerKey(customerId), orgId, { ttlSeconds: CUSTOMER_MAP_TTL_DAYS * 24 * 60 * 60 });
        }
        try { await events.record('org_tier_assigned', { orgId, tierId, source: 'stripe' }); } catch (_e) {}
      } else {
      ...
      return { handled: true, action: 'tier-set', email, tierId, orgId: orgId || null };
    }
    ...
    case 'customer.subscription.deleted': {
      // Subscription cancelled at end-of-period. Stripe fires this AT
      // period end, not at the moment the user clicks cancel — so by
      // the time we get here the customer has used up the period they
      // paid for. Safe to drop them to free.
      const customerId = obj.customer || null;
      if (!customerId) return { handled: false, reason: 'missing-customer' };

      // Sprint BG-3.3 phase 2 — prefer the subscription's own metadata
      // (persisted by Stripe across the subscription lifecycle), fall
      // back to the reverse-lookup KV map we wrote at checkout time.
      // The metadata path is bulletproof against KV evictions; the
      // KV path is a backstop for events fired before phase 2 shipped.
      const { orgId: metaOrgId } = inferTierFromEvent(event);
      const orgId = metaOrgId || (await kv.get(orgByCustomerKey(customerId)));

      if (orgId) {
        // Org-level subscription: downgrade the ORG, not the individual.
        // setOrgTier with tierId:'free' (rather than clearOrgTier) keeps
        // an audit trail: the org WAS on a paid tier, now it's not, and
        // we know exactly when.
        await userTier.setOrgTier(orgId, {
          tierId: 'free',
          billingCycle: null,
          since: new Date().toISOString(),
          source: 'stripe-cancelled',
        });
        try { await events.record('org_tier_assigned', { orgId, tierId: 'free', source: 'stripe-cancelled' }); } catch (_e) {}
        return { handled: true, action: 'downgrade-org-to-free', orgId };
      }

      // Per-email path (subscribers without an org at checkout time).
      const email = await kv.get(emailByCustomerKey(customerId));
    ...
    case 'invoice.payment_failed': {
      // Dunning: track attempt count per subscription, send a different
      // email at each escalation level. Stripe Smart Retries handles
      // the actual re-charge; we just narrate.
      const subscriptionId = obj.subscription || (obj.lines && obj.lines.data && obj.lines.data[0] && obj.lines.data[0].subscription) || null;
      const customerId = obj.customer || null;
      const customerEmail = (obj.customer_email || '').toLowerCase().trim() ||
        (customerId ? await kv.get(`stripe:email-by-customer:${customerId}`) : null);
      if (!customerEmail) return { handled: false, reason: 'no-email-for-customer' };

      const dunningKey = DUNNING_PREFIX + (subscriptionId || customerId || customerEmail);
      const attempt = await kv.incr(dunningKey, { ttlSeconds: DUNNING_COUNTER_TTL_DAYS * 24 * 60 * 60 });
      try { await sendDunningEmail({ email: customerEmail, attempt, invoice: obj }); } catch (_e) {}
      return { handled: true, action: 'dunning-sent', email: customerEmail, attempt };
    }
    case 'invoice.payment_succeeded': {
      // Reset the dunning counter — the card came good.
      const subscriptionId = obj.subscription || null;
      const customerId = obj.customer || null;
      const customerEmail = (obj.customer_email || '').toLowerCase().trim() ||
        (customerId ? await kv.get(`stripe:email-by-customer:${customerId}`) : null);
      const dunningKey = DUNNING_PREFIX + (subscriptionId || customerId || customerEmail || '');
      if (dunningKey !== DUNNING_PREFIX) {
        await kv.del(dunningKey);
      }
      return { handled: true, action: 'dunning-reset' };
    }
    default:
...
// Dunning email content varies by attempt count. Soft-fails on Resend
// misconfiguration so a missing key never breaks the webhook ack.
async function sendDunningEmail({ email, attempt, invoice }) {
  if (!emailModule.isConfigured()) return { ok: false, reason: 'email not configured' };
  const portalUrl = `${SITE_ORIGIN}/account/billing/`;
  const amountDue = invoice && Number.isFinite(Number(invoice.amount_due))
    ? '€' + (Number(invoice.amount_due) / 100).toLocaleString('en-IE', { minimumFractionDigits: 2 })
    : null;

  // Stripe Smart Retries usually attempts 3 times over ~7 days before
  // marking the subscription unpaid. Our three messages map to those.
  const messages = {
    1: {
      subject: 'Payment didn\'t go through — please update your card',
      body: [
        `Your latest OrcaTrade payment didn't go through${amountDue ? ` (${amountDue} due)` : ''}.`,
        ``,
        `Most commonly this is an expired card, a recent address change, or your`,
        `bank blocking the charge as out-of-pattern. We'll automatically try again`,
        `in 1–3 days.`,
        ``,
        `If you'd like to update your card now (recommended), open the billing portal:`,
        portalUrl,
        ``,
        `Your access to the platform is unaffected during this retry window.`,
        ``,
        `— OrcaTrade`,
      ].join('\n'),
    },
    2: {
      subject: 'Second payment attempt failed — action needed soon',
      body: [
        `Stripe just tried your card again and it failed${amountDue ? ` (${amountDue} due)` : ''}.`,
        ``,
        `If we can't take payment by the third attempt, your subscription will`,
        `automatically pause and your tier will revert to Free.`,
        ``,
        `Update your card here:`,
        portalUrl,
        ``,
        `Reply to this email if you need help — you'll reach the founders directly.`,
        ``,
        `— OrcaTrade`,
      ].join('\n'),
    },
    3: {
      subject: 'Final retry failed — your subscription will pause shortly',
      body: [
        `The third payment attempt has failed${amountDue ? ` (${amountDue} due)` : ''}.`,
        ``,
        `Stripe will mark your subscription unpaid and your access will revert to`,
        `the Free tier within 24 hours. Your saved plans, history, and account`,
        `stay intact — you can resubscribe any time.`,
        ``,
        `Last chance to keep your current tier — update your card now:`,
        portalUrl,
        ``,
        `Reply to this email if there's anything we can do.`,
        ``,
        `— OrcaTrade`,
      ].join('\n'),
    },
  ...
  // Anything ≥4 collapses to the final-attempt copy.
  const msg = messages[attempt] || messages[3];
  return emailModule.send({ to: email, subject: msg.subject, text: msg.body });
}
...
module.exports.sendDunningEmail = sendDunningEmail;
module.exports.DUNNING_PREFIX = DUNNING_PREFIX;
