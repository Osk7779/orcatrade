...
// Sprint 7 — RBAC gate set for the Review queue surface. Mirrors
// OPS_REVIEW_ROLES on lib/handlers/imports.js; drift-guarded by
// test/account-role.test.js below + the sprint 6 RBAC test.
...
module.exports = async (req, res) => {
  ...
  if (action === 'role' && req.method === 'GET') {
    return handleRole(req, res, user);
  }

  return jsonResponse(res, 404, {
    error: `Unknown /api/account/${action}. Available: GET /api/account/export, POST /api/account/delete, GET /api/account/activity, GET/POST /api/account/preferences, GET /api/account/onboarding, GET /api/account/calendar, GET/POST /api/account/alerts, GET /api/account/calibration, GET /api/account/role.`,
  });
};

// ── /api/account/role (sprint 7) ──────────────────────────────────────
//
// Returns the signed-in user's canonical role within their primary
// org plus an `isOpsRole` boolean for client-side gating (Sidebar
// hides the Review queue link when false).
//
// Shape:
//   { ok: true, orgId: string | null, orgName: string | null,
//     role: string | null, isOpsRole: boolean }
//
// A user signed in but not yet a member of any org → role: null,
// isOpsRole: false. The Sidebar treats that as "non-ops" and hides
// the queue link, which is the safe default.
async function handleRole(req, res, user) {
  // Sprint 61 — also surface the user's emailHash so the UI can
  // do owner-check rendering (e.g. sprint-55 internal note
  // edit/delete buttons render only when the note's byEmailHash
  // matches the caller). Hashed form mirrors the server-side
  // identity discipline; raw email NEVER hits the client.
  const userHash = emailHash(user.email);
  try {
    const userOrgs = await orgs.listOrgsForEmail(user.email);
    if (!Array.isArray(userOrgs) || userOrgs.length === 0) {
      return jsonResponse(res, 200, {
        ok: true, orgId: null, orgName: null, role: null, isOpsRole: false, emailHash: userHash,
      });
    }
    const primaryOrg = userOrgs[0];
    const rawRole = await orgs.getMemberRole(primaryOrg.id, user.email);
    const role = rbac.canonicalRole(rawRole || '') || null;
    const isOpsRole = !!role && OPS_REVIEW_ROLES.has(String(role));
    return jsonResponse(res, 200, {
      ok: true,
      orgId: primaryOrg.id,
    ...
    log.warn('account/role lookup failed', {
      err: err instanceof Error ? err.message : String(err),
    });
    // Safe-default DENY: surface the role as null so the sidebar
    // hides the queue link. The user is still signed in (we got past
    // ensureSignedIn earlier) so a 200 with null role is the right
    // response — better than 500-ing and breaking the sidebar render.
    return jsonResponse(res, 200, {
      ok: true, orgId: null, orgName: null, role: null, isOpsRole: false, emailHash: userHash,
    });
  }
...
module.exports.handleRole = handleRole;
