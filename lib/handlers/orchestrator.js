...
// Sprint delegation-v1 (Pillar I6) — orchestrator-native delegation tools. The
// orchestrator decomposes a multi-domain request into specialist sub-tasks and
// merges their results deterministically (lib/intelligence/delegation.js).
const delegation = require('../intelligence/delegation');
const DELEGATION_TOOLS = [
  {
    name: 'planDelegation',
    description: "Decompose a multi-domain import request into an ordered set of specialist sub-tasks (sourcing → compliance → logistics → finance) and decide which specialist toolset owns each. Call this FIRST on a broad question that spans several domains, then work the steps in the returned order, calling each specialist's tools. Returns { multiDomain, order, steps:[{specialist,focus}] }.",
    input_schema: { type: 'object', properties: { task: { type: 'string', description: "The user's full request." } }, required: ['task'] },
  },
  {
    name: 'mergeSpecialistFindings',
    description: "Merge the results you gathered from each specialist into one ordered, deduplicated answer — combines citations, collects tool-cited numbers, and propagates any human-review escalation. Call this LAST, after working the delegation steps. Input: { findings: [{ specialist, summary, citations?, numbers?, escalate? }] }.",
    input_schema: { type: 'object', properties: { findings: { type: 'array', items: { type: 'object' } } }, required: ['findings'] },
  },
];
const delegationImpls = {
  planDelegation: ({ task }) => delegation.planDelegation(task),
  mergeSpecialistFindings: ({ findings }) => delegation.mergeSpecialistFindings(findings),
};

const TOOLS = mergeTools(compliance.TOOLS, logistics.TOOLS, sourcing.TOOLS, finance.TOOLS, DELEGATION_TOOLS);
const toolImpls = mergeImpls(compliance.toolImpls, logistics.toolImpls, sourcing.toolImpls, finance.toolImpls, delegationImpls);
...
module.exports = async (req, res) => {
  ...
  // Defensive floor that fires alongside agentQueriesPerMonth — a
  // pathological prompt (very long context on Opus 4.7) can blow
  // past the per-query budget the quota assumes. Reads the
  // cumulative cents-spent for the current month; rejects with 429
  // when the tier cap is hit. Fail-OPEN on missing identity or KV
  // outage (see lib/gating.checkAgentSpend).
  const spendGate = await gating.checkAgentSpend(req);
  if (!spendGate.allowed) return gating.gate(res, spendGate);
      ...
      const response = await withCostTelemetry(
        {
          ...
          // Apex P1.7 — surface the requesting user's email so
          // cost-telemetry can record the cents-spent into the
          // per-tenant ledger that gating.checkAgentSpend reads
          // pre-flight. Anonymous identity (auth.getCurrentUser
          // returns null) → spend isn't ledger'd, gate fails OPEN
          // anyway, IP rate-limit is the anonymous-traffic guard.
          email: (auth.getCurrentUser(req) || {}).email || null,
        },
        () => callAnthropic({
...
function classifyTool(name) {
  const compliance = new Set([
    ...
    'getComplianceCalendar', 'screenCounterparty',
  ]);
  ...
  if (name === 'planDelegation' || name === 'mergeSpecialistFindings') return 'orchestration';
