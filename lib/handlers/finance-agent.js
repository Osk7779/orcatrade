...
const toolImpls = {
  ...
  async lookupHsCode({ productDescription, originCountry, intendedUse }) {
    // P0.11: real calculator-grounded lookup (lib/intelligence/hs-code-lookup.js)
    // replaces the prior fake-confidence placeholder. ADR 0016.
    const hsCodeLookup = require('../intelligence/hs-code-lookup');
    return hsCodeLookup.lookupHsCode({ productDescription, originCountry, intendedUse });
  },
  ...
  async requestHumanReview({ reason, severity, handoffTo, context }) {
    // P0.10: real queue (lib/human-review.js) replaces the prior fake
    // ticketId. handoffTo is preserved in context so a future Phase 1
    // routing layer can dispatch tickets accordingly.
    const humanReview = require('../human-review');
    const ticket = await humanReview.appendTicket({
      agent: 'finance',
      reason,
      severity,
      context: { ...(context || {}), handoffTo: handoffTo || 'human_ops' },
    });
    return {
      ticketId: ticket.id,
      ...
      message: `Routed to ${handoffTo || 'human ops'}. Ticket ${ticket.id}. Severity: ${ticket.severity}. Status: ${ticket.status}.`,
    };
  },
};
      ...
      const response = await withCostTelemetry(
        {
          ...
          // Per-tenant attribution for cost telemetry + spend cap.
