...
async function handleTransition(req, res, ctx, externalId) {
  ...
  // Sprint 9 ch 2 — notify the customer of the source import_request
  // (if any) that their shipment has reached a new status. The helper
  // resolves the contact email from the shipment.metadata link and
  // short-circuits cleanly when there's no source request, no
  // contact, or the destination status is non-notifiable. Fire-and-
  // forget so the email path NEVER blocks an operational transition.
  if (!result.unchanged) {
    const importsEmails = require('../imports-emails');
    importsEmails.sendShipmentStatusUpdateEmail({
      shipment: result.shipment,
      toStatus: String(toStatus),
    }).catch((err) => {
      log.warn('sendShipmentStatusUpdateEmail threw', {
        externalId,
        toStatus: String(toStatus),
