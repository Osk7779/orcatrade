...
const { getUpcomingObligations } = require('../intelligence/compliance-calendar');
...
const toolImpls = {
  getComplianceCalendar({ productCategory, productDescription, originCountry, hsCode, importerEntity, globalTurnoverEur, horizonDays, asOf }) {
    ...
    const regimes = [];
    if (cbam.applies) regimes.push('cbam');
    if (eudr.applies) regimes.push('eudr');
    ...
    const obligations = getUpcomingObligations({ regimes, isSME, horizonDays: horizonDays || undefined, asOf });
    return {
      regimesInScope: regimes,
      isSME,
