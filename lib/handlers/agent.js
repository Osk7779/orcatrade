...
const toolImpls = {
  ...
  async lookupHsCode({ productDescription, originCountry, intendedUse }) {
    ...
    // replaces the prior fake-confidence placeholder. Deterministic
    // suggestion from the curated HS6 map; optional live MFN-rate
    // enrichment when origin is provided. ADR 0016.
      ...
      const response = await withCostTelemetry(
        {
          ...
          // Anonymous (signed-out) calls bucket under '(anonymous)' on
          // the dashboard and against the free-tier cap.
