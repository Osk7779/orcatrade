...
// Audit-anchor-history snapshot job (apex III2 follow-on to the
// rolling-history surface shipped in the parent PR). Captures the
// live chain head once a day so the public /api/audit-anchor/history
// endpoint reflects a real time-series instead of an empty array.
// 1-hour dedupe window in recordAnchorSnapshot protects against
// missed-then-double-fired runs. KV outage is logged + non-throwing
// per the history module's contract.
async function runAuditAnchorSnapshot() {
  return require('../audit-anchor-history').recordAnchorSnapshot();
}

// Sprint 19 — quote expiry sweep. Transitions every quoted request
// whose quote_expires_at is in the past to 'expired'. Posts a system
// message on the thread so the customer sees the reason. Audit-logged
// + ORG_ACTIVITY_TYPES (the activity feed picks it up). Idempotent —
// re-runs are no-ops because the WHERE clause requires status='quoted'.
async function runImportRequestQuoteExpiry({ limit = 500 } = {}) {
  const importRequests = require('../db/import-requests');
  const result = await importRequests.expireStaleQuotes({ limit });
  if (!result.ok) {
    return { ok: false, error: (result.errors && result.errors[0]) || 'expireStaleQuotes failed' };
  }
  return {
    ok: true,
...
// Sprint 26 — weekly Ops Insights digest. Per-org fan-out: enumerate
// every org via KV, resolve its numeric Postgres org_id, compute the
// 7-day insights (same aggregateOpsInsights as the live page), and
// send the digest email to ops admins (per-recipient pref-gated via
// importInsightsDigestEmails). One org's failure does NOT block the
// others — we accumulate per-org results in errors[] / sentByOrg.
async function runImportRequestInsightsDigest({ windowDays = 7 } = {}) {
  const orgs = require('../orgs');
  const importRequests = require('../db/import-requests');
  const importsEmails = require('../imports-emails');
  const dbClient = require('../db/client');

  const allOrgs = await orgs.listAllOrgs({ limit: 10000 });
  if (!Array.isArray(allOrgs) || allOrgs.length === 0) {
    return { ok: true, orgsConsidered: 0, sentByOrg: 0, errors: [] };
  }

  /** @type {Array<{ orgId: string, error: string }>} */
  const errors = [];
  let sentByOrg = 0;
  let mutedByOrg = 0;
  ...
  for (const org of allOrgs) {
    try {
      // Resolve numeric Postgres org id; KV stores the external string
      // id ('org_…') but the data layer keys on the bigint.
      let orgIdNumeric = null;
      if (typeof org.dbId === 'number') {
        orgIdNumeric = org.dbId;
      } else if (dbClient.isConfigured()) {
        const row = await dbClient.queryOne(
          `SELECT id FROM organisations WHERE external_id = $1`,
          [org.id],
        );
        orgIdNumeric = row ? Number(row.id) : null;
      }
      if (!Number.isFinite(orgIdNumeric)) {
        errors.push({ orgId: String(org.id || '?'), error: 'no-numeric-org-id' });
        continue;
      }
      // Sprint 94 — the digest now renders knob-dependent numbers
      // (SLA attainment vs negotiated targets, the at-risk snapshot,
      // effective cohort thresholds), so the runner threads the
      // FULL org config exactly like handleInsights — the digest is
      // the whole-cockpit summary and must never disagree with it
      // (the sprint-88 lesson, applied before the bug this time).
      const operatorConfig = require('../operator-config');
      const orgConfig = await operatorConfig.getOperatorConfig(orgIdNumeric);
      const agg = await importRequests.aggregateOpsInsights({
        orgId: orgIdNumeric,
        windowDays,
        stallThresholdDays: orgConfig.stallThresholdDays,
        declineSpikeRateMultiplier: orgConfig.declineSpikeRateMultiplier,
        supplierConcentrationThreshold: orgConfig.supplierConcentrationThreshold,
        ratingTrendDropThreshold: orgConfig.ratingTrendDropThreshold,
        quoteFollowUpThresholdDays: orgConfig.quoteFollowUpThresholdDays,
        slaQuoteTurnaroundTargetHours: orgConfig.slaQuoteTurnaroundTargetHours,
        slaFirstResponseTargetHours: orgConfig.slaFirstResponseTargetHours,
      });
      if (!agg.ok) {
        errors.push({ orgId: String(org.id), error: (agg.errors && agg.errors[0]) || 'aggregate failed' });
        continue;
      }
      // Skip orgs with literally zero activity — no signal to send.
      if (Number(agg.insights.totalInWindow || 0) === 0) {
        continue;
      }
      const sendResult = await importsEmails.sendOpsInsightsDigest({
        orgIdNumeric,
      ...
      if (sendResult.ok) {
        sentByOrg += 1;
      } else if (sendResult.reason === 'all-muted') {
        mutedByOrg += 1;
      } else if (sendResult.reason !== 'no-inbox') {
        errors.push({ orgId: String(org.id), error: sendResult.reason || 'send failed' });
      }
    } catch (err) {
      errors.push({
        orgId: String(org.id || '?'),
...
// Sprint 39 — daily stalled-queue alert. Per-org fan-out, mirrors the
// sprint-26 weekly digest structure. Stalls (status='awaiting_review'
// with no activity > 7 days, sprint 38) deserve daily attention, not
// weekly; cron runs UK morning time so ops can act before customers
// escalate. One org's failure does NOT block the others.
//
// Discipline: reuses aggregateOpsInsights for cohort #6 — no separate
// SQL path, the same numbers the live cockpit shows. The send helper
// short-circuits on count===0 so healthy orgs send no email; the
// pref gate (importStalledQueueAlertEmails) handles per-recipient
// muting.
async function runImportRequestStalledQueueAlert() {
  const orgs = require('../orgs');
  const importRequests = require('../db/import-requests');
  const importsEmails = require('../imports-emails');
  const dbClient = require('../db/client');
  // Sprint 42 — per-org config (v1: stallThresholdDays). Loaded
  // per-org inside the loop so a slow KV doesn't blow up the
  // whole fan-out.
  const operatorConfig = require('../operator-config');
  ...
  let mutedByOrg = 0;
  let healthyByOrg = 0;
  // Sprint 75 — orgs skipped because their cadence is 'weekly'.
  let weeklyOnlyByOrg = 0;
  ...
  for (const org of allOrgs) {
    try {
      ...
      // Sprint 75 — org-level cadence gate. 'weekly' orgs opted out
      // of daily proactive alert emails; their signals stay on the
      // live cockpit + the weekly digests. Runs BEFORE the
      // aggregation so a silenced org costs one KV read, not a SQL
      // fan-out. Fail-open inside getAlertCadence: a KV blip
      // resolves to 'daily' (an unwanted email beats a missed one).
      const alertCadence = await operatorConfig.getAlertCadence(orgIdNumeric);
      if (alertCadence === 'weekly') {
        weeklyOnlyByOrg += 1;
        continue;
      }
      // Reuses the live aggregateOpsInsights (cohort #6 lives at
      // insights.stalledQueue). windowDays is irrelevant to the stall
      // cohort (it's window-agnostic by design) but the aggregation
      // expects a value — pass 7 to mirror the alert cadence.
      // Sprint 42 — load per-org config so the stall threshold the
      // SQL uses matches what the customer set; falls back silently
      // to the platform default if config is absent (defaults
      // merged inside operator-config.getOperatorConfig).
      const orgConfig = await operatorConfig.getOperatorConfig(orgIdNumeric);
      const agg = await importRequests.aggregateOpsInsights({
        orgId: orgIdNumeric,
        windowDays: 7,
      ...
      const stalledQueue = agg.insights && agg.insights.stalledQueue;
      if (!stalledQueue || stalledQueue.count === 0) {
        healthyByOrg += 1;
        continue;
      }
      const sendResult = await importsEmails.sendStalledQueueAlert({
        orgIdNumeric,
        orgName: typeof org.name === 'string' ? org.name : undefined,
        stalledQueue,
      });
      if (sendResult.ok) {
        ...
        mutedByOrg += 1;
      } else if (sendResult.reason !== 'no-inbox' && sendResult.reason !== 'no-stalls') {
        errors.push({ orgId: String(org.id), error: sendResult.reason || 'send failed' });
      }
    } catch (err) {
      errors.push({
...
// Sprint 70 — daily aging-quotes alert. Per-org fan-out, mirrors
// runImportRequestStalledQueueAlert. The cohort (sprint-69 #11)
// names specific quotes waiting on customer decision past 5 days.
// Complementary to sprint 54's weekly quote-acceptance RATE alert
// — that surfaces the population-level trend, this surfaces the
// specific quotes to nudge.
async function runImportRequestQuoteFollowUpAlert() {
  const orgs = require('../orgs');
  const importRequests = require('../db/import-requests');
  const importsEmails = require('../imports-emails');
  const dbClient = require('../db/client');
  // Sprint 71 — per-org threshold override. Same load-per-org
  // pattern as sprint 42/43/60/64 alerts.
  const operatorConfig = require('../operator-config');
  ...
  for (const org of allOrgs) {
    try {
      ...
      // Reuses the live aggregateOpsInsights (cohort #11 lives at
      // insights.quoteFollowUp). windowDays is irrelevant to the
      // aging cohort (window-agnostic like sprint 38) but the
      // aggregation expects a value — pass 7 to mirror the cadence.
      // Sprint 71 — load per-org threshold so the SQL matches what
      // the customer set; defaults merge inside getOperatorConfig.
      const orgConfig = await operatorConfig.getOperatorConfig(orgIdNumeric);
      const agg = await importRequests.aggregateOpsInsights({
        orgId: orgIdNumeric,
        windowDays: 7,
      ...
      const quoteFollowUp = agg.insights && agg.insights.quoteFollowUp;
      if (!quoteFollowUp || quoteFollowUp.count === 0) {
        healthyByOrg += 1;
        continue;
      }
      const sendResult = await importsEmails.sendQuoteFollowUpAlert({
        orgIdNumeric,
        orgName: typeof org.name === 'string' ? org.name : undefined,
        quoteFollowUp,
      });
      if (sendResult.ok) {
        ...
        mutedByOrg += 1;
      } else if (sendResult.reason !== 'no-inbox' && sendResult.reason !== 'no-aging') {
        errors.push({ orgId: String(org.id), error: sendResult.reason || 'send failed' });
      }
    } catch (err) {
      errors.push({
...
// Sprint 41 — daily decline-spike alert. Per-org fan-out, mirrors
// runImportRequestStalledQueueAlert. Spikes (sprint-40 cohort #7)
// deserve daily attention, not weekly, when supplier-compliance or
// regulatory-shift issues catch a trend early enough to act on. One
// org's failure does NOT block the others.
//
// Discipline: reuses aggregateOpsInsights for cohort #7 — no
// separate SQL path, the same numbers the live cockpit shows. The
// send helper short-circuits on spikes.length===0 so healthy orgs
// send no email; the pref gate (importDeclineSpikeAlertEmails)
// handles per-recipient muting.
async function runImportRequestDeclineSpikeAlert() {
  const orgs = require('../orgs');
  const importRequests = require('../db/import-requests');
  const importsEmails = require('../imports-emails');
  const dbClient = require('../db/client');
  // Sprint 43 — per-org config (declineSpikeRateMultiplier). Same
  // per-org load-inside-loop pattern as the sprint-42 stall alert.
  const operatorConfig = require('../operator-config');
  ...
  for (const org of allOrgs) {
    try {
      ...
      // Reuses the live aggregateOpsInsights (cohort #7 lives at
      // insights.declineSpike). windowDays is irrelevant to the
      // spike cohort (it's window-agnostic, using the sprint-40
      // constants directly) but the aggregation expects a value —
      // 7 matches the alert cadence.
      // Sprint 43 — load per-org config so the spike-classifier
      // multiplier matches what the customer set; falls back
      // silently to the platform default if config is absent.
      const orgConfig = await operatorConfig.getOperatorConfig(orgIdNumeric);
      const agg = await importRequests.aggregateOpsInsights({
        orgId: orgIdNumeric,
        windowDays: 7,
      ...
      const declineSpike = agg.insights && agg.insights.declineSpike;
      if (!declineSpike || !Array.isArray(declineSpike.spikes) || declineSpike.spikes.length === 0) {
        healthyByOrg += 1;
        continue;
      }
      const sendResult = await importsEmails.sendDeclineSpikeAlert({
        orgIdNumeric,
        orgName: typeof org.name === 'string' ? org.name : undefined,
        declineSpike,
      });
      if (sendResult.ok) {
        ...
        mutedByOrg += 1;
      } else if (sendResult.reason !== 'no-inbox' && sendResult.reason !== 'no-spikes') {
        errors.push({ orgId: String(org.id), error: sendResult.reason || 'send failed' });
      }
    } catch (err) {
      errors.push({
...
// Sprint 54 — weekly quote-acceptance rate alert. Per-org
// fan-out. Reuses aggregateOpsInsights (cohort #8 lives at
// insights.quoteAcceptance — single source of truth, alert agrees
// with the live cockpit reading the same data). Healthy orgs
// (!isDegraded) skip silently. One org's failure does NOT block
// the others.
async function runImportRequestQuoteAcceptanceAlert() {
  const orgs = require('../orgs');
  const importRequests = require('../db/import-requests');
  const importsEmails = require('../imports-emails');
  const dbClient = require('../db/client');
  ...
  for (const org of allOrgs) {
    try {
      ...
      // windowDays is irrelevant to the quote-acceptance cohort
      // (it uses the sprint-53 30d/60d constants directly); 7 is
      ...
      const quoteAcceptance = agg.insights && agg.insights.quoteAcceptance;
      if (!quoteAcceptance || !quoteAcceptance.isDegraded) {
        healthyByOrg += 1;
        continue;
      }
      const sendResult = await importsEmails.sendQuoteAcceptanceAlert({
        orgIdNumeric,
        orgName: typeof org.name === 'string' ? org.name : undefined,
        quoteAcceptance,
      });
      if (sendResult.ok) {
        ...
        mutedByOrg += 1;
      } else if (sendResult.reason !== 'no-inbox' && sendResult.reason !== 'not-degraded') {
        errors.push({ orgId: String(org.id), error: sendResult.reason || 'send failed' });
      }
    } catch (err) {
      errors.push({
...
// Sprint 102 — hourly intake rescue sweep. Rows the client
// created but never processed (the tab died between the create
// POST and the process POST) sit in 'submitted' with their SLA
// clock running and nothing moving. The rescue runs the SAME
// orchestrator the button runs, acting AS THE REQUEST'S CREATOR
// — it completes the customer's own submit action, so the audit
// trail reads exactly as it would have had the tab survived.
// Wrong-state races (a human clicked Process between the read
// and the run) are tolerated silently — the state machine is the
// arbiter. NOT an email; cadence- and pref-independent.
async function runIntakeRescueSweep() {
  const importRequests = require('../db/import-requests');
  const importRequestOrchestrator = require('../ai/import-request-orchestrator');

  const orphans = await importRequests.listOrphanedSubmittedRequests({});
  if (orphans.length === 0) {
    return { ok: true, scanned: 0, rescued: 0, raced: 0, skippedNoActor: 0, errors: [] };
  }

  /** @type {Array<{ externalId: string, code: string }>} */
  const errors = [];
  let rescued = 0;
  let raced = 0;
  ...
  for (const orphan of orphans) {
    // Defensive: a legacy row without a creator hash can't be
    // rescued as-the-creator; leave it for a human (it still
    // surfaces on the SLA-at-risk cohort).
    if (!orphan.createdByEmailHash) {
      skippedNoActor += 1;
      continue;
    }
    try {
      const result = await importRequestOrchestrator.runOrchestrator({
        ...
        actorEmailHash: orphan.createdByEmailHash,
      });
      if (result.ok) {
        rescued += 1;
      } else if (result.code === 'wrong_state' || result.code === 'concurrent_modification') {
        // Someone (or a parallel tick) got there first — success
        // by another name.
        raced += 1;
      } else {
        // Real pipeline failures already transitioned the row to
        // 'failed' with the reason in failure_state — the customer-
        // visible recovery path takes over from there.
        errors.push({ externalId: orphan.externalId, code: result.code || 'unknown' });
      }
    } catch (err) {
      errors.push({
        externalId: orphan.externalId,
...
// Sprint 97 — hourly SLA breach sweep. Per-org fan-out. Records
// each newly-breached request (unquoted past the org's NEGOTIATED
// turnaround target) as exactly one audit-chained, webhook-
// dispatched import_request_sla_breached event. This is AUDIT
// RECORDING, not an email — the sprint-75 alertCadence does NOT
// gate it (a weekly-digests org still gets its breaches on the
// record), and no recipient prefs apply. Hourly so the recorded
// breach moment lands within an hour of the actual crossing.
async function runSlaBreachSweep() {
  ...
  const allOrgs = await orgs.listAllOrgs({ limit: 10000 });
  if (!Array.isArray(allOrgs) || allOrgs.length === 0) {
    return { ok: true, orgsConsidered: 0, recorded: 0, errors: [] };
  }
  ...
  for (const org of allOrgs) {
    try {
      ...
      const orgConfig = await operatorConfig.getOperatorConfig(orgIdNumeric);
      const result = await importRequests.sweepSlaBreachesForOrg({
        orgId: orgIdNumeric,
        targetHours: orgConfig.slaQuoteTurnaroundTargetHours,
      });
      if (!result.ok) {
        errors.push({ orgId: String(org.id), error: (result.errors && result.errors[0]) || 'sweep failed' });
        continue;
      }
      recorded += result.recorded;
      stamped += result.stamped;
      revertedCount += (result.reverted || []).length;
      // Sprint 100 — the first-response half, against knob 7. One
      // org's FR-sweep failure doesn't void its turnaround pass.
      const frResult = await importRequests.sweepFirstResponseBreachesForOrg({
        orgId: orgIdNumeric,
        targetHours: orgConfig.slaFirstResponseTargetHours,
      });
      if (!frResult.ok) {
        errors.push({ orgId: String(org.id), error: (frResult.errors && frResult.errors[0]) || 'fr sweep failed' });
        continue;
      }
      recorded += frResult.recorded;
      stamped += frResult.stamped;
      revertedCount += (frResult.reverted || []).length;
    } catch (err) {
      errors.push({
        orgId: String(org.id || '?'),
        error: err instanceof Error ? err.message : String(err),
...
// Sprint 92 — daily SLA-at-risk alert. Per-org fan-out, mirrors
// the sprint-39/70 daily alerts. Cohort #13 lives at
// insights.slaAtRisk — the breach-prevention signal, computed
// against each org's NEGOTIATED turnaround target (knob 6), so
// the runner loads org config and threads it through. Honours
// the sprint-75 alertCadence like every daily alert. Healthy
// orgs (total 0) skip silently.
async function runImportRequestSlaAtRiskAlert() {
  const orgs = require('../orgs');
  const importRequests = require('../db/import-requests');
  const importsEmails = require('../imports-emails');
  const dbClient = require('../db/client');
  ...
  for (const org of allOrgs) {
    try {
      ...
      // Sprint 75 — org-level cadence gate (daily alert family).
      const alertCadence = await operatorConfig.getAlertCadence(orgIdNumeric);
      if (alertCadence === 'weekly') {
        weeklyOnlyByOrg += 1;
        continue;
      }
      // Load org config so the risk line follows the NEGOTIATED
      // target — the whole point of cohort #13.
      const orgConfig = await operatorConfig.getOperatorConfig(orgIdNumeric);
      const agg = await importRequests.aggregateOpsInsights({
        orgId: orgIdNumeric,
        windowDays: 7,
      ...
      const slaAtRisk = agg.insights && agg.insights.slaAtRisk;
      const total = slaAtRisk
        ? (Number(slaAtRisk.atRiskCount) || 0) + (Number(slaAtRisk.breachedCount) || 0)
        : 0;
      if (total === 0) {
        healthyByOrg += 1;
        continue;
      }
      const sendResult = await importsEmails.sendSlaAtRiskAlert({
        orgIdNumeric,
        orgName: typeof org.name === 'string' ? org.name : undefined,
        slaAtRisk,
      });
      if (sendResult.ok) {
        ...
        mutedByOrg += 1;
      } else if (sendResult.reason !== 'no-inbox' && sendResult.reason !== 'no-at-risk') {
        errors.push({ orgId: String(org.id), error: sendResult.reason || 'send failed' });
      }
    } catch (err) {
      errors.push({
...
// Sprint 78 — weekly expired-quotes alert. Per-org fan-out.
// Reuses aggregateOpsInsights (cohort #12 lives at
// insights.expiredQuotes — single source of truth, alert agrees
// with the live cockpit reading the same data). Healthy orgs
// (count===0) skip silently. One org's failure does NOT block
// the others. Deliberately NOT gated by the sprint-75
// alertCadence — that gate covers the three DAILY alerts only;
// weekly is precisely what cadence-weekly orgs asked for.
async function runImportRequestExpiredQuotesAlert() {
  const orgs = require('../orgs');
  const importRequests = require('../db/import-requests');
  const importsEmails = require('../imports-emails');
  const dbClient = require('../db/client');
  ...
  for (const org of allOrgs) {
    try {
      ...
      // windowDays is irrelevant to the expired-quotes cohort (it
      // binds the fixed EXPIRED_QUOTES_WINDOW_DAYS constant); 7 is
      // arbitrary but harmless.
      const agg = await importRequests.aggregateOpsInsights({
        orgId: orgIdNumeric,
        windowDays: 7,
      ...
      const expiredQuotes = agg.insights && agg.insights.expiredQuotes;
      if (!expiredQuotes || expiredQuotes.count === 0) {
        healthyByOrg += 1;
        continue;
      }
      const sendResult = await importsEmails.sendExpiredQuotesAlert({
        orgIdNumeric,
        orgName: typeof org.name === 'string' ? org.name : undefined,
        expiredQuotes,
      });
      if (sendResult.ok) {
        ...
        mutedByOrg += 1;
      } else if (sendResult.reason !== 'no-inbox' && sendResult.reason !== 'no-expired') {
        errors.push({ orgId: String(org.id), error: sendResult.reason || 'send failed' });
      }
    } catch (err) {
      errors.push({
...
// Sprint 58 — weekly supplier-concentration alert. Per-org
// fan-out. Reuses aggregateOpsInsights (cohort #9 lives at
// insights.supplierConcentration — single source of truth, alert
// agrees with the live cockpit reading the same data). Healthy
// orgs (!isConcentrated) skip silently. One org's failure does
// NOT block the others.
async function runImportRequestSupplierConcentrationAlert() {
  const orgs = require('../orgs');
  const importRequests = require('../db/import-requests');
  const importsEmails = require('../imports-emails');
  const dbClient = require('../db/client');
  // Sprint 60 — per-org config (supplierConcentrationThreshold).
  // Same per-org load-inside-loop pattern as the sprint-42/43
  // alerts.
  const operatorConfig = require('../operator-config');
  ...
  for (const org of allOrgs) {
    try {
      ...
      // windowDays is irrelevant to the supplier-concentration
      // cohort (it uses the sprint-57 30d constant directly); 7
      // is arbitrary but harmless.
      // Sprint 60 — load per-org config so the concentration
      // threshold the classifier uses matches what the customer
      // set; falls back silently to the platform default if
      // config is absent.
      const orgConfig = await operatorConfig.getOperatorConfig(orgIdNumeric);
      const agg = await importRequests.aggregateOpsInsights({
        orgId: orgIdNumeric,
      ...
      const supplierConcentration = agg.insights && agg.insights.supplierConcentration;
      if (!supplierConcentration || !supplierConcentration.isConcentrated) {
        healthyByOrg += 1;
        continue;
      }
      const sendResult = await importsEmails.sendSupplierConcentrationAlert({
        orgIdNumeric,
        orgName: typeof org.name === 'string' ? org.name : undefined,
      ...
      if (sendResult.ok) {
        ...
        mutedByOrg += 1;
      } else if (sendResult.reason !== 'no-inbox' && sendResult.reason !== 'not-concentrated') {
        errors.push({ orgId: String(org.id), error: sendResult.reason || 'send failed' });
      }
    } catch (err) {
      errors.push({
...
// Sprint 63 — weekly rating-trend alert. Per-org fan-out.
// Reuses aggregateOpsInsights (cohort #10 lives at
// insights.ratingTrend — single source of truth, alert agrees
// with the live cockpit reading the same data). Healthy orgs
// (!isDeclining) skip silently. One org's failure does NOT block
// the others.
async function runImportRequestRatingTrendAlert() {
  const orgs = require('../orgs');
  const importRequests = require('../db/import-requests');
  const importsEmails = require('../imports-emails');
  const dbClient = require('../db/client');
  // Sprint 64 — per-org config (ratingTrendDropThreshold).
  // Same per-org load-inside-loop pattern as sprints 42/43/60.
  const operatorConfig = require('../operator-config');
  ...
  for (const org of allOrgs) {
    try {
      ...
      // windowDays is irrelevant to the rating-trend cohort
      // (it uses the sprint-62 7d/30d constants directly); 7
      // is arbitrary but harmless.
      // Sprint 64 — load per-org config so the drop threshold
      // the classifier uses matches what the customer set;
      // falls back silently to the platform default if config
      // is absent.
      const orgConfig = await operatorConfig.getOperatorConfig(orgIdNumeric);
      const agg = await importRequests.aggregateOpsInsights({
        orgId: orgIdNumeric,
      ...
      const ratingTrend = agg.insights && agg.insights.ratingTrend;
      if (!ratingTrend || !ratingTrend.isDeclining) {
        healthyByOrg += 1;
        continue;
      }
      const sendResult = await importsEmails.sendRatingTrendAlert({
        orgIdNumeric,
        orgName: typeof org.name === 'string' ? org.name : undefined,
        ratingTrend,
      });
      if (sendResult.ok) {
        ...
        mutedByOrg += 1;
      } else if (sendResult.reason !== 'no-inbox' && sendResult.reason !== 'not-declining') {
        errors.push({ orgId: String(org.id), error: sendResult.reason || 'send failed' });
      }
    } catch (err) {
      errors.push({
...
// Sprint 50 — drain due webhook retries. Reads each org's
// subscriptions, then each subscription's due-retry index, and
// re-attempts via dispatch.flushRetriesForSubscription. Failures
// reschedule with backoff; max attempts → abandoned. One
// subscription's failure does NOT block the others.
async function runWebhookRetryFlush() {
  const orgs = require('../orgs');
  ...
  const allOrgs = await orgs.listAllOrgs({ limit: 10000 });
  if (!Array.isArray(allOrgs) || allOrgs.length === 0) {
    return { ok: true, orgsConsidered: 0, drained: 0, succeeded: 0, abandoned: 0, errors: [] };
  }
  /** @type {Array<{ orgId: string, error: string }>} */
  const errors = [];
  let totalDrained = 0;
  ...
  for (const org of allOrgs) {
    try {
      ...
      const subIds = (await kv.get(webhooks.ORG_INDEX_PREFIX + String(orgIdNumeric))) || [];
      if (!Array.isArray(subIds) || subIds.length === 0) continue;
      for (const subId of subIds) {
        try {
          const result = await dispatch.flushRetriesForSubscription({ subId });
          if (result.drained > 0) {
            totalDrained += result.drained;
            totalSucceeded += result.succeeded || 0;
            totalAbandoned += result.abandoned || 0;
            totalSubsTouched += 1;
          }
        } catch (err) {
          errors.push({
            orgId: String(org.id),
            error: `subId=${subId}: ${err instanceof Error ? err.message : String(err)}`,
          });
        }
      }
    } catch (err) {
...
const JOBS = {
  ...
  'audit-anchor-snapshot': runAuditAnchorSnapshot,
  // Sprint 19 — nightly sweep of quoted import requests whose
  // quote_expires_at has passed.
  'import-request-quote-expiry': runImportRequestQuoteExpiry,
  // Sprint 26 — weekly Monday Ops Insights digest. Per-org fan-out.
  'import-request-insights-digest': runImportRequestInsightsDigest,
  // Sprint 39 — daily stalled-queue alert. Per-org fan-out, fires
  // UK morning time. Healthy orgs (count===0) skip silently.
  'import-request-stalled-queue-alert': runImportRequestStalledQueueAlert,
  // Sprint 41 — daily decline-spike alert. Per-org fan-out, fires
  // 30 min after the stall alert (08:30 UTC = 09:30 BST).
  // Healthy orgs (spikes.length===0) skip silently.
  'import-request-decline-spike-alert': runImportRequestDeclineSpikeAlert,
  // Sprint 70 — daily aging-quotes alert. Sibling to sprint 39's
  // stall alert (customer-decision side of the workflow). Fires
  // an hour after the stall alert (08:00 UTC + 60 min so ops
  // reads the two consecutively). Healthy orgs (count===0) skip
  // silently.
  'import-request-quote-follow-up-alert': runImportRequestQuoteFollowUpAlert,
  // Sprint 54 — weekly quote-acceptance rate alert (Wed 09:00 UTC).
  // Slow signal (30d window vs 60d baseline) so cadence is
  // weekly, not daily. Per-org fan-out; healthy orgs
  // (!isDegraded) skip silently.
  'import-request-quote-acceptance-alert': runImportRequestQuoteAcceptanceAlert,
  // Sprint 78 — weekly expired-quotes alert (Tue 09:00 UTC).
  // Retrospective loss report on cohort #12 — one batch of
  // re-quote candidates beats seven drip emails. Per-org fan-out;
  // healthy orgs (count===0) skip silently.
  'import-request-expired-quotes-alert': runImportRequestExpiredQuotesAlert,
  // Sprint 92 — daily SLA-at-risk alert (08:15 UTC, between the
  // stall and spike alerts). Breach prevention against each org's
  // negotiated target. Healthy orgs skip silently.
  'import-request-sla-at-risk-alert': runImportRequestSlaAtRiskAlert,
  // Sprint 97 — hourly SLA breach sweep. Audit recording, not an
  // email: cadence-independent, pref-independent.
  'sla-breach-sweep': runSlaBreachSweep,
  // Sprint 102 — hourly intake rescue. Self-healing pipeline:
  // orphaned 'submitted' rows get processed server-side.
  'intake-rescue-sweep': runIntakeRescueSweep,
  // Sprint 58 — weekly supplier-concentration alert (Thu 09:15 UTC
  // — moved from 09:00 by the sprint-78 collision fix: the old
  // slot collided with monitoring-scan and never fired).
  // RISK signal — not urgent like stalls but more important than a
  // slow trend. Per-org fan-out; healthy orgs (!isConcentrated)
  // skip silently.
  'import-request-supplier-concentration-alert': runImportRequestSupplierConcentrationAlert,
  // Sprint 63 — weekly rating-trend alert (Fri 09:00 UTC). End-of-
  // week wrap on the slow-cook customer-satisfaction signal so
  // ops can plan investigation Mon AM. Per-org fan-out; healthy
  // orgs (!isDeclining) skip silently.
  'import-request-rating-trend-alert': runImportRequestRatingTrendAlert,
  // Sprint 50 — webhook retry queue drain. Every 5 min. Reads
  // each org's subs + each sub's due-retry index, attempts each
  // due retry, reschedules with backoff (or abandons after 5
  // attempts). One sub's failure does NOT block others.
  'webhook-retry-flush': runWebhookRetryFlush,
};
...
module.exports.runImportRequestQuoteExpiry = runImportRequestQuoteExpiry;
module.exports.runImportRequestInsightsDigest = runImportRequestInsightsDigest;
module.exports.runImportRequestStalledQueueAlert = runImportRequestStalledQueueAlert;
module.exports.runImportRequestDeclineSpikeAlert = runImportRequestDeclineSpikeAlert;
module.exports.runImportRequestQuoteFollowUpAlert = runImportRequestQuoteFollowUpAlert;
module.exports.runImportRequestQuoteAcceptanceAlert = runImportRequestQuoteAcceptanceAlert;
module.exports.runImportRequestExpiredQuotesAlert = runImportRequestExpiredQuotesAlert;
module.exports.runImportRequestSlaAtRiskAlert = runImportRequestSlaAtRiskAlert;
module.exports.runSlaBreachSweep = runSlaBreachSweep;
module.exports.runIntakeRescueSweep = runIntakeRescueSweep;
module.exports.runImportRequestSupplierConcentrationAlert = runImportRequestSupplierConcentrationAlert;
module.exports.runImportRequestRatingTrendAlert = runImportRequestRatingTrendAlert;
module.exports.runWebhookRetryFlush = runWebhookRetryFlush;
