...
// EN-first reminder body. The cron's other emails are fully localised (PL/DE);
// this stream will join them in a follow-up — until then every recipient gets
// the EN body.
function buildDeadlineEmail({ obligations, planUrl, unsubUrl, prefsUrl }) {
  const soonest = obligations[0];
  const plural = obligations.length === 1 ? '' : 's';
  const dayWord = n => `${n} day${n === 1 ? '' : 's'}`;
  const subject = `Compliance deadline${plural}: ${soonest.regime.toUpperCase()} ${soonest.title} in ${dayWord(soonest.daysUntil)}`;
  const lines = obligations.map(o =>
    `• ${o.dueDate} (in ${dayWord(o.daysUntil)} · ${o.severity}) — ${o.regime.toUpperCase()}: ${o.title} [${o.citation}]`);
  const text = [
    `You have ${obligations.length} upcoming trade-compliance ${obligations.length === 1 ? 'obligation' : 'obligations'} on your saved plans:`,
    ``,
    ...lines,
    ``,
    `These dates are statutory. Open your plans for the full breakdown and what to file:`,
    planUrl,
    ``,
    `— OrcaTrade · automated compliance calendar`,
    ``,
    `Not useful? Unsubscribe from deadline reminders: ${unsubUrl}`,
    `Or manage all email preferences: ${prefsUrl}`,
  ].join('\n');
  return { subject, text };
}
...
async function runComplianceDeadlineReminders({ dryRun = false, maxUsers = 5000, asOf } = {}) {
  ...
  for (const userKey of userListKeys) {
    ...
    // Aggregate across the user's plans, deduped by regime+dueDate so the same
    // statutory date appearing on two plans is one line, not two.
    const byKey = new Map();
    for (const planId of planIds) {
      const record = await kv.get(savedPlans.planKey(planId));
      ...
      try {
        result = obligationsForShipment({
          ...
          horizonDays: DEADLINE_REMINDER_HORIZON_DAYS,
          asOf,
        });
      } catch (_) { continue; }
    ...
    const due = [...byKey.values()].sort((a, b) => (a.daysUntil - b.daysUntil) || a.regime.localeCompare(b.regime));
    ...
    const prefsUrl = `${SITE_ORIGIN}/account/preferences/`;
    const planUrl = `${SITE_ORIGIN}/account/plans/`;
    const { subject, text } = buildDeadlineEmail({ obligations: due, planUrl, unsubUrl, prefsUrl });
    const result = await email.send({ to: userEmail, subject, text });
    if (result.ok) {
      sent++;
      try {
