...
async function ingestSanctions({ source = 'OFAC-SDN', text, format = 'ofac-sdn', dryRun = false, maxEntries = 30000 } = {}) {
  ...
  const parsed = format === 'simple' ? parse.parseSimpleCsv(text, { source }) : parse.parseOfacSdnCsv(text);
...
async function runSanctionsRefresh({ dryRun = false, url, source = 'OFAC-SDN', maxEntries = 30000 } = {}) {
  const store = require('../intelligence/sanctions-list-store');
  if (!dryRun && !store.isAvailable()) return { ok: false, reason: 'DATABASE_URL not set' };
  const srcUrl = url || process.env.SANCTIONS_SDN_URL || 'https://www.treasury.gov/ofac/downloads/sdn.csv';
  let text;
  try {
  ...
  return ingestSanctions({ source, text, dryRun, maxEntries });
}
