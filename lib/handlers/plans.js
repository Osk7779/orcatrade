...
const fxQuote = require('../intelligence/fx-quote');
const cbam = require('../intelligence/cbam-analysis');
...
// GET /api/plans
// Stamp `reproducible` (and `dataDrifted`) per plan from a single per-request
// `currentDataSnapshotId` — the same content address `/reproduce` computes,
// so the list view can render a verdict badge without an extra fetch per plan.
function attachReproducibility(record, currentSnapshotId) {
  if (!record || typeof record !== 'object') return record;
  const stored = record.dataSnapshotId || null;
  if (!stored) return { ...record, reproducible: null, dataDrifted: null };
  const matches = stored === currentSnapshotId;
  return { ...record, reproducible: matches, dataDrifted: !matches, currentDataSnapshotId: currentSnapshotId };
}

async function handleList(req, res, user) {
  const records = await savedPlans.listPlans(user.email);
  const currentSnapshotId = dataSnapshot.currentDataSnapshot().id;
  const enriched = await Promise.all(
    records.map(async r => attachReproducibility(
      withActualVariance(planDiff.enrichRecord(r, await computeSnapshot(r.inputs))),
      currentSnapshotId,
    )),
  );
...
async function handleGet(req, res, user, planId) {
  ...
  const enriched = attachReproducibility(
    withActualVariance(planDiff.enrichRecord(record, await computeSnapshot(record.inputs))),
    dataSnapshot.currentDataSnapshot().id,
  );
...
async function handleClearActual(req, res, user, planId) {
  ...
  if (!updated) return jsonResponse(res, 404, { error: 'Plan not found' });
  // ADR 0005: audit-log write propagates; the dispatcher's outer
  // try/catch (api/[...path].js) returns 5xx if events.record throws.
  // Phase 1 will reorder to audit-FIRST so the mutation can be refused
  // on audit-subsystem failure; for now, the visible 5xx is the
  // promise=enforcement step over the previous silent swallow.
...
async function handleRevokeShare(req, res, user, planId) {
  ...
  // ADR 0005 — see handleClearActual above for the rationale.
...
async function handleReproduce(req, res, user, planId) {
  ...
  // Reproducibility-v2 slice 3a+3b: recompute the ORIGINAL landed total by
  // pinning the calculators (FX rates + AD/CVD measures) to the stored snapshot,
  // deterministically — the headline "your original euros, recovered" artifact.
  // (CBAM ETS pinning, which moves the separate CBAM exposure line not the
  // landed total, is slice 3c.)
  let landedReproduction = null;
  try {
    // Merge the per-quote resolved MFN rate (captured on the plan's snapshot at
    // save) into the pinned data so a live-TARIC plan recomputes its ORIGINAL
    // duty, not today's rate (reproducibility-v2 3c).
    const pinnedData = {
      ...stored.snapshot,
      mfnRate: record.snapshot && Number.isFinite(Number(record.snapshot.mfnRate))
        ? Number(record.snapshot.mfnRate) : undefined,
    };
    const originalPlan = await startHandler.composePlan(record.inputs, { pinnedData });
    if (originalPlan && originalPlan.ok) {
      const original = planDiff.extractSnapshot(originalPlan);
      landedReproduction = {
        original: { perShipmentLandedTotal: original.perShipmentLandedTotal, dutyEur: original.dutyEur },
        current: recomputed
          ? { perShipmentLandedTotal: recomputed.perShipmentLandedTotal, dutyEur: recomputed.dutyEur }
          : null,
      };
    }
  } catch (_) { /* reproduction is best-effort context, never breaks the verdict */ }

  // Reproducibility-v2 slice 3a: actually reproduce the ORIGINAL FX numbers by
  // pinning the calculator to the stored snapshot's rate table, alongside the
  // current (live) figures — concrete proof the old euros are recoverable, not
  // merely that the data moved. (Duty/CBAM pinning lands in slice 3b.)
  let fxReproduction = null;
  const cur = record.inputs && record.inputs.quoteCurrency;
  if (cur && String(cur).toUpperCase() !== 'EUR' && stored.snapshot.fx) {
    const fxArgs = {
      customsValueEur: record.inputs.customsValueEur,
      quoteCurrency: cur,
      paymentTermsDays: record.inputs.paymentTermsDays,
    };
    const original = fxQuote.assessFxRisk({ ...fxArgs, pinnedFx: stored.snapshot.fx });
    const live = fxQuote.assessFxRisk({ ...fxArgs, pinnedFx: current.snapshot.fx });
    if (original.ok && live.ok) {
      fxReproduction = {
        currency: original.currency,
        original: { asOf: original.asOf, spotRateForeignPerEur: original.spotRateForeignPerEur, riskEur5pctMove: original.riskEur5pctMove, hedgeCostEur: original.hedgeCostEur },
        current: { asOf: live.asOf, spotRateForeignPerEur: live.spotRateForeignPerEur, riskEur5pctMove: live.riskEur5pctMove, hedgeCostEur: live.hedgeCostEur },
      };
    }
  }

  // Reproducibility-v2 slice 3c (CBAM ETS pinning): when the plan's
  // inputs match a CBAM Annex I category and the origin is non-EU,
  // recompute the certificate-cost exposure twice — once at the
  // snapshot's pinned ETS price, once at the current price — so the
  // CBAM-line drift is itemised alongside the FX + duty drift above.
  // composePlan doesn't surface a `cbamExposure` field today (CBAM
  // lives on the analysis path), so this is computed on-the-fly from
  // the saved inputs rather than read back from the stored snapshot.
  // The recomputation is deterministic: calculateCertificateExposure
  // already accepts etsPriceEur as a parameter; we thread the pinned
  // value through.
  let cbamReproduction = null;
  try {
    const inputs = record.inputs || {};
    const applicability = cbam.determineCbamApplicability({
    ...
    // Only itemise CBAM drift when the calculator agrees CBAM applies
    // AND we have a tonnage from the saved inputs.
    const tonnesGoods = Number(inputs.weightKg) > 0 ? Number(inputs.weightKg) / 1000 : null;
    const pinnedEts = stored.snapshot.cbamEts && Number(stored.snapshot.cbamEts.priceEurPerTonne);
    const currentEts = current.snapshot.cbamEts && Number(current.snapshot.cbamEts.priceEurPerTonne);
    if (
      applicability.applies && applicability.categoryKey &&
      Number.isFinite(tonnesGoods) && Number.isFinite(pinnedEts) && Number.isFinite(currentEts)
    ) {
      const originalExposure = cbam.calculateCertificateExposure({
        tonnesGoods, categoryKey: applicability.categoryKey, etsPriceEur: pinnedEts,
      });
      const currentExposure = cbam.calculateCertificateExposure({
        tonnesGoods, categoryKey: applicability.categoryKey, etsPriceEur: currentEts,
      });
      if (originalExposure && currentExposure) {
        cbamReproduction = {
          categoryKey: applicability.categoryKey,
          tonnesGoods,
          original: {
            etsPriceEurPerTonne: pinnedEts,
            asOf: stored.snapshot.cbamEts && stored.snapshot.cbamEts.asOf,
            certificateCostEurCentral: originalExposure.certificateCostEur.central,
          },
          current: {
            etsPriceEurPerTonne: currentEts,
            asOf: current.snapshot.cbamEts && current.snapshot.cbamEts.asOf,
            certificateCostEurCentral: currentExposure.certificateCostEur.central,
          },
        };
      }
    }
  } catch (_) { /* CBAM reproduction is best-effort context, never breaks the verdict */ }
