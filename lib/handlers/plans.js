...
async function handleList(req, res, user) {
  ...
  const enriched = await Promise.all(
    records.map(async r => planDiff.enrichRecord(r, await computeSnapshot(r.inputs)))
  );
...
async function handleGet(req, res, user, planId) {
  ...
  const enriched = planDiff.enrichRecord(record, await computeSnapshot(record.inputs));
