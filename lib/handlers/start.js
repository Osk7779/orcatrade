...
async function sendPlanEmail({ email, name, companyName, plan }) {
  ...
  const t = plan.totals;
  const subject = `Your import plan: ${inputs.productCategory} ${inputs.originCountry} → ${inputs.destinationCountry}`;
  const shareUrl = `${SITE_ORIGIN}/start/?p=${encodeInputs(inputs)}`;

  const userBody = `Hello${name ? ' ' + name : ''},
...
NEXT STEPS
- Open the Operations Orchestrator to refine: ${SITE_ORIGIN}/agent/orchestrator/
- Browse our calculator-grounded guides: ${SITE_ORIGIN}/guides/
- Reply to this email for a 15-minute call with our HK office.

— OrcaTrade Group
  Warsaw · London · Hong Kong`;

  const founderBody = `New import-plan lead from /start/
...
Reach out within 24h for high-quality intent.`;
  ...
  // Founder/lead notification
  const founderEmail = await resendSend({ apiKey, from, to: founderInbox, subject: `LEAD · ${subject}`, text: founderBody });
