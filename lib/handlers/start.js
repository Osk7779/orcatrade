...
// PR #142: origin country (ISO-2) → supplier currency (ISO-4217).
// Only ECB-supported currencies appear here; VND/BDT (structurally-
// managed; no daily ECB reference rate) intentionally absent — the
// finance async path falls back to the sync mirror for those.
...
async function composePlan(input, { pinnedData = null } = {}) {
  ...
  // HS code is needed BOTH for sourcing (Comtrade trade-flow lookup
  // when wired via recommendCountryAsync — PR #140) AND for customs
  // (TARIC live-fetch — PR #89). Compute once, share. The '99'
  // fallback is the standard "chapter 99 = unclassified" — Comtrade
  // gracefully returns null for it and recommendCountryAsync falls
  // back to the sync rate-card path.
  const hsCode = input.hsCode || CATEGORY_TO_HS[productCategory] || '99';

  // The Comtrade trade-flow lookup needs a period (annual data, last
  // full calendar year is the most reliable). sourcing-quote's
  // recommendCountryAsync mandates an explicit period because the
  // calculator must stay deterministic — only the handler can read
  // the clock. See PR #139 + test/calculator-determinism.test.js.
  const comtradePeriod = String(new Date().getFullYear() - 1);

  // 1. Sourcing comparison (skipped if origin already chosen and not exploring alternatives)
  // PR #140: swapped from sync recommendCountry to
  // recommendCountryAsync. When Comtrade returns trade-flow data for
  // the supplied HS code, the result carries tradeFlowMeta and the
  // downstream buildTierAInput emits a primary_regulator snapshot
  // instead of the rate-card mirror — flipping plan.sourcing.tier_a
  // from eligible:false to eligible:true in production.
  const sourcingResult = await sourcing.recommendCountryAsync({
    productCategory,
    hsCode,
  ...
  // 2. Routing
  //
  // PR #146: swapped from sync calculateQuote to calculateQuoteAsync.
  // When Eurostat returns a fresh water-transport PPI observation
  // (NACE H50) for the requested area, the result carries
  // liveFreightMeta and the downstream buildTierAInput emits a
  // primary_regulator snapshot — flipping plan.routing.tier_a from
  // eligible:false to eligible:true.
  //
  // freightArea fixed at 'EU27_2020' (the EU27 post-Brexit
  // aggregate) — same choice as warehouse-quote (PR #144) because
  // the routing calculator's destination matrix spans both EA and
  // non-EA Member States.
  const routingResult = await routing.calculateQuoteAsync(
    {
      weightKg,
  ...
  // to the sync calculateQuote. (hsCode declared above the sourcing call
  // — PR #140 — so both Comtrade + TARIC paths share the same value.)
  const customsResult = await customs.calculateQuoteAsync({
    ...
    // Reproducibility-v2 3b/3c: a historical recompute pins the AD/CVD measures
    // AND the original resolved MFN rate from the stored snapshot, fully
    // deterministically (no live TARIC re-fetch). Live path is untouched.
    pinnedTradeDefence: pinnedData.tradeDefence && pinnedData.tradeDefence.measures,
    mfnRateOverride: Number.isFinite(pinnedData.mfnRate) ? pinnedData.mfnRate : undefined,
    mfnRateOverrideSource: pinnedData.mfnRate != null ? 'pinned snapshot' : undefined,
    useLiveTaric: false,
  } : {});

  // 4. Warehouse — only if monthly volume is provided
  //
  // PR #144: swapped from sync calculateQuote to calculateQuoteAsync.
  // When Eurostat returns a warehousing-PPI snapshot (NACE H52) for
  // the requested area, the result carries livePpiMeta and the
  // downstream buildTierAInput emits a primary_regulator snapshot —
  // flipping plan.warehouse.tier_a from eligible:false to
  // eligible:true.
  //
  // ppiArea is fixed at 'EU27_2020' (the EU27 post-Brexit aggregate)
  // because the hub catalogue spans both Euro-area (NL/DE/ES) and
  // non-EA (PL/CZ) Member States — EU27 covers them all. EA20 is the
  // narrower alternative if we ever drop the non-EA hubs.
  let warehouseResult = null;
  if (monthlyOrders && monthlyOrders >= 100) {
    warehouseResult = await warehouse.calculateQuoteAsync(
      {
        monthlyOrders,
  ...
  // 5. Finance — payment instrument comparison.
  // PR #142: swapped from sync comparePaymentInstruments to
  // comparePaymentInstrumentsAsync. When ECB returns a fresh spot
  // rate for the supplier-currency on the supplied fxDate, the
  // result carries liveFxMeta and the downstream buildTierAInput
  // emits a primary_regulator snapshot — flipping
  // plan.finance.tier_a from eligible:false to eligible:true.
  //
  // Origin country → supplier currency (ECB-supported only; VND/BDT
  // structurally-managed currencies have no daily ECB reference
  // rate and fall back to the sync mirror path automatically).
  const supplierCurrency = COUNTRY_TO_CURRENCY[originCountry] || null;
  // The handler is allowed to read the clock (calculator stays
  // deterministic per ADR 0020). ECB publishes the previous
  // business day's rate at ~16:00 CET; using today's date as the
  // endPeriod returns the most recent observation.
  const ecbDate = new Date().toISOString().slice(0, 10);

  const financeResult = await finance.comparePaymentInstrumentsAsync(
    {
    ...
    supplierCurrency ? { fxCurrency: supplierCurrency, fxDate: ecbDate } : {},
  );
  ...
  // Tier-A determination on the customs sub-result (ADR 0020).
  // Best-effort: a thrown evaluator returns null and the plan still
  // composes — failure-mode degrades AWAY from Tier-A, never silently
  // into it. The wizard UI reads this field to render the underwriter-
  // grade badge; the observability log lets us track eligible-rate
  // across the wizard's high-traffic path.
  let customsTierA = null;
  if (customsResult.ok) {
    try {
      customsTierA = await tierA.evaluate(customs.buildTierAInput(customsResult));
      log.info('tier_a determined', {
        event: 'tier_a_determined',
        calculatorName: 'customs-quote',
        surface: 'start-wizard',
        eligible: customsTierA.eligible,
        failedReason: customsTierA.failedReason || null,
      });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'tier-a evaluate threw';
      log.warn('tier-a evaluation failed — plan composed without verdict', { err: message });
    }
  }

  // Tier-A determination on the sourcing sub-result. Mirrors the customs
  // block above. Second calculator-grounded verdict per plan — sourcing
  // decisions are six-figure+ and benefit from the same Tier-A signal
  // as duty calculations.
  let sourcingTierA = null;
  if (sourcingResult.ok) {
    try {
      sourcingTierA = await tierA.evaluate(sourcing.buildTierAInput(sourcingResult));
      log.info('tier_a determined', {
        event: 'tier_a_determined',
        calculatorName: 'sourcing-quote',
        surface: 'start-wizard',
        eligible: sourcingTierA.eligible,
        failedReason: sourcingTierA.failedReason || null,
      });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'tier-a evaluate threw';
      log.warn('sourcing tier-a evaluation failed — plan composed without verdict', { err: message });
    }
  }

  // Tier-A determination on the routing sub-result. Mirrors the customs +
  // sourcing blocks above. Third calculator-grounded verdict per plan —
  // freight cost is the largest line item on most landed-cost models;
  // routing decisions move the per-shipment number more than any other
  // single calculator.
  let routingTierA = null;
  if (routingResult.ok) {
    try {
      routingTierA = await tierA.evaluate(routing.buildTierAInput(routingResult));
      log.info('tier_a determined', {
        event: 'tier_a_determined',
        calculatorName: 'routing-quote',
        surface: 'start-wizard',
        eligible: routingTierA.eligible,
        failedReason: routingTierA.failedReason || null,
      });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'tier-a evaluate threw';
      log.warn('routing tier-a evaluation failed — plan composed without verdict', { err: message });
    }
  }

  // Tier-A determination on the finance sub-result. Fourth calculator-
  // grounded verdict per plan. PRICING_SNAPSHOT is OrcaTrade's partner-
  // bank rate table → mirror only, so TA-2 reliably fails today; when
  // ECB FX lands (see finance-quote.js note above buildTierAInput) the
  // verdict will start passing TA-2 for hedging quotes.
  let financeTierA = null;
  if (financeResult.ok) {
    try {
      financeTierA = await tierA.evaluate(finance.buildTierAInput(financeResult));
      log.info('tier_a determined', {
        event: 'tier_a_determined',
        calculatorName: 'finance-quote',
        surface: 'start-wizard',
        eligible: financeTierA.eligible,
        failedReason: financeTierA.failedReason || null,
      });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'tier-a evaluate threw';
      log.warn('finance tier-a evaluation failed — plan composed without verdict', { err: message });
    }
  }

  // Tier-A determination on the warehouse sub-result. Fifth and final
  // calculator-grounded verdict per plan — closes the wedge at 5/5 at
  // the composer layer. Warehouse is conditionally computed (only when
  // monthlyOrders >= 100), so the verdict is null when the wizard
  // caller skipped the volume question. Today's PRICING_SNAPSHOT is
  // the OrcaTrade quarterly 3PL benchmark survey (mirror only), so
  // TA-2 reliably fails until we wire in primary-regulator sources
  // (Eurostat warehousing PPI, or direct hub API feeds).
  let warehouseTierA = null;
  if (warehouseResult && warehouseResult.ok) {
    try {
      warehouseTierA = await tierA.evaluate(warehouse.buildTierAInput(warehouseResult));
      log.info('tier_a determined', {
        event: 'tier_a_determined',
        calculatorName: 'warehouse-quote',
        surface: 'start-wizard',
        eligible: warehouseTierA.eligible,
        failedReason: warehouseTierA.failedReason || null,
      });
    } catch (err) {
      const message = err instanceof Error ? err.message : 'tier-a evaluate threw';
      log.warn('warehouse tier-a evaluation failed — plan composed without verdict', { err: message });
    }
  }
  ...
  return {
    ...
    finance: {
      ok: financeResult.ok,
      recommendation: financeResult.ok ? financeResult.recommendation : null,
      instruments: financeResult.ok ? financeResult.instruments : null,
      paymentEducation: financeResult.ok ? financeResult.paymentEducation : null,
      tier_a: financeTierA,
    },
    compliance: {
...
// ── Goods master quote-time inheritance (L1.1 follow-up) ─────────────
//
// When a signed-in user submits the wizard with an `sku` field, look up
// the matching active goods_master row for their primary org and inherit
// hsCode + originCountry + cbamInScope into the wizard inputs (without
// overwriting anything the user explicitly supplied). This is the moment
// the system-of-record thesis becomes visible to the customer: a
// returning user types `WIDGET-001` and never types HS code again.
//
// Failure modes are all benign + silent:
//   - no session cookie → no inheritance; wizard works as before
//   - signed-in but no org → no inheritance
//   - no DATABASE_URL → no inheritance
//   - no SKU in body → no inheritance
//   - SKU not found → no inheritance
// The wizard's anonymous-user funnel is preserved end-to-end.
//
// When inheritance DOES happen, the resolved body carries the inherited
// fields and the returned `inheritance` block names what came from the
// goods master so the UI / email can surface "[from your goods master]".

/**
 * @param {*} req
 * @param {Record<string, any>} body
 * @returns {Promise<{ body: Record<string, any>, inheritance: object | null }>}
 */
async function resolveGoodsMasterInheritance(req, body) {
  const sku = body && typeof body.sku === 'string' ? body.sku.trim() : '';
  if (!sku) return { body, inheritance: null };

  // Best-effort auth: if the cookie isn't present, fall through.
  let user;
  try { user = await auth.getCurrentUser(req); } catch (_) { user = null; }
  if (!user || !user.email) return { body, inheritance: null };

  // Resolve the user's primary org → numeric Postgres id.
  let userOrgs;
  try { userOrgs = await orgsLib.listOrgsForEmail(user.email); } catch (_) { userOrgs = null; }
  if (!Array.isArray(userOrgs) || userOrgs.length === 0) return { body, inheritance: null };

  if (!dbClient.isConfigured()) return { body, inheritance: null };

  /** @type {Record<string, any> | null} */
  let orgRow = null;
  try {
    orgRow = await dbClient.queryOne(
      `SELECT id FROM organisations WHERE external_id = $1`,
      [userOrgs[0].id],
    );
  } catch (_) { orgRow = null; }
  if (!orgRow) return { body, inheritance: null };

  const lookup = await goodsDb.getGoodsBySku({ orgId: Number(orgRow.id), sku });
  if (!lookup.ok) return { body, inheritance: null };
  /** @type {Record<string, any>} */
  const goods = /** @type {any} */ (lookup).goods;

  // Inherit only fields the wizard input left empty. The user's explicit
  // values always win — inheritance is a default-fill, not an override.
  const inheritedFields = [];
  /** @type {Record<string, any>} */
  const enriched = { ...body };
  if (!enriched.hsCode && goods.hsCode) { enriched.hsCode = goods.hsCode; inheritedFields.push('hsCode'); }
  if (!enriched.originCountry && goods.originCountry) { enriched.originCountry = goods.originCountry; inheritedFields.push('originCountry'); }

  if (inheritedFields.length === 0) {
    // SKU matched but nothing actually inherited — still surface the
    // match so the UI can show "we recognised this SKU".
    return {
      body: enriched,
      inheritance: {
        matched: true,
        ...
        cbamInScope: goods.cbamInScope === true,
        inheritedFields: [],
      },
    };
  }

  log.info('goods master inherited fields', {
    event: 'goods_master_inherited',
    sku: goods.sku,
    externalId: goods.externalId,
    inheritedFields,
  });
...
async function sendPlanEmail({ email, name, companyName, plan, locale }) {
  ...
  const from = require('../email').resolveFrom();
  const founderInbox = process.env.ORCATRADE_LEADS_INBOX || require('../email').teamInbox();
...
module.exports = async (req, res) => {
  ...
  // L1.1 follow-up: enrich the body with any matching goods_master row.
  // Anonymous users + missing-SKU + no-PG paths fall through unchanged.
  const { body, inheritance: goodsInheritance } = await resolveGoodsMasterInheritance(req, rawBody);
  ...
  if (goodsInheritance) {
    plan.goodsMasterInheritance = goodsInheritance;
  }
...
module.exports.resolveGoodsMasterInheritance = resolveGoodsMasterInheritance;
