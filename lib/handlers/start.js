...
async function composePlan(input, { pinnedData = null } = {}) {
  ...
  // 5. Finance — payment instrument comparison. The wizard caller is a
  // first-time importer with no supplier-relationship history yet, so we
  // pass relationshipMonths=0 and a balanced risk appetite. amountEur is
  // the consignment value (customsValueEur). Calculator-grounded per
  // ADR 0002 — comparePaymentInstruments returns a deterministic top
  // pick, full instrument array, and a pricing snapshot.
  const financeResult = finance.comparePaymentInstruments({
