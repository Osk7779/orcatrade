...
function composePlan(input) {
  ...
  for (const origin of allOrigins) {
    ...
    const altLanded = (altRecMode?.totalEur || 0)
      + customsValueEur
      + (altStandard?.dutyEur || 0)
      + (altStandard?.vatEur || 0)
      + (altStandard?.brokerageEur || 0);

    originMatrix.push({
      origin,
      isUserChoice: origin === originCountry,
      dutyRatePct: altCustoms.duty.ratePercent,
      transportEur: altRecMode?.totalEur || 0,
      transportMode: altRouting.recommendation.primary,
      transitDaysLabel: altRecMode?.transitDaysLabel || '—',
      dutyEur: altStandard?.dutyEur || 0,
      vatEur: altStandard?.vatEur || 0,
      brokerageEur: altStandard?.brokerageEur || 0,
  ...
  return {
    ...
    fx: quoteCurrency !== 'EUR'
      ? fx.assessFxRisk({ customsValueEur, quoteCurrency, paymentTermsDays })
      : null,
...
function composePlanWithRoadmap(input) {
...
module.exports = async (req, res) => {
  ...
  const plan = composePlanWithRoadmap(body);
  if (!plan.ok) {
