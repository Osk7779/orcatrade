...
const METHODOLOGY = Object.freeze({
  comparison: 'customer-reported actual landed cost vs the estimate snapshotted at plan-save time (not re-priced later)',
...
async function computeLedger() {
  const rows = await actuals.listFromPg({ limit: 10000 });
  return ledgerCalc.computeAccuracyLedger(rows);
}
