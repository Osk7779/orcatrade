...
— OrcaTrade Group
  ...
  try {
    ...
    reqLog.warn('resend send threw', { action: 'send_magic_link', err: err.message });
...
async function handleRequest(req, res) {
  ...
  await sendMagicLinkEmail({ email, magicLink }, reqLog);

  // Always return 202 with a generic message
  res.statusCode = 202;
  ...
  switch (action) {
    ...
    default:
      ...
      return res.end(JSON.stringify({
        ...
        available: ['request', 'verify', 'me', 'logout'],
      }));
  }
};
