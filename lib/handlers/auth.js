...
async function handleMe(req, res) {
  ...
  res.statusCode = 200;
  return res.end(JSON.stringify({ ok: true, user, isAdmin, hasPassword }));
}
...
async function handleSignup(req, res) {
  ...
  if (password) {
    ...
    const pending = await auth.createPendingSignup(email, passwordRecord);
    if (!pending) {
      res.statusCode = 500;
...
async function handleSignupConfirm(req, res) {
  ...
  res.statusCode = 302;
  res.setHeader('Location', '/account/?welcome=1');
  return res.end();
}
  ...
  switch (action) {
    ...
    default:
      ...
      return res.end(JSON.stringify({
        ...
        available: ['request', 'verify', 'me', 'logout', 'login', 'signup', 'password', 'revoke-all', 'sessions'],
      }));
  }
};
