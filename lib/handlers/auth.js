...
async function handleRequest(req, res) {
  ...
  // Enforced-SSO (apex III1): if this email's org mandates SSO, refuse the
  // magic link and point them at their IdP. Checked before minting a token so
  // no link is ever sent for an enforced domain.
  const enforcement = await ssoConfig.isEnforcedForEmail(email);
  if (enforcement.enforced) {
    log.withContext({ requestId: req.requestId, action: 'request_magic_link' })
      .info('magic link blocked — SSO enforced', { orgId: enforcement.orgId });
    res.statusCode = 403;
    return res.end(JSON.stringify({
      error: 'Your organisation requires single sign-on. Please sign in through your company identity provider.',
      ssoRequired: true,
      ssoInitiateUrl: `${SITE_ORIGIN}/api/auth/sso/initiate?org=${encodeURIComponent(enforcement.orgId)}`,
    }));
  }
...
async function handleVerify(req, res) {
  ...
  // Sprint account-uniqueness-v1 — every successful magic-link verify
  // either creates or refreshes the account marker. First verify for a
  // brand-new email writes the createdAt; subsequent verifies just
  // bump lastSignInAt. Both are fire-and-forget.
  auth.markAccountExists(email, { source: 'magic-link' }).catch(() => {});
...
async function handleSignup(req, res) {
  ...
  const returnTo = auth.isSafeReturnTo(body.returnTo) || null;
  // Sprint account-uniqueness-v1 — refuse to create a second account
  // for an email that already has one. The product decision is to
  // surface this explicitly (HTTP 409) rather than mask it behind a
  // generic 202; user-visible duplicate-detection beats enumeration-
  // resistance for the signup form (a separate /account/ sign-in flow
  // already exists for returning users).
  if (await auth.accountExists(email)) {
    try { await events.record('auth_signup_blocked_duplicate', { email, ip }); } catch (_) {}
    reqLog.info('signup blocked — account already exists', { email });
    res.statusCode = 409;
    return res.end(JSON.stringify({
...
async function handleSignupConfirm(req, res) {
  ...
  // Sprint account-uniqueness-v1 — stamp the durable account marker so
  // a second signup attempt for this email is refused (see handleSignup).
  auth.markAccountExists(email, { source: pending.passwordRecord ? 'signup-password' : 'signup-magic' }).catch(() => {});
  try {
...
// ── Sprint sso-oidc-v1 (phase 2) — enterprise SSO ───────
//
// GET /api/auth/sso/initiate?org=<id> — start the OIDC code flow: mint
//   state/nonce/PKCE, stash the flow, 302 to the org's IdP.
// GET /api/auth/sso/callback?code=&state= — exchange the code, verify the
//   ID token against the IdP JWKS (lib/oidc), and mint our session.
//
// Fail-closed: any verification problem → no session, redirect to a
// generic sign-in-error page. The org's IdP is configured out-of-band
// via ssoConfig.setConfig (admin surface is a follow-on).

function ssoErrorRedirect(res, reason) {
  res.statusCode = 302;
  res.setHeader('Location', '/account/?sso_error=' + encodeURIComponent(reason || 'failed'));
  return res.end();
}

// Small network helpers — split out so tests can stub global.fetch.
//
// Both wrap the upstream IdP fetch in circuit.run() per ADR 0006. A slow or
// hanging IdP must not exhaust Vercel function concurrency; without the
// breaker (audit gap 2026-06-08), a 5xx-stuck token_endpoint could pin every
// callback handler at its function timeout. The fallback returns null, which
// the call-site (handleSsoCallback) already maps to ssoErrorRedirect — so
// the breaker's "open" state degrades to a clean user-facing redirect rather
// than a hung tab.
//
// Threshold/cooldown are tighter than the default Anthropic/Resend wraps:
// SSO callbacks are infrequent enough that 3 consecutive failures is a real
// signal, and 60s cooldown means a flapping IdP gets one probe per minute
// rather than one per 30s (matches the typical IdP-side rate-limit posture).
const OIDC_FETCH_TIMEOUT_TOKEN_MS = 5000;
const OIDC_FETCH_TIMEOUT_JWKS_MS = 3000;
const OIDC_CIRCUIT_THRESHOLD = 3;
const OIDC_CIRCUIT_COOLDOWN_MS = 60_000;

async function exchangeCodeForTokens(cfg, code, codeVerifier, redirectUri) {
  ...
  return circuit.run('oidc-token', async () => {
    const resp = await fetch(cfg.tokenEndpoint, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded', Accept: 'application/json' },
      body: body.toString(),
      signal: AbortSignal.timeout(OIDC_FETCH_TIMEOUT_TOKEN_MS),
    });
    if (!resp.ok) {
      // Non-2xx → treat as failure so the breaker counts it. Returning null
      // here would mask a hosed IdP from the circuit.
      throw new Error(`oidc-token ${resp.status}`);
    }
    try { return await resp.json(); } catch (_) { return null; }
  }, {
    threshold: OIDC_CIRCUIT_THRESHOLD,
    cooldownMs: OIDC_CIRCUIT_COOLDOWN_MS,
    fallback: ({ shortCircuited, err }) => {
      if (shortCircuited) {
        log.warn('oidc token exchange skipped (circuit open)', { action: 'oidc_token' });
      } else {
        log.warn('oidc token exchange failed', { action: 'oidc_token', err: err && err.message });
      }
      return null;
    },
  });
...
async function fetchJwks(cfg) {
  return circuit.run('oidc-jwks', async () => {
    const resp = await fetch(cfg.jwksUri, {
      headers: { Accept: 'application/json' },
      signal: AbortSignal.timeout(OIDC_FETCH_TIMEOUT_JWKS_MS),
    });
    if (!resp.ok) {
      throw new Error(`oidc-jwks ${resp.status}`);
    }
    try { return await resp.json(); } catch (_) { return null; }
  }, {
    threshold: OIDC_CIRCUIT_THRESHOLD,
    ...
    fallback: ({ shortCircuited, err }) => {
      if (shortCircuited) {
        log.warn('oidc jwks fetch skipped (circuit open)', { action: 'oidc_jwks' });
      } else {
        log.warn('oidc jwks fetch failed', { action: 'oidc_jwks', err: err && err.message });
      }
      return null;
    },
  });
...
function readQueryParam(req, name) {
  if (req.query && req.query[name] != null) {
    return Array.isArray(req.query[name]) ? req.query[name][0] : String(req.query[name]);
  }
  try { return new URL(req.url || '/', SITE_ORIGIN).searchParams.get(name) || ''; } catch (_) { return ''; }
}

async function handleSsoInitiate(req, res) {
  if (req.method !== 'GET') {
    res.statusCode = 405;
    return res.end(JSON.stringify({ error: 'Method not allowed' }));
  }
  const orgId = readQueryParam(req, 'org');
  if (!orgId) {
    res.statusCode = 400;
    return res.end(JSON.stringify({ error: 'org query param required' }));
  }
  const cfg = await ssoConfig.getConfig(orgId);
  if (!ssoConfig.isComplete(cfg)) {
    res.statusCode = 404;
    return res.end(JSON.stringify({ error: 'No SSO configured for this organisation' }));
  }
  const state = oidc.generateState();
  const nonce = oidc.generateNonce();
  const pkce = oidc.generatePkce();
  const stored = await ssoConfig.createFlow(state, { orgId, nonce, codeVerifier: pkce.verifier });
  if (!stored) {
    res.statusCode = 500;
    return res.end(JSON.stringify({ error: 'Could not start SSO flow' }));
  }
  const url = oidc.buildAuthorizationUrl({
    authorizationEndpoint: cfg.authorizationEndpoint,
    clientId: cfg.clientId,
    redirectUri: `${SITE_ORIGIN}/api/auth/sso/callback`,
    state,
    nonce,
...
// GET /api/auth/sso/discover?email=<email> — public. Resolve an email's
// domain to the org that has claimed it for SSO, so the /account/ "Sign
// in with SSO" box can route a user to the right IdP without them knowing
// an org id. Returns only the initiate URL — never any config/secret. A
// non-match returns ssoAvailable:false (the UI falls back to email/password).
async function handleSsoDiscover(req, res) {
  if (req.method !== 'GET') {
    res.statusCode = 405;
    return res.end(JSON.stringify({ error: 'Method not allowed' }));
  }
  const ip = req.headers['x-forwarded-for']?.split(',')[0]?.trim() || 'unknown';
  const rate = await consumeRateLimit('sso-discover', ip, 30, 60 * 1000);
  if (rate.limited) {
    res.statusCode = 429;
    return res.end(JSON.stringify({ error: 'Too many requests' }));
  }
  const email = readQueryParam(req, 'email');
  const orgId = await ssoConfig.findOrgByDomain(email);
  if (!orgId) {
    res.statusCode = 200;
    return res.end(JSON.stringify({ ok: true, ssoAvailable: false }));
  }
  res.statusCode = 200;
  return res.end(JSON.stringify({
    ok: true,
...
async function handleSsoCallback(req, res) {
  ...
  if (!code || !state) return ssoErrorRedirect(res, 'missing-params');

  // Single-use flow lookup (deleted on read → state can't be replayed).
  const flow = await ssoConfig.consumeFlow(state);
  if (!flow || !flow.orgId) return ssoErrorRedirect(res, 'bad-state');

  const cfg = await ssoConfig.getConfig(flow.orgId);
  if (!ssoConfig.isComplete(cfg)) return ssoErrorRedirect(res, 'no-config');

  const reqLog = log.withContext({ requestId: req.requestId, action: 'sso_callback' });
  let tokens;
  try {
    tokens = await exchangeCodeForTokens(cfg, code, flow.codeVerifier, `${SITE_ORIGIN}/api/auth/sso/callback`);
  } catch (err) { reqLog.warn('sso token exchange threw', { err: err && err.message }); return ssoErrorRedirect(res, 'exchange-failed'); }
  if (!tokens || !tokens.id_token) return ssoErrorRedirect(res, 'exchange-failed');

  let jwks;
  try { jwks = await fetchJwks(cfg); } catch (err) { reqLog.warn('sso jwks fetch threw', { err: err && err.message }); return ssoErrorRedirect(res, 'jwks-failed'); }
  if (!jwks) return ssoErrorRedirect(res, 'jwks-failed');

  const verified = oidc.verifyIdToken(tokens.id_token, {
    jwks,
    issuer: cfg.issuer,
    clientId: cfg.clientId,
    nonce: flow.nonce,
  ...
  if (!verified.ok) {
    reqLog.warn('sso id_token rejected', { reason: verified.reason, orgId: flow.orgId });
    return ssoErrorRedirect(res, 'token-' + verified.reason);
  }

  const email = verified.claims.email;
  // Optional domain allowlist (defence-in-depth).
  if (!ssoConfig.emailDomainAllowed(cfg, email)) {
    reqLog.warn('sso email domain not allowed', { orgId: flow.orgId });
    return ssoErrorRedirect(res, 'domain-not-allowed');
  }

  // Verified — mint our session (JIT; auth is email-keyed).
  const { sid, signinIp } = mintSessionResponse(req, res, email, { source: 'sso' });
  try { await events.record('auth_signin', { email, source: 'sso', orgId: flow.orgId, ip: signinIp, sid }); } catch (_) {}
  reqLog.info('sso sign-in ok', { email, orgId: flow.orgId });
  res.statusCode = 302;
  res.setHeader('Location', '/account/');
  return res.end();
}
...
module.exports = async (req, res) => {
  ...
  switch (action) {
    ...
    case 'sso':
      // /api/auth/sso/initiate?org=<id> → 302 to the org's IdP
      // /api/auth/sso/callback?code=&state= → exchange + verify + session
      if (sid === 'initiate') return handleSsoInitiate(req, res);
      if (sid === 'callback') return handleSsoCallback(req, res);
      if (sid === 'discover') return handleSsoDiscover(req, res);
      res.statusCode = 404;
      return res.end(JSON.stringify({
        error: 'Unknown /api/auth/sso sub-action',
        available: ['GET /api/auth/sso/initiate?org=<id>', 'GET /api/auth/sso/callback', 'GET /api/auth/sso/discover?email=<email>'],
      }));
    case 'sessions':
    ...
    default:
      ...
      return res.end(JSON.stringify({
        ...
        available: ['request', 'verify', 'me', 'logout', 'login', 'signup', 'password', 'mfa', 'sso', 'revoke-all', 'sessions'],
      }));
  }
};
...
module.exports.handleSsoInitiate = handleSsoInitiate;
module.exports.handleSsoCallback = handleSsoCallback;
module.exports.handleSsoDiscover = handleSsoDiscover;
...
// Exposed for ADR 0006 enforcement tests — see test/auth-oidc-circuit-wrap.test.js.
// Both helpers wrap their fetch in lib/circuit.js; exporting them lets the test
// stub global.fetch and confirm the fallback fires.
module.exports.exchangeCodeForTokens = exchangeCodeForTokens;
module.exports.fetchJwks = fetchJwks;
