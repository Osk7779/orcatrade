...
// The six canonical RBAC roles plus the legacy 'member' alias (= viewer), so
// existing memberships stay valid while new seats use the enterprise roles.
const ALLOWED_ROLES = new Set([...rbac.ROLES, 'member']);
...
// Change an existing member's role. Owner is reserved for transferOwnership —
// you can neither grant it here nor change the current owner's role. Returns
// { changed:true, member } or { changed:false, reason }.
async function changeMemberRole(orgId, { email, role }) {
  const e = normaliseEmail(email);
  if (!e) throw new Error('changeMemberRole: email required');
  if (!ALLOWED_ROLES.has(role)) {
    throw new Error(`changeMemberRole: role must be one of ${[...ALLOWED_ROLES].join(', ')}`);
  }
  if (role === 'owner') throw new Error('changeMemberRole: cannot grant owner; use transferOwnership');

  const members = await listMembers(orgId);
  const target = members.find(m => m.email === e);
  if (!target) return { changed: false, reason: 'not-a-member' };
  if (target.role === 'owner') return { changed: false, reason: 'cannot-change-owner' };
  if (target.role === role) return { changed: false, reason: 'no-change', member: target };

  const updatedMember = { ...target, role };
  const updated = members.map(m => (m.email === e ? updatedMember : m));
  await kv.set(membersKey(orgId), updated, { ttlSeconds: ORG_TTL_SECONDS });
  return { changed: true, member: updatedMember };
}
...
// The raw role a user holds in an org, or null if not a member. The RBAC layer
// (lib/rbac.js) maps the role → permissions; handlers gate on rbac.can(role, …).
async function getMemberRole(orgId, email) {
  const e = normaliseEmail(email);
  const members = await listMembers(orgId);
  const member = members.find(m => m.email === e);
