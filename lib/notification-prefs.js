...
const PREF_KEYS = [
  ...
  // Sprint 54 — weekly quote-acceptance rate alert. Matches the
  // sprint-53 cohort #8: of requests that EXITED `quoted` in the
  // last 30 days, what % went to customer_approved? When the
  // current rate drops below 75% of the 60-day baseline AND the
  // denominator is >= 5, surface the signal. WEEKLY cadence
  // (Wed 09:00 UTC) — the 30d/60d windows are slow-moving, daily
  // alerts would be noise. Healthy weeks (!isDegraded) skip
  // silently.
  'importQuoteAcceptanceAlertEmails',// weekly quote-acceptance rate alert → ops admin email
  // Sprint 58 — weekly supplier-concentration alert. Matches the
  // sprint-57 cohort #9: of all picks in the last 30 days, what
  // share went to ONE dominant country? When that share crosses
  // 75% AND there are >= 5 picks to compare, surface the
  // sourcing-risk signal so the team can diversify before a
  // tariff shift / sanctions update takes the pipeline offline.
  // WEEKLY cadence (Thu 09:00 UTC) — RISK state, not urgent like
  // stalls but more important than a slow trend. Healthy weeks
  // (!isConcentrated) skip silently.
  'importSupplierConcentrationAlertEmails',// weekly supplier-concentration alert → ops admin email
  // Sprint 63 — weekly rating-trend alert. Matches the sprint-62
  // cohort #10: is the current 7-day avg rating drifting BELOW
  // the prior 23-day baseline by >= 0.5 stars? Catches the
  // slow-cook degradation that sprint-33 per-event 1-2★ alerts
  // miss. WEEKLY cadence (Fri 09:00 UTC) — slow-moving signal,
  // end-of-week wrap so ops can plan investigation Mon AM.
  // Healthy weeks (!isDeclining) skip silently.
  'importRatingTrendAlertEmails',// weekly rating-trend alert → ops admin email
  // Sprint 70 — daily aging-quotes alert. Matches the sprint-69
  // cohort #11: quotes sitting in `quoted` status past 5 days with
  // no customer_approved / customer_declined. Sibling to sprint 39's
  // stalled-queue alert but on the CUSTOMER side of the workflow.
  // DAILY cadence (same as stalls + spikes) — an aging quote
  // benefits from a same-day nudge, not a week-later
  // reminder. Cron runs staggered after the stall alert. Healthy
  // days (count === 0) skip silently.
  'importQuoteFollowUpAlertEmails',// daily aging-quotes alert → ops admin email
  // Sprint 78 — weekly expired-quotes alert. Matches the sprint-77
  // cohort #12: quotes that hit expiry undecided in the last 30
  // days, value-weighted (the € at stake is the headline). WEEKLY
  // cadence (Tue 09:00) — expiry is a retrospective loss report,
  // not a same-day chase; one batch of re-quote candidates beats
  // seven drip emails about the same rows. Healthy months
  // (count === 0) skip silently.
  'importExpiredQuotesAlertEmails',// weekly expired-quotes alert → ops admin email
  // Sprint 92 — daily SLA-at-risk alert. Matches the sprint-91
  // cohort #13: unquoted requests past 75% of the org's
  // NEGOTIATED turnaround target, split at-risk vs breached.
  // DAILY 08:15 — the breach-prevention signal where minutes
  // matter most. Healthy days (total 0) skip silently.
  'importSlaAtRiskAlertEmails',// daily SLA-at-risk alert → ops admin email
];
