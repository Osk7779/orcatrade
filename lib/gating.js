...
//
// Mirrors checkQuota's posture but reads cumulative cents-spent
// for the current month from lib/ai/spend-cap.js (the ledger that
// lib/ai/cost-telemetry.js maintains after every Anthropic call).
//
// Failure posture: fail-OPEN on missing identity / KV outage.
// The cap is a defensive floor — the user-facing agentQueriesPerMonth
// quota is the actual gate; this only kicks in when a pathological
// prompt blows past the budget the per-query quota implicitly assumes.
// A KV outage during read would be the wrong moment to lock everyone
// out of the agent surface.
async function checkAgentSpend(req) {
  const resolved = await resolveTierForRequest(req);
  const capCents = tiers.getQuota(resolved.tierId, 'monthlyAnthropicSpendCapCents');
  if (capCents === Infinity || !Number.isFinite(capCents) || capCents <= 0) {
    return { allowed: true, currentCents: 0, capCents: 'unlimited', tierId: resolved.tierId };
  }
  if (resolved.kind !== 'email') {
    // Anonymous identity — no ledger to read. Fall through to
    // allow; the per-IP rate-limit on the handler is the
    // anonymous-traffic protection.
    return { allowed: true, currentCents: 0, capCents, tierId: resolved.tierId, kind: 'anonymous' };
  }
  // Lazy-require to avoid a require cycle (spend-cap.js doesn't
  // import gating, but cost-telemetry might transitively).
  const spendCap = require('./ai/spend-cap');
  const { currentCents, bucket } = await spendCap.getMonthlySpend({ email: resolved.identity });
  if (currentCents < capCents) {
    return {
      allowed: true,
      currentCents,
      capCents,
      remainingCents: capCents - currentCents,
      bucket,
  ...
  return {
    ...
    status: 429,
    body: {
      error: `Monthly Anthropic spend cap reached (${(currentCents / 100).toFixed(2)} / ${(capCents / 100).toFixed(2)} EUR).`,
      code: 'tier_spend_cap',
      currentTier: resolved.tierId,
      currentEur: Math.round(currentCents) / 100,
      capEur: Math.round(capCents) / 100,
      period: bucket,
      upgradeUrl: '/pricing/',
    },
  };
