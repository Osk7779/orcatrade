...
// Sprint 56 — per-key scope taxonomy. Sprint 44 keys were
// admin-equivalent (full read access to every endpoint); sprint 56
// lets the key creator narrow that to a subset so a leaked key has
// a bounded blast radius.
//
// Scope taxonomy v1 (read-only — sprint 45 already gates writes):
//   imports:read   — list + detail + history + dossier + quote
//   insights:read  — cohort aggregates (/api/imports/insights)
//   audit:read     — per-request + org-wide audit CSV exports
//   exports:read   — list CSV export (/api/imports/export.csv)
//
// Back-compat: scopes:[] (empty) means "no narrowing — admin-
// equivalent" (= legacy sprint-44 behaviour). The keys already in
// KV from sprint 44 default to this shape, so no migration is
// needed.
...
// Validate the scopes payload at create time. Strict: every entry
// must be a string AND must be in API_KEY_SCOPES. Duplicates
// silently de-duped. Empty array is the legacy "admin-equivalent"
// shape — explicitly allowed.
function validateScopes(scopes) {
  ...
  for (const s of scopes) {
    if (typeof s !== 'string') {
      return { ok: false, error: 'scopes entries must be strings' };
    }
    if (!API_KEY_SCOPES.includes(s)) {
      return { ok: false, error: `scopes contains unsupported value: ${s}` };
    }
...
//
// Sprint 56 — accepts an optional `scopes` array. Empty array (or
// omitted) preserves the legacy sprint-44 "admin-equivalent"
// behaviour. Non-empty narrows the bearer to those endpoints only.
//
// @param {{ orgIdNumeric: number, label: string, actorEmailHash?: string, scopes?: string[] }} input
async function createApiKey({ orgIdNumeric, label, actorEmailHash, scopes }) {
  ...
  // Sprint 56 — validate scopes when provided; treat omitted /
  // undefined / null as empty (back-compat with sprint-44 callers
  // that didn't know about scopes).
  const scopeCheck = validateScopes(Array.isArray(scopes) ? scopes : []);
  ...
  const meta = {
    ...
    // Sprint 56 — narrowed permission set. Empty array = "no
    // narrowing, admin-equivalent" so sprint-44 keys read out
    // from KV with `scopes: undefined` continue to work as
    // before (the bearer-path defensive normalizes to []).
  ...
  for (const h of hashes) {
    try {
      ...
      out.push({
        ...
        // Sprint 56 — surface scopes in the list response so the
        // UI can render the scope chip per row. Defensive
        // normalize: a sprint-44 record without the field reads
        // as [] (unscoped / admin-equivalent).
...
async function lookupByBearer(authHeader) {
  ...
  return {
    ...
    label: meta.label,
    keyId: hash,
    // Sprint 56 — surface scopes alongside the org context so
    // the imports dispatcher can enforce per-action scope
    // checks. Defensive normalize: a sprint-44 key read out
    // from KV without the field is [] (unscoped, admin-
    // equivalent — exactly the legacy behaviour).
    scopes: Array.isArray(meta.scopes) ? meta.scopes : [],
  };
}
