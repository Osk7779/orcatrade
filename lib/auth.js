...
// ── Sprint returnto-resume-v1 — open-redirect-safe return URL ───
//
// Used to thread "where the user came from" through magic-link round
// trips, signup confirmations, password resets, and password logins.
// Anything we honour MUST be a same-site relative URL so we can't be
// weaponised as an open redirector.
//
// Accept:  /pricing/, /pricing/?subscribe=growth-monthly, /account/billing/
// Reject:  https://evil.com, //evil.com, /\\evil.com (backslash bypass),
//          javascript:, data:, mailto:, anything > 512 chars
//
// Returns the normalised relative URL on success, null on rejection.
function isSafeReturnTo(value) {
  ...
  // Protocol-relative URLs (//evil.com) — block
  if (trimmed[1] === '/' || trimmed[1] === '\\') return null;
  // Backslash anywhere (some browsers normalise \\ to //, which would
  // bypass the protocol-relative check above)
  if (trimmed.includes('\\')) return null;
  // Bare protocol smells — defence in depth even though the leading
  // "/" already forbids these
  if (/^\/?(javascript|data|file|vbscript|mailto):/i.test(trimmed)) return null;
  return trimmed;
}
...
// ── Account existence marker (Sprint account-uniqueness-v1) ─────
//
// We need a single canonical answer to "does an account already exist
// for this email?" so /api/auth/signup can refuse duplicates with a
// clear message. The pre-existing `password:<email>` record only
// covers password signups — magic-link-only users have no such record,
// yet they are still "accounts" (saved plans, alerts, agent memory are
// keyed to their email).
//
// Solution: set an `account:<email>` KV marker on first successful
// auth (signup-confirm OR magic-link verify). The marker is the
// authoritative "this email has been claimed" signal. It's cleared on
// GDPR account deletion so a deleted user can sign up again.

const ACCOUNT_KEY_PREFIX = 'account:';

function accountKvKey(email) {
  return ACCOUNT_KEY_PREFIX + String(email).toLowerCase().trim();
}
...
async function accountExists(email) {
  ...
  if (!e) return false;
  try {
    const marker = await kv.get(accountKvKey(e));
    if (marker) return true;
  } catch (_) { /* fall through to password check */ }
  // Defence in depth: a pre-marker password account is still an
  // existing account. This also covers the brief migration window
  // where existing users haven't been re-marked yet.
  return await hasPassword(e);
}

async function markAccountExists(email, { source } = {}) {
  const kv = require('./intelligence/kv-store');
  const e = String(email || '').toLowerCase().trim();
  if (!e) return false;
  try {
    // Idempotent — preserve the original createdAt if a marker already
    // exists so we can report account age accurately later.
    const existing = await kv.get(accountKvKey(e));
    const now = new Date().toISOString();
    const record = (existing && existing.createdAt)
      ? { ...existing, lastSignInAt: now, lastSignInSource: source || existing.lastSignInSource || null }
      : { createdAt: now, source: source || null, lastSignInAt: now, lastSignInSource: source || null };
    // No TTL — the marker is durable until the user deletes their account.
    await kv.set(accountKvKey(e), record);
    return true;
  } catch (_) { return false; }
}

async function clearAccountExists(email) {
  const kv = require('./intelligence/kv-store');
  const e = String(email || '').toLowerCase().trim();
  if (!e) return false;
  try { await kv.del(accountKvKey(e)); return true; } catch (_) { return false; }
}
...
async function createPendingSignup(email, passwordRecord, opts = {}) {
  const kv = require('./intelligence/kv-store');
  const e = String(email || '').toLowerCase().trim();
  if (!e) return null;
  const token = generateSignupToken();
  // Sprint returnto-resume-v1 — optional same-site relative URL the
  // user lands on after clicking the confirmation email. Validated by
  // isSafeReturnTo before storage so we never persist an unsafe value.
  const returnTo = isSafeReturnTo(opts.returnTo) || null;
  try {
    await kv.set(signupKvKey(token), {
      email: e,
...
// ── TOTP MFA (Sprint mfa-totp-v1) ───────────────────────
//
// Storage:
//   mfa:<email> → {
//     secretHex,                 // the TOTP secret (hex)
//     enabled: bool,             // false while enrolling, true once confirmed
//     enabledAt,                 // ISO timestamp when confirmed
//     backupCodes: [sha256hex],  // remaining (unused) recovery codes
//     algo: 'totp-sha1',
//   }
//
// Enrollment is two-step so a misconfigured authenticator can't lock a
// user out: beginMfaEnrollment writes a record with enabled=false; the
// user must submit a valid code (enableMfa) before enabled flips true.
// Until then the MFA gate at login does NOT fire (isMfaEnabled checks
// enabled, not mere presence).
//
// Backup codes: 10 single-use recovery codes generated at enable time,
// returned to the user ONCE in plaintext, stored as SHA-256 hashes.
// (No scrypt needed — the codes are high-entropy random, so a fast hash
// is fine; the slow-hash threat model is low-entropy human passwords.)
//
// Challenge tokens: after a first factor (password or magic-link) the
// session is NOT minted when MFA is enabled. Instead a short-lived
// `mfachal:<id>` → email record (5 min) is created; the user submits a
// TOTP/backup code against it to complete sign-in.

const MFA_KEY_PREFIX = 'mfa:';
const MFA_CHALLENGE_PREFIX = 'mfachal:';
const MFA_CHALLENGE_TTL_SECONDS = 5 * 60;
const MFA_BACKUP_CODE_COUNT = 10;

function mfaKvKey(email) {
  return MFA_KEY_PREFIX + String(email).toLowerCase().trim();
}
function mfaChallengeKey(id) {
  return MFA_CHALLENGE_PREFIX + String(id).toLowerCase().trim();
}

async function getMfaRecord(email) {
  const kv = require('./intelligence/kv-store');
  const e = String(email || '').toLowerCase().trim();
  if (!e) return null;
  try { return (await kv.get(mfaKvKey(e))) || null; } catch (_) { return null; }
}

async function isMfaEnabled(email) {
  const rec = await getMfaRecord(email);
  return !!(rec && rec.enabled && rec.secretHex);
}

// Step 1 of enrollment — generate a fresh secret, persist it as a
// not-yet-enabled record, and hand back the otpauth URI + base32 secret
// for the user to add to their authenticator app. Overwrites any prior
// pending (or even enabled) secret, so "re-enroll" is just calling this
// again — but note it sets enabled=false, so a re-enroll temporarily
// disables MFA until confirmed. Callers that already have MFA enabled
// should gate this behind a verified current code.
async function beginMfaEnrollment(email) {
  const kv = require('./intelligence/kv-store');
  const totp = require('./totp');
  const e = String(email || '').toLowerCase().trim();
  if (!e) return null;
  const secretHex = totp.generateSecretHex();
  const record = { secretHex, enabled: false, backupCodes: [], algo: 'totp-sha1' };
  try {
    await kv.set(mfaKvKey(e), record);
    return {
      secretHex,
      secretB32: totp.base32Encode(Buffer.from(secretHex, 'hex')),
      otpauthUri: totp.otpauthUri({ secretHex, email: e }),
    };
  } catch (_) { return null; }
}

function hashBackupCode(code) {
  const normalised = String(code || '').replace(/[\s-]/g, '').toLowerCase();
  return crypto.createHash('sha256').update(normalised).digest('hex');
}

function generateBackupCodes(n = MFA_BACKUP_CODE_COUNT) {
  const codes = [];
  for (let i = 0; i < n; i++) {
    // 8 hex chars, displayed as xxxx-xxxx for legibility.
...
// Step 2 of enrollment — verify a code against the pending secret and,
// on success, flip enabled=true + generate backup codes. Returns the
// plaintext backup codes ONCE (the caller shows them; we store hashes).
async function enableMfa(email, code) {
  const kv = require('./intelligence/kv-store');
  const totp = require('./totp');
  const e = String(email || '').toLowerCase().trim();
  if (!e) return { ok: false, reason: 'no-email' };
  const rec = await getMfaRecord(e);
  if (!rec || !rec.secretHex) return { ok: false, reason: 'no-pending-secret' };
  if (!totp.verifyTotp(rec.secretHex, code)) return { ok: false, reason: 'bad-code' };
  const plaintextCodes = generateBackupCodes();
  const updated = {
    secretHex: rec.secretHex,
    enabled: true,
    enabledAt: new Date().toISOString(),
    backupCodes: plaintextCodes.map(hashBackupCode),
    algo: 'totp-sha1',
  };
  try {
    await kv.set(mfaKvKey(e), updated);
...
async function disableMfa(email) {
  ...
  const e = String(email || '').toLowerCase().trim();
  if (!e) return false;
  try { await kv.del(mfaKvKey(e)); return true; } catch (_) { return false; }
}

// Consume a single backup code (used during a login challenge when the
// user lost their authenticator). Removes the matching hash so each
// code works exactly once. Returns true if a code matched.
async function consumeBackupCode(email, code) {
  const kv = require('./intelligence/kv-store');
  const e = String(email || '').toLowerCase().trim();
  const rec = await getMfaRecord(e);
  if (!rec || !rec.enabled || !Array.isArray(rec.backupCodes)) return false;
  const target = hashBackupCode(code);
  const idx = rec.backupCodes.indexOf(target);
  if (idx === -1) return false;
  rec.backupCodes.splice(idx, 1);
  try { await kv.set(mfaKvKey(e), rec); return true; } catch (_) { return false; }
}

// Verify a code at login: TOTP first, then fall back to a single-use
// backup code. Returns { ok, method } where method is 'totp' | 'backup'.
async function verifyMfaCode(email, code) {
  const totp = require('./totp');
  const rec = await getMfaRecord(email);
  if (!rec || !rec.enabled || !rec.secretHex) return { ok: false, reason: 'not-enabled' };
  if (totp.verifyTotp(rec.secretHex, code)) return { ok: true, method: 'totp' };
  // Backup-code path — only attempt for code shapes that look like one
  // (8 hex chars, optionally dashed), so a mistyped TOTP doesn't burn a
  // backup code.
  const normalised = String(code || '').replace(/[\s-]/g, '').toLowerCase();
  if (/^[a-f0-9]{8}$/.test(normalised)) {
    const used = await consumeBackupCode(email, code);
    if (used) return { ok: true, method: 'backup' };
  }
...
function generateMfaChallengeId() {
  return crypto.randomBytes(16).toString('hex');
}

async function createMfaChallenge(email) {
  const kv = require('./intelligence/kv-store');
  const e = String(email || '').toLowerCase().trim();
  if (!e) return null;
  const id = generateMfaChallengeId();
  try {
    await kv.set(mfaChallengeKey(id), e, { ttlSeconds: MFA_CHALLENGE_TTL_SECONDS });
    return id;
  } catch (_) { return null; }
}

// Read the challenge WITHOUT consuming — so a wrong code doesn't void
// the challenge and force a full re-auth. The challenge is deleted only
// once a correct code completes sign-in (deleteMfaChallenge).
async function peekMfaChallenge(id) {
  const kv = require('./intelligence/kv-store');
  if (!id || !/^[a-f0-9]{32}$/.test(id)) return null;
  try {
    const email = await kv.get(mfaChallengeKey(id));
    return (email && typeof email === 'string') ? email : null;
  } catch (_) { return null; }
}

async function deleteMfaChallenge(id) {
  const kv = require('./intelligence/kv-store');
  if (!id) return;
  try { await kv.del(mfaChallengeKey(id)); } catch (_) { /* best effort */ }
}
