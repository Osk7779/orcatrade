...
// ── Warehouse (PR #118 foundation + PR #119 composer + this PR's
//                email surface) ──────────────────────────────────────
//
// Parallel to the customs + sourcing + routing + finance blocks. Same
// wording discipline: describes what eligibility MEANS for the
// warehouse recommendation (EU Eurostat warehousing producer-price
// indices, 30-day snapshot freshness, regression-tested warehouse
// calculator, no manual overrides) and calls out the accuracy
// guarantee as FORTHCOMING. Note: today's warehouse-quote uses the
// OrcaTrade quarterly 3PL benchmark survey (mirror only), so TA-2
// fails and these helpers reliably return ''. When Eurostat / hub-
// published rate cards land, the surface lights up automatically.

function tierABlockWarehouseEn(verdict) {
  if (!verdict || verdict.eligible !== true) return '';
  return `\nTIER-A · UNDERWRITER-GRADE WAREHOUSE QUOTE
This warehouse recommendation cited primary-regulator sources
(EU Eurostat warehousing producer-price indices) snapshotted
within the last 30 days, was produced by our regression-tested
warehouse calculator, and carried no manual overrides.
Our liability-bearing accuracy guarantee for Tier-A calculations
launches in Q1 2027 (covered by E&O insurance, subject to binding).
Until then, Tier-A is a transparency signal you can audit, not a
financial guarantee.`;
...
function tierABlockWarehousePl(verdict) {
  if (!verdict || verdict.eligible !== true) return '';
  return `\nTIER-A · WYCENA MAGAZYNU POZIOMU UNDERWRITERSKIEGO
Ta rekomendacja magazynu została oparta wyłącznie na źródłach
regulatora pierwotnego (indeksy cen producenckich magazynowania
Eurostat), pochodzących z ostatnich 30 dni, wyprodukowana przez
nasz kalkulator magazynowy pokryty regresjami i nie zawierała
żadnych manualnych nadpisów. Nasza gwarancja poprawności
kalkulacji Tier-A (z odpowiedzialnością odszkodowawczą)
wystartuje w Q1 2027 (zabezpieczona ubezpieczeniem E&O, w trakcie
wiązania). Do tego czasu Tier-A to sygnał przejrzystości, który
...
function tierABlockWarehouseDe(verdict) {
  if (!verdict || verdict.eligible !== true) return '';
  return `\nTIER-A · UNDERWRITER-GRADE-LAGERANGEBOT
Diese Lager-Empfehlung basierte ausschließlich auf
primärregulatorischen Quellen (EU-Eurostat-Lagerhaltungs-
Produzentenpreisindizes), die innerhalb der letzten 30 Tage
gepinnt wurden, wurde von unserem regressionsgetesteten
Lagerkalkulator erstellt und enthielt keine manuellen
Übersteuerungen. Unsere haftungstragende Genauigkeitsgarantie für
Tier-A-Berechnungen startet im Q1 2027 (durch E&O-Versicherung
gedeckt, vorbehaltlich der Bindung). Bis dahin ist Tier-A ein
auditierbares Transparenzsignal — keine finanzielle Garantie.`;
}
...
${tierABlockEn(plan.customs && plan.customs.tier_a)}${tierABlockSourcingEn(plan.sourcing && plan.sourcing.tier_a)}${tierABlockRoutingEn(plan.routing && plan.routing.tier_a)}${tierABlockFinanceEn(plan.finance && plan.finance.tier_a)}${tierABlockWarehouseEn(plan.warehouse && plan.warehouse.tier_a)}
...
${tierABlockPl(plan.customs && plan.customs.tier_a)}${tierABlockSourcingPl(plan.sourcing && plan.sourcing.tier_a)}${tierABlockRoutingPl(plan.routing && plan.routing.tier_a)}${tierABlockFinancePl(plan.finance && plan.finance.tier_a)}${tierABlockWarehousePl(plan.warehouse && plan.warehouse.tier_a)}
...
${plan.warehouse?.recommendation ? `- 3PL-Hub: ${plan.warehouse.recommendation.primary}` : ''}
${tierABlockDe(plan.customs && plan.customs.tier_a)}${tierABlockSourcingDe(plan.sourcing && plan.sourcing.tier_a)}${tierABlockRoutingDe(plan.routing && plan.routing.tier_a)}${tierABlockFinanceDe(plan.finance && plan.finance.tier_a)}${tierABlockWarehouseDe(plan.warehouse && plan.warehouse.tier_a)}
...
module.exports = {
  ...
  tierABlockFinancePl,
  tierABlockFinanceDe,
  tierABlockWarehouseEn,
  tierABlockWarehousePl,
  tierABlockWarehouseDe,
};
