...
function extractSnapshot(plan) {
  ...
  const dutyRatePct = plan.customs && plan.customs.duty && Number.isFinite(Number(plan.customs.duty.ratePercent))
    ...
    : null;
  // Reproducibility-v2 3c: capture the resolved MFN rate + its source so a
  // live-TARIC plan can recompute its ORIGINAL duty later. Persisted with the
  // snapshot but NOT part of the diff (kept out of SNAPSHOT_FIELDS) so plan-delta
  // semantics are unchanged.
  const duty = (plan.customs && plan.customs.duty) || null;
  const mfnRate = duty && Number.isFinite(Number(duty.mfnRate)) ? Number(duty.mfnRate) : null;
  const mfnSource = duty && typeof duty.mfnSource === 'string' ? duty.mfnSource : null;
  return {
    asOf: plan.asOf || new Date().toISOString().slice(0, 10),
    perShipmentLandedTotal: num(t.perShipmentLandedTotal),
    // Sprint BG-0c: track P&L cost (net of recoverable VAT) alongside gross
    // landed total. Legacy snapshots without this field still diff cleanly
    // — diffSnapshots returns null effectiveLandedDelta when one side is missing.
    effectiveLandedTotal: t.effectiveLandedTotal != null ? num(t.effectiveLandedTotal) : null,
...
// Fields persisted on the saved snapshot. SNAPSHOT_FIELDS (the diffed set) plus
// reproducibility provenance that travels with the plan but isn't diffed.
const PERSIST_FIELDS = [...SNAPSHOT_FIELDS, 'mfnRate', 'mfnSource'];
...
function diffSnapshots(saved, current, savedAtIso) {
  ...
  // P&L cost delta — only defined when both sides recorded the field.
  // Legacy snapshots (pre-BG-0c) have effectiveLandedTotal=null.
  let effectiveLandedDeltaEur = null;
  let effectiveLandedDeltaPct = null;
  if (saved.effectiveLandedTotal != null && current.effectiveLandedTotal != null) {
    effectiveLandedDeltaEur = num(current.effectiveLandedTotal) - num(saved.effectiveLandedTotal);
    const savedEffective = num(saved.effectiveLandedTotal);
    effectiveLandedDeltaPct = savedEffective > 0
      ? Math.round((effectiveLandedDeltaEur / savedEffective) * 1000) / 10
      : 0;
  }
  ...
  return {
    ...
    landedDeltaPct,
    effectiveLandedDeltaEur: effectiveLandedDeltaEur != null ? Math.round(effectiveLandedDeltaEur) : null,
    effectiveLandedDeltaPct,
    primaryDriver,
