...
/**
 * Look up an active good by its customer-supplied SKU. Used by the
 * /api/start wizard for quote-time inheritance (L1.1 follow-up):
 * a returning customer types `sku: WIDGET-001` and the wizard
 * inherits hsCode + originCountry + cbamInScope from the stored row.
 *
 * Archived rows are explicitly excluded — the partial unique index on
 * (org_id, sku) WHERE archived_at IS NULL guarantees at most one match
 * for an active SKU. Re-used codes after a retire return the new row,
 * never the historical one.
 *
 * @param {{ orgId: number, sku: string }} input
 */
async function getGoodsBySku({ orgId, sku }) {
  // Validation before PG check — a 400-shaped error must not be masked
  // by the 503 fallthrough when DATABASE_URL is unset.
  if (!Number.isInteger(orgId)) return { ok: false, errors: ['orgId required (positive integer)'] };
  if (!sku || typeof sku !== 'string') return { ok: false, errors: ['sku required'] };
  if (!db.isConfigured()) return notConfigured();
  try {
    const rows = await db.query(
      `SELECT * FROM goods_master
       WHERE org_id = $1 AND sku = $2 AND archived_at IS NULL
       LIMIT 1`,
      [orgId, sku],
    );
    ...
    return failureFromDb(err instanceof Error ? err.message : 'goods lookup-by-sku failed');
  }
}
