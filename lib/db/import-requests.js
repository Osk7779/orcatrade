 ...
 *
 * @param {{ orgId: number, windowDays?: number, stallThresholdDays?: number, declineSpikeRateMultiplier?: number, supplierConcentrationThreshold?: number, ratingTrendDropThreshold?: number, quoteFollowUpThresholdDays?: number }} input
 */
async function aggregateOpsInsights({ orgId, windowDays = 30, stallThresholdDays, declineSpikeRateMultiplier, supplierConcentrationThreshold, ratingTrendDropThreshold, quoteFollowUpThresholdDays }) {
  if (!Number.isInteger(orgId)) return { ok: false, errors: ['orgId required'] };
  if (!db.isConfigured()) return notConfigured();
