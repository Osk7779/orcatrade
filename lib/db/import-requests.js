...
// Sprint 69 — aging-quotes cohort (11th cohort, 6th proactive
// signal). Sibling to sprint 38: same "stalled row" pattern but
// on the CUSTOMER side of the workflow. A quote sitting in
// 'quoted' status past QUOTE_FOLLOWUP_THRESHOLD_DAYS days without
// customer_approved/customer_declined transitioning it out is a
// follow-up gap — the customer went silent after receiving the
// quote and the ops team should nudge.
//
// Distinct from:
//   - sprint 38 (stalled awaiting_review): team hasn't quoted yet
//   - sprint 53 (quote-acceptance rate): population statistic on
//     approved vs total, not individual aging rows
// This is the individual-request view of the acceptance-side
// pipeline. Complementary to sprint 53's rate signal — one tells
// you "your acceptance rate is dropping"; this one tells you
// "here are the SPECIFIC quotes waiting for a customer nudge."
//
// 5 days = the typical B2B "no reply, resend" cadence. A customer
// who hasn't touched the quote inside a working week usually
// benefits from a follow-up. Bounded above the same way the
// stalled-queue is (CAP=10 for the top-N list; the count is the
// full org-wide total).
const QUOTE_FOLLOWUP_THRESHOLD_DAYS = 5;
const QUOTE_FOLLOWUP_CAP = 10;

// Sprint 77 — expired-quotes cohort (cohort #12 — seventh proactive
// signal). Sibling of sprint 69's aging quotes: #11 catches silent
// customers BEFORE the quote lapses; #12 catches the ones that fell
// through — quotes that hit quote_expires_at with no customer
// decision (the sprint-19 auto-expiry cron flips them to
// 'expired'). Value-weighted: the cohort sums each row's
// landed_quote totalLandedCents so ops sees "€X walked away
// unanswered in the last 30 days", not just a count. The sum is
// integer-cents arithmetic end to end (ADR-0004) — SUM of bigint
// in Postgres, never JS float math.
//
// Window is a FIXED 30 days (window-agnostic like sprints 38/69 —
// the dashboard's `days` toggle must not move this cohort: a quote
// lost 3 weeks ago is a re-quote candidate regardless of the view).
// Cap 10 for the card list, most recently expired first ("just
// lost" is the most recoverable); the count + € total are always
// org-wide over the full window.
const EXPIRED_QUOTES_WINDOW_DAYS = 30;
const EXPIRED_QUOTES_CAP = 10;
...
// Sprint 62 — rating-trend watch (cohort #10 — FIFTH proactive
// signal). Sprint 33 alerts on individual 1-2★ ratings as they
// happen; sprint 62 watches the TREND — is the current 7-day
// average drifting BELOW the prior 23-day baseline?
//
// Catches process-degradation that wouldn't fire per-event (e.g.
// a streak of 3★ ratings — none low enough for sprint 33 — but
// the avg quietly drops from 4.5★ to 3.5★ over a week). Sourcing
// issue + onboarding regression + new account-manager handoff
// all show up as trend-drift first.
//
// Disjoint window math (matches sprint 53 quote-acceptance, NOT
// sprint 40 spike's overlap pattern):
//   Current  — ratedAt >= now - 7d
//   Baseline — ratedAt < now - 7d AND ratedAt >= now - 37d
// Disjoint windows give a cleaner "current vs prior" signal —
// including current in baseline drifts the baseline toward
// current, smoothing real degradation.
//
// Flagging:
//   - currentCount >= MIN_COUNT (3) — single-digit denominator
//     turns into noise on the avg
//   - baselineAvg !== null && (baselineAvg - currentAvg) >=
//     DROP_THRESHOLD (0.5 stars) — half-star drop is the
//     smallest reliably-detectable degradation on a 1-5 scale
const RATING_TREND_CURRENT_DAYS = 7;
const RATING_TREND_BASELINE_DAYS = 30;
const RATING_TREND_MIN_COUNT = 3;
const RATING_TREND_DROP_THRESHOLD = 0.5;
...
// Sprint 81 — actual-outcome capture bounds. €1B sanity cap mirrors
// lib/actuals.js MAX_LANDED_EUR (anything above is operator error);
// 500-char notes mirrors the plan-actuals cap — the outcome note is
// context, not a thread.
const ACTUAL_MAX_LANDED_EUR = 1e9;
const ACTUAL_NOTES_MAX = 500;
...
function rowToImportRequest(r) {
  ...
  return {
    ...
    customerRating: (r.customer_rating && typeof r.customer_rating === 'object' && !Array.isArray(r.customer_rating))
      ? r.customer_rating
      : null,
    // Sprint 81 — reported actual outcome (what the customer
    // actually paid, vs the landed_quote estimate). Same
    // object-shape guard as customerRating.
    actualOutcome: (r.actual_outcome && typeof r.actual_outcome === 'object' && !Array.isArray(r.actual_outcome))
      ? r.actual_outcome
      : null,
    // Sprint 83 — first customer-visible quoted moment (SLA clock
    // stop). Null on pre-deploy rows and never-quoted requests.
    quotedAt: r.quoted_at instanceof Date ? r.quoted_at.toISOString() : (r.quoted_at || null),
    // Sprint 84 — first HUMAN ops action (review decision or ops
    // message). The first-response SLA clock stop.
    firstOpsActionAt: r.first_ops_action_at instanceof Date
      ? r.first_ops_action_at.toISOString()
      : (r.first_ops_action_at || null),
 ...
 * }} input
 */
async function listImportRequestsForOrg({ orgId, includeArchived = false, archivedOnly = false, limit = 200, status, createdByEmailHash, declineReason, q, supplierPickCountry }) {
  ...
  /** @type {any[]} */
  const params = [orgId];
  // Sprint 103 — archivedOnly is a SERVER-side cut. The previous
  // client-side filter over includeArchived was wrong at scale:
  // the LIMIT applied before the client cut, so an org with 200+
  // live rows could see an EMPTY archived view while archived
  // rows existed. archivedOnly wins over includeArchived.
  if (archivedOnly) where.push('archived_at IS NOT NULL');
  ...
  try {
    // Sprint 104 — COUNT(*) OVER() rides the same query so the
    // caller learns the TRUE match count alongside the capped
    // page. 'No silent caps' (ADR-0021 family): a list that
    // truncates must say so, and it can't say so without knowing.
    const rows = await db.query(
      `SELECT *, COUNT(*) OVER()::int AS __total_matching
         FROM import_requests WHERE ${where.join(' AND ')}
       ORDER BY updated_at DESC LIMIT ${params.length}`,
      params,
    );
    const totalMatching = rows.length > 0 ? Number(rows[0].__total_matching) || rows.length : 0;
    return { ok: true, importRequests: rows.map(rowToImportRequest), totalMatching };
  } catch (err) {
 ...
 *
 * @param {{ orgId: number, windowDays?: number, stallThresholdDays?: number, declineSpikeRateMultiplier?: number, supplierConcentrationThreshold?: number, ratingTrendDropThreshold?: number, quoteFollowUpThresholdDays?: number, slaQuoteTurnaroundTargetHours?: number, slaFirstResponseTargetHours?: number }} input
 */
async function aggregateOpsInsights({ orgId, windowDays = 30, stallThresholdDays, declineSpikeRateMultiplier, supplierConcentrationThreshold, ratingTrendDropThreshold, quoteFollowUpThresholdDays, slaQuoteTurnaroundTargetHours, slaFirstResponseTargetHours }) {
  if (!Number.isInteger(orgId)) return { ok: false, errors: ['orgId required'] };
  if (!db.isConfigured()) return notConfigured();
  ...
  // Sprint 60 — per-org supplier-concentration threshold override.
  // Same defence-in-depth pattern as sprint 42/43: defensive
  // re-bound to [0.50, 0.95] catches a corrupt cache entry that
  // bypassed write-time validation. Float-tolerant
  // (Number.isFinite + range) — the threshold carries two-decimal
  // precision so integer checks would reject 0.75.
  const candidateConcentration = Number(supplierConcentrationThreshold);
  const effectiveConcentrationThreshold = Number.isFinite(candidateConcentration) && candidateConcentration >= 0.5 && candidateConcentration <= 0.95
    ? candidateConcentration
    : SUPPLIER_CONCENTRATION_THRESHOLD;
  // Sprint 64 — per-org rating-trend drop threshold. Same
  // defence-in-depth pattern as sprints 42/43/60: defensive
  // re-bound to [0.2, 2.0] catches a corrupt cache entry that
  // bypassed write-time validation. Float-tolerant
  // (Number.isFinite + range) — the threshold carries
  // one-decimal precision.
  const candidateRatingDrop = Number(ratingTrendDropThreshold);
  const effectiveRatingDropThreshold = Number.isFinite(candidateRatingDrop) && candidateRatingDrop >= 0.2 && candidateRatingDrop <= 2.0
    ? candidateRatingDrop
    : RATING_TREND_DROP_THRESHOLD;
  // Sprint 71 — per-org aging-quotes threshold. Same defence-in-
  // depth pattern as sprints 42/43/60/64: defensive re-bound to
  // [1, 30] catches a corrupt cache entry that bypassed write-
  // time validation. Integer knob mirroring sprint 42's stall
  // shape — Number.isInteger catches undefined→NaN cleanly and
  // lets TS narrow the value under strict null checks.
  const candidateQuoteFollowUp = Number(quoteFollowUpThresholdDays);
  const effectiveQuoteFollowUpThreshold = Number.isInteger(candidateQuoteFollowUp) && candidateQuoteFollowUp >= 1 && candidateQuoteFollowUp <= 30
    ? candidateQuoteFollowUp
    : QUOTE_FOLLOWUP_THRESHOLD_DAYS;
  ...
  try {
    ...
    // ── 6b. Aging quotes (sprint 69) ──
    // Cohort #11, sixth proactive signal. Sibling to sprint 38:
    // same "no activity past N days" pattern but on the CUSTOMER
    // side of the workflow (status='quoted' waiting for
    // customer_approved / customer_declined). Complements sprint
    // 53's rate-of-acceptance cohort — that tells ops "your
    // conversion is degrading"; this tells ops "here are the
    // SPECIFIC quotes to nudge."
    //
    // Same architecture as sprint 38: separate COUNT(*) so the
    // headline number isn't capped at QUOTE_FOLLOWUP_CAP + the
    // LIMITed list for the card. Sort by updated_at ASC (oldest
    // first) because "waiting 21 days" is more urgent than "just
    // crossed the 5-day line."
    //
    // Window-agnostic — a quote that went silent 3 weeks ago is
    // still worth chasing regardless of the dashboard's `days`
    // toggle.
    const agingQuoteCountRows = await db.query(
      `SELECT COUNT(*)::int AS n
         FROM import_requests
        WHERE org_id = $1
          AND archived_at IS NULL
          AND status = 'quoted'
          AND updated_at < now() - ($2 || ' days')::interval`,
      [orgId, String(effectiveQuoteFollowUpThreshold)],
    );
    const agingQuoteCount = Number(
      (agingQuoteCountRows[0] && agingQuoteCountRows[0].n) || 0,
    );
    /** @type {Array<{ externalId: string, label: string, updatedAt: string, daysPending: number }>} */
    let agingQuoteItems = [];
    if (agingQuoteCount > 0) {
      const agingQuoteListRows = await db.query(
        `SELECT external_id, label, updated_at,
                EXTRACT(EPOCH FROM (now() - updated_at)) / 86400 AS days_pending
           FROM import_requests
          WHERE org_id = $1
            AND archived_at IS NULL
            AND status = 'quoted'
          ...
          ORDER BY updated_at ASC
          LIMIT $3`,
        [orgId, String(effectiveQuoteFollowUpThreshold), QUOTE_FOLLOWUP_CAP],
      );
      agingQuoteItems = agingQuoteListRows.map(/** @param {any} r */ (r) => ({
        externalId: String(r.external_id),
        label: String(r.label || ''),
        updatedAt: r.updated_at instanceof Date ? r.updated_at.toISOString() : String(r.updated_at),
        // Server-side rounding — one decimal, mirrors the sprint-38
        // stalled-queue daysStalled precision so the UI reads
        // consistently across both cohorts.
        daysPending: Math.round(Number(r.days_pending) * 10) / 10,
      }));
    }

    // ── 6c. Expired quotes (sprint 77) ──
    // Cohort #12, seventh proactive signal. The lost-deal mirror of
    // 6b: quotes that hit quote_expires_at undecided (sprint-19
    // auto-expiry flipped them to 'expired') within the last
    // EXPIRED_QUOTES_WINDOW_DAYS. Value-weighted — the € total is
    // the headline ("€X walked away unanswered"), the list is the
    // re-quote worklist.
    //
    // Window-agnostic like 6a/6b — uses the CONSTANT, never the
    // dashboard `days` var. quote_expires_at (not updated_at) is
    // the recency dimension: it marks the moment the quote lapsed,
    // and a later archive/note touching updated_at must not move a
    // row in or out of the cohort.
    //
    // Money: SUM of (landed_quote->>'totalLandedCents')::bigint in
    // Postgres — integer-cents end to end (ADR-0004), no JS float
    // arithmetic. Neon returns bigint as string; Number() is safe
    // far past any realistic org total (< 2^53 cents ≈ €90T).
    const expiredQuoteRows = await db.query(
      `SELECT COUNT(*)::int AS n,
        ...
        WHERE org_id = $1
          AND archived_at IS NULL
          AND status = 'expired'
          AND quote_expires_at >= now() - ($2 || ' days')::interval`,
      [orgId, String(EXPIRED_QUOTES_WINDOW_DAYS)],
    );
    const expiredQuoteCount = Number(
      (expiredQuoteRows[0] && expiredQuoteRows[0].n) || 0,
    );
    const expiredQuoteTotalCents = Number(
      (expiredQuoteRows[0] && expiredQuoteRows[0].total_cents) || 0,
    );
    /** @type {Array<{ externalId: string, label: string, expiredAt: string, landedCents: number | null }>} */
    let expiredQuoteItems = [];
    if (expiredQuoteCount > 0) {
      const expiredQuoteListRows = await db.query(
        `SELECT external_id, label, quote_expires_at,
                (landed_quote->>'totalLandedCents')::bigint AS landed_cents
           FROM import_requests
          WHERE org_id = $1
            AND archived_at IS NULL
            AND status = 'expired'
            AND quote_expires_at >= now() - ($2 || ' days')::interval
          ORDER BY quote_expires_at DESC
          LIMIT $3`,
        [orgId, String(EXPIRED_QUOTES_WINDOW_DAYS), EXPIRED_QUOTES_CAP],
      );
      expiredQuoteItems = expiredQuoteListRows.map(/** @param {any} r */ (r) => ({
        externalId: String(r.external_id),
        label: String(r.label || ''),
        expiredAt: r.quote_expires_at instanceof Date
          ? r.quote_expires_at.toISOString()
          : String(r.quote_expires_at),
        // Defensive null — a quoted→expired row always carries a
        // landed quote in practice, but a historical row without
        // one must render as "—", never as €0 (a real €0 quote
        // and a missing quote are different truths).
        landedCents: (r.landed_cents === null || r.landed_cents === undefined)
          ? null
          : Number(r.landed_cents),
      }));
    }

    // ── 6d. Per-org quote accuracy (sprint 82 — Track B phase 3) ──
    // The org's own slice of the public Quote Accuracy Ledger:
    // THEIR reported outcomes vs THEIR quotes, scored by the same
    // calculator with the same honesty gates (metrics withheld
    // below 10 scoreable outcomes — a customer's view must be as
    // incapable of flattery as the public one). Window-agnostic:
    // accuracy is cumulative, not a dashboard-toggle cohort.
    const accuracyLedgerCalc = require('../intelligence/accuracy-ledger');
    const orgOutcomeRows = await listActualOutcomesForLedger({ orgId });
    const accuracyLedger = accuracyLedgerCalc.computeAccuracyLedger(orgOutcomeRows);

    // ── 6e. Quote-turnaround SLA (sprint 83 — Track C phase 1) ──
    // Measured attainment against the 48h target over the rolling
    // 90-day window, computed by the deterministic SLA calculator
    // (shared honesty gates with the accuracy ledger — one
    // definition of "enough data" across every trust instrument).
    // Attainment accrues from the schema-023 deploy: pre-deploy
    // rows have no quoted_at and truthfully stay out of the sample.
    const slaCalc = require('../intelligence/sla');
    // Sprint 89 — per-org negotiated target. Same defence-in-depth
    // re-bound pattern as sprints 42/43/60/64/71: write-time
    // validation enforces [4, 168], this is the deepest layer.
    const candidateTurnTarget = Number(slaQuoteTurnaroundTargetHours);
    const effectiveTurnTarget = Number.isInteger(candidateTurnTarget) && candidateTurnTarget >= 4 && candidateTurnTarget <= 168
      ? candidateTurnTarget
      : slaCalc.SLA_QUOTE_TURNAROUND_TARGET_HOURS;
    const slaRows = await listQuoteTurnaroundsForSla({
      orgId,
      windowDays: slaCalc.SLA_WINDOW_DAYS,
    });
    const slaQuoteTurnaround = {
      windowDays: slaCalc.SLA_WINDOW_DAYS,
      ...slaCalc.computeSlaAttainment(slaRows, {
        targetHours: effectiveTurnTarget,
      }),
    };

    // ── 6f. First-response SLA (sprint 84 — Track C phase 2) ──
    // Same calculator, same window, same shared honesty gates —
    // the clock pair here is created → first HUMAN ops action.
    // Sprint 89 — per-org negotiated target, re-bound to [1, 72].
    const candidateFrTarget = Number(slaFirstResponseTargetHours);
    const effectiveFrTarget = Number.isInteger(candidateFrTarget) && candidateFrTarget >= 1 && candidateFrTarget <= 72
      ? candidateFrTarget
      : slaCalc.SLA_FIRST_RESPONSE_TARGET_HOURS;
    const firstResponseRows = await listFirstResponsesForSla({
      orgId,
      windowDays: slaCalc.SLA_WINDOW_DAYS,
    });
    const slaFirstResponse = {
      windowDays: slaCalc.SLA_WINDOW_DAYS,
      ...slaCalc.computeSlaAttainment(firstResponseRows, {
        targetHours: effectiveFrTarget,
      }),
    };

    // ── 6g. SLA-at-risk cohort (sprint 91 — cohort #13) ──
    // Measurement → PREVENTION: the attainment blocks above report
    // the past; this cohort lists the live requests about to break
    // the promise. Uses the ORG'S NEGOTIATED target (knob 6) — the
    // alert line follows the contract, unlike the cross-org triage
    // console which stays on the platform line by design.
    //
    // Two truths, split honestly:
    //   atRiskCount   — unquoted, past 75% of the budget, not yet
    //                   over it (recoverable: act now)
    //   breachedCount — unquoted AND past the full target (the
    //                   promise is already broken; still listed
    //                   because the customer is still waiting)
    // items = the top 10 oldest across BOTH buckets; hoursRemaining
    // goes negative on breached rows (the UI renders the sign, it
    // never recomputes). Window-agnostic; archived rows out
    // (worklist semantics).
    const slaRiskLineHours = Math.round(effectiveTurnTarget * slaCalc.SLA_RISK_FRACTION);
    const slaAtRiskRows = await db.query(
      `SELECT external_id, label, status,
              EXTRACT(EPOCH FROM (now() - created_at)) / 3600 AS age_hours
         FROM import_requests
        WHERE org_id = $1
          AND archived_at IS NULL
          AND quoted_at IS NULL
          AND status IN ('submitted', 'processing', 'awaiting_review')
          AND created_at < now() - ($2 || ' hours')::interval
        ORDER BY created_at ASC
        LIMIT 10`,
      [orgId, String(slaRiskLineHours)],
    );
    const slaAtRiskCountRows = await db.query(
      `SELECT COUNT(*) FILTER (
        ...
        WHERE org_id = $1
          ...
          AND status IN ('submitted', 'processing', 'awaiting_review')
          AND created_at < now() - ($2 || ' hours')::interval`,
      [orgId, String(slaRiskLineHours), String(effectiveTurnTarget)],
    );
    const slaAtRisk = {
      riskThresholdHours: slaRiskLineHours,
      targetHours: effectiveTurnTarget,
      atRiskCount: Number((slaAtRiskCountRows[0] && slaAtRiskCountRows[0].at_risk) || 0),
      breachedCount: Number((slaAtRiskCountRows[0] && slaAtRiskCountRows[0].breached) || 0),
      items: slaAtRiskRows.map(/** @param {any} r */ (r) => ({
        externalId: String(r.external_id),
        label: String(r.label || ''),
        status: String(r.status),
        ageHours: Math.round(Number(r.age_hours) * 10) / 10,
        // Negative = already breached. One derivation, server-side.
    ...
    const isConcentrated = (
      ...
      && concentrationShare >= effectiveConcentrationThreshold
    );

    // ── 10. Rating-trend watch (sprint 62) ──
    // Fifth proactive cohort. Current 7-day avg vs prior 23-day
    // baseline. Disjoint windows (matches sprint-53 quote-
    // acceptance) so the baseline doesn't drift toward current.
    //
    // Two queries, mirrored shape:
    //   Current  — customer_rating->>ratedAt >= now - 7d
    //   Baseline — customer_rating->>ratedAt in [now-37d, now-7d]
    // Both filter to customer_approved + non-null customer_rating
    // (same gate as sprint-31 ratingCohort).
    const ratingTrendCurrentRows = await db.query(
      `SELECT (customer_rating->>'score')::int AS score
         FROM import_requests
        WHERE org_id = $1
          AND archived_at IS NULL
          ...
          AND status = 'customer_approved'
          AND (customer_rating->>'ratedAt')::timestamptz >= now() - ($2 || ' days')::interval`,
      [orgId, String(RATING_TREND_CURRENT_DAYS)],
    );
    const ratingTrendBaselineRows = await db.query(
      `SELECT (customer_rating->>'score')::int AS score
         FROM import_requests
        WHERE org_id = $1
          AND archived_at IS NULL
          ...
          AND status = 'customer_approved'
          AND (customer_rating->>'ratedAt')::timestamptz < now() - ($2 || ' days')::interval
          AND (customer_rating->>'ratedAt')::timestamptz >= now() - (($2::int + $3::int) || ' days')::interval`,
      [orgId, String(RATING_TREND_CURRENT_DAYS), String(RATING_TREND_BASELINE_DAYS)],
    );
    /** @param {any[]} rows */
    function computeRatingTrendAverage(rows) {
      let sum = 0;
      let count = 0;
      for (const r of rows) {
      ...
      // Server-side rounding: one decimal (matches sprint-31
      // ratingCohort precision). Null when no valid scores so
      // the UI can render "no baseline yet" rather than 0★.
      const avg = count > 0 ? Math.round((sum / count) * 10) / 10 : null;
      return { count, avg };
    }
    const ratingTrendCurrent = computeRatingTrendAverage(ratingTrendCurrentRows);
    const ratingTrendBaseline = computeRatingTrendAverage(ratingTrendBaselineRows);
    // delta = baselineAvg - currentAvg (positive when current
    // dropped). Null when either side is null.
    const ratingTrendDelta = (ratingTrendCurrent.avg !== null && ratingTrendBaseline.avg !== null)
      ? Math.round((ratingTrendBaseline.avg - ratingTrendCurrent.avg) * 10) / 10
      : null;
    const isRatingDeclining = (
      ratingTrendCurrent.count >= RATING_TREND_MIN_COUNT
      && ratingTrendBaseline.avg !== null
      && ratingTrendCurrent.avg !== null
    ...
    return {
      ...
      insights: {
        ...
        // Sprint 69 — cohort #11, sixth proactive signal. Aging
        // quotes waiting on customer decision past QUOTE_FOLLOWUP_
        // THRESHOLD_DAYS. count is org-wide (not capped); items is
        // the top QUOTE_FOLLOWUP_CAP, oldest first. thresholdDays
        // surfaced so the UI can name the follow-up gate exactly.
        quoteFollowUp: {
          // Sprint 71 — surface the EFFECTIVE threshold (org config
          // applied) so the UI + email composers render the actual
          // value the SQL queried, not the static default. Same
          // discipline as sprints 42/43/60/64 for the other four
          // knobs.
          thresholdDays: effectiveQuoteFollowUpThreshold,
          count: agingQuoteCount,
          items: agingQuoteItems,
        },
        // Sprint 77 — cohort #12, seventh proactive signal. Quotes
        // that expired undecided in the last windowDays. count +
        // totalLandedCents are org-wide over the full window
        // (integer cents, ADR-0004); items is the top
        // EXPIRED_QUOTES_CAP, most recently expired first.
        expiredQuotes: {
          windowDays: EXPIRED_QUOTES_WINDOW_DAYS,
          count: expiredQuoteCount,
          totalLandedCents: expiredQuoteTotalCents,
          items: expiredQuoteItems,
        },
        // Sprint 82 — the org's own quote-accuracy slice (same
        // calculator + honesty gates as the public /api/accuracy
        // ledger). sampleSize 0 until the org reports outcomes.
        accuracyLedger,
        // Sprint 83 — measured quote-turnaround SLA attainment
        // (48h target, rolling 90-day window, shared honesty
        // gates). Accrues from the quoted_at deploy.
        slaQuoteTurnaround,
        // Sprint 84 — first-response SLA (24h to the first HUMAN
        // ops action; automated transitions never stop the clock).
        slaFirstResponse,
        // Sprint 91 — cohort #13. Live requests approaching (or
        // past) the org's NEGOTIATED turnaround target. The UI
        // gates the card on atRiskCount + breachedCount > 0.
        slaAtRisk,
        ...
        supplierConcentration: {
          ...
          // Sprint 60 — surface the EFFECTIVE threshold (after
          // per-org config + defensive re-bound). UI + email read
          // this so they render what the classifier actually used.
          threshold: effectiveConcentrationThreshold,
        ...
        // Sprint 62 — cohort #10. Fifth proactive signal.
        // Rating-trend drift. Always present; the UI gates the
        // card render on isDeclining.
        ratingTrend: {
          currentDays: RATING_TREND_CURRENT_DAYS,
          baselineDays: RATING_TREND_BASELINE_DAYS,
          minCount: RATING_TREND_MIN_COUNT,
          // Sprint 64 — surface the EFFECTIVE threshold (after
          // per-org config + defensive re-bound). UI + email read
          // this so they render what the classifier actually used.
          dropThreshold: effectiveRatingDropThreshold,
          currentCount: ratingTrendCurrent.count,
          currentAvg: ratingTrendCurrent.avg,
          baselineCount: ratingTrendBaseline.count,
          baselineAvg: ratingTrendBaseline.avg,
          delta: ratingTrendDelta,
          isDeclining: isRatingDeclining,
        },
      },
    };
  } catch (err) {
...
// ── Bulk archive (sprint 74) ─────────────────────────────────────────

/**
 * Bulk version of archiveImportRequest — the list-page counterpart
 * of sprint-20's bulk-review. Wraps the existing per-row archive
 * path so every invariant (org-scoped WHERE, audit-before-success,
 * webhook dispatch per row via events.record, already-archived
 * idempotence) is preserved verbatim.
 *
 * Same 50-row soft cap + Set dedup as bulkAttachTeamReview — an
 * accidental "select all" on a big list should require a
 * deliberate second pass, and a double-submit must not fan out
 * duplicate audit events.
 *
 * The batch report separates three outcomes (SAP-GTS batch-log
 * posture — a truthful per-item report, never a collapsed "ok"):
 *   - archived:  newly archived this call
 *   - unchanged: already archived before the call (idempotent
 *                success, NOT an error — but reported distinctly
 *                so "archived 8, 2 already archived" reads true)
 *   - failed:    per-row isolation — one row failing (not found,
 *                db error) MUST NOT roll back the rest
 *
 * @param {{ orgId: number, externalIds: string[], actorEmailHash: string }} input
 */
async function bulkArchiveImportRequests({ orgId, externalIds, actorEmailHash }) {
  if (!Number.isInteger(orgId)) {
    return { ok: false, errors: ['orgId required'] };
  }
  if (!Array.isArray(externalIds) || externalIds.length === 0) {
  ...
  if (externalIds.length > 50) {
    return { ok: false, errors: ['bulk-archive capped at 50 requests per call'] };
  }
  // Fail fast at the batch level — a not-configured DB would
  // otherwise surface as N identical per-row failures, which the
  // handler can't distinguish from real partial failure (it must
  // 503, not 200-with-a-failure-list).
  if (!db.isConfigured()) return notConfigured();
  const ids = [...new Set(externalIds.map(String))];
  /** @type {Array<{ externalId: string, unchanged: boolean }>} */
  const succeeded = [];
  /** @type {Array<{ externalId: string, error: string, notFound?: boolean }>} */
  const failed = [];

  for (const externalId of ids) {
    const result = await archiveImportRequest({ orgId, externalId, actorEmailHash });
    if (result.ok) {
      const win = /** @type {any} */ (result);
      succeeded.push({ externalId, unchanged: win.unchanged === true });
    } else {
      const fail = /** @type {any} */ (result);
      failed.push({
        externalId,
  ...
  return {
    ok: true,
    archivedCount: succeeded.filter((s) => !s.unchanged).length,
    unchangedCount: succeeded.filter((s) => s.unchanged).length,
    failedCount: failed.length,
    succeeded,
    failed,
  };
  ...
  try {
    const rows = await db.query(
      `UPDATE import_requests
       ...
       RETURNING *`,
      [JSON.stringify([message]), orgId, externalId, role],
    );
    if (rows.length === 0) {
...
/**
 * Sprint 61 — edit an existing internal ops note.
 *
 * OWNER-ONLY RBAC: only the note's original author (matched via
 * byEmailHash === actorEmailHash) can edit. Admins can write
 * internal notes but cannot rewrite another ops member's note —
 * prevents silent revisionism on the per-request side-channel.
 * Audit-logged before success.
 *
 * @param {{ orgId: number, externalId: string, noteId: string, actorEmailHash: string, body: string }} input
 */
async function editInternalNote({ orgId, externalId, noteId, actorEmailHash, body }) {
  if (!Number.isInteger(orgId) || !externalId || !noteId || !actorEmailHash) {
    return { ok: false, errors: ['orgId, externalId, noteId, actorEmailHash required'] };
  }
  if (typeof body !== 'string') {
    return { ok: false, errors: ['body must be a string'] };
  }
  ...
  const beforeRow = /** @type {any} */ (before).importRequest;
  const existing = Array.isArray(beforeRow.internalNotes) ? beforeRow.internalNotes : [];
  const idx = existing.findIndex(/** @param {any} n */ (n) => n && n.id === noteId);
  if (idx === -1) {
    return { ok: false, notFound: true, errors: ['note not found'] };
  }
  const current = existing[idx];
  // Soft-deleted notes can't be edited (preserves the deletion
  // intent; if you want it back, restoration is its own action
  // a future sprint can add).
  if (current.deletedAt) {
    return { ok: false, conflict: true, errors: ['note is deleted and cannot be edited'] };
  }
  // OWNER-ONLY gate — drift-guard pinned. byEmailHash comparison
  // is constant-time-equivalent (both 16-hex strings); no timing
  // leak concern at this hash length.
  if (current.byEmailHash !== actorEmailHash) {
    return { ok: false, forbidden: true, errors: ['only the note author can edit this note'] };
  }
  // No-op short-circuit — editing to the same body is a non-event;
  // doesn't audit-log, doesn't bump editedAt. Mirrors sprint-51
  // reactivateWebhook noOp posture.
  if (current.body === trimmed) {
    return { ok: true, importRequest: beforeRow, note: current, noOp: true };
  }

  // Apply the edit in-process + rewrite the array. JSONB doesn't
  // give us a clean array-item-update primitive at the SQL level,
  // so we replace the whole array — same posture as the sprint-30
  // customer_rating supersession path.
  const updatedNote = {
    ...current,
    body: trimmed,
  ...
  nextArr[idx] = updatedNote;

  try {
    const rows = await db.query(
      `UPDATE import_requests
         SET internal_notes = $1::jsonb,
             updated_at = now()
       WHERE org_id = $2 AND external_id = $3
       RETURNING *`,
      [JSON.stringify(nextArr), orgId, externalId],
    );
    if (rows.length === 0) {
      return { ok: false, notFound: true, errors: ['Not found'] };
    }
    const importRequest = rowToImportRequest(rows[0]);
    // Audit-log BEFORE return (ADR-0005). Detail records the
    // note id + new length only — NOT the body (privacy posture
    // matches sprint-55 append).
    await events.record('import_request_internal_note_edited', {
      orgId,
      actorEmailHash,
      entityType: 'import_request',
      entityId: externalId,
      detail: { noteId, length: trimmed.length },
    });
    return { ok: true, importRequest, note: updatedNote };
  } catch (err) {
    return failureFromDb(err instanceof Error ? err.message : 'edit internal note failed');
  }
}

/**
 * Sprint 61 — soft-delete an existing internal ops note.
 *
 * OWNER-ONLY RBAC (same gate as editInternalNote). Sets deletedAt
 * + deletedByEmailHash on the note; the row stays in the JSONB
 * array for audit-history reconstruction. The handler-side GET
 * projection + UI render filter out deleted entries from the
 * visible list — a future "restore" path can flip deletedAt back
 * to null if needed.
 *
 * @param {{ orgId: number, externalId: string, noteId: string, actorEmailHash: string }} input
 */
async function deleteInternalNote({ orgId, externalId, noteId, actorEmailHash }) {
  if (!Number.isInteger(orgId) || !externalId || !noteId || !actorEmailHash) {
    return { ok: false, errors: ['orgId, externalId, noteId, actorEmailHash required'] };
  }
  if (!db.isConfigured()) return notConfigured();
  ...
  const current = existing[idx];
  if (current.byEmailHash !== actorEmailHash) {
    return { ok: false, forbidden: true, errors: ['only the note author can delete this note'] };
  }
  // Idempotent — re-delete returns noOp. Audit chain captures the
  // first deletion; a re-delete is a no-op.
  if (current.deletedAt) {
    return { ok: true, importRequest: beforeRow, note: current, noOp: true };
  }
  ...
  const updatedNote = {
    ...
    deletedByEmailHash: actorEmailHash,
  };
  const nextArr = existing.slice();
  nextArr[idx] = updatedNote;
  ...
  try {
    ...
    const importRequest = rowToImportRequest(rows[0]);
    await events.record('import_request_internal_note_deleted', {
      orgId,
      actorEmailHash,
      entityType: 'import_request',
      entityId: externalId,
    ...
    return failureFromDb(err instanceof Error ? err.message : 'delete internal note failed');
  }
}
...
// ── Actual outcome (sprint 81 — Track B phase 2) ────────────────────

/**
 * Record (or supersede) the actual landed outcome on an approved
 * request — what the customer really paid, vs the landed_quote
 * estimate. The wedge's half of the Quote Accuracy Ledger loop
 * (saved plans got theirs in BG-1.4; /api/accuracy merges both).
 *
 * Mirrors recordCustomerRating's shape: status-gated on
 * customer_approved (earlier statuses have no fulfilled outcome;
 * terminal failures aren't an outcome of OUR quote), last-write-
 * wins with the supersession preserved in the audit chain,
 * same-org WHERE (cross-org → notFound).
 *
 * Money: landedEur converts to integer cents AT THIS BOUNDARY
 * (ADR 0004); the variance vs the quote is integer-cent
 * subtraction with a 1dp display percentage. Notes stay on the
 * row, never in the audit chain head (privacy + locality — the
 * sprint-30 posture).
 *
 * Unlike ratings (creator-only), BOTH the customer and ops can
 * report — ops often learns the outcome first from carrier +
 * customs settlement. The handler enforces org membership; no
 * narrower gate is correct here.
 *
 * @param {{
 *   orgId: number,
 *   externalId: string,
 ...
 * }} input
 */
async function recordImportRequestActual({ orgId, externalId, actorEmailHash, landedEur, notes }) {
  if (!Number.isInteger(orgId) || !externalId || !actorEmailHash) {
    return { ok: false, errors: ['orgId, externalId, actorEmailHash required'] };
  }
  const eur = Number(landedEur);
  if (!Number.isFinite(eur) || eur <= 0 || eur > ACTUAL_MAX_LANDED_EUR) {
    return { ok: false, errors: ['landedEur must be a positive number ≤ 1,000,000,000'] };
  }
  let trimmedNotes = '';
  if (notes != null) {
    if (typeof notes !== 'string') return { ok: false, errors: ['notes must be a string'] };
    trimmedNotes = notes.trim();
    if (trimmedNotes.length > ACTUAL_NOTES_MAX) {
      return { ok: false, errors: [`notes must be <= ${ACTUAL_NOTES_MAX} chars`] };
    }
  }
  if (!db.isConfigured()) return notConfigured();
  ...
  if (beforeRow.status !== 'customer_approved') {
    return {
      ok: false,
      conflict: true,
      errors: [`recordImportRequestActual requires status='customer_approved', got '${beforeRow.status}'`],
    };
  }

  const actual = {
    // Integer cents at the boundary — every downstream comparison
  ...
  const isSupersession = Boolean(
    beforeRow.actualOutcome
    && typeof beforeRow.actualOutcome === 'object'
    && Number.isFinite(beforeRow.actualOutcome.landedCents),
  );
  // Variance vs the quote — integer-cent delta; pct is 1dp display.
  // null when the row carries no scoreable quote (historical rows).
  const quoteCents = (beforeRow.landedQuote && Number.isFinite(Number(beforeRow.landedQuote.totalLandedCents)))
    ? Number(beforeRow.landedQuote.totalLandedCents)
    : null;
  const variance = (quoteCents && quoteCents > 0)
    ? {
      quoteCents,
      actualCents: actual.landedCents,
      deltaCents: actual.landedCents - quoteCents,
      deltaPct: Math.round(((actual.landedCents - quoteCents) / quoteCents) * 1000) / 10,
    }
    : null;
  ...
  try {
    const rows = await db.query(
      `UPDATE import_requests
       ...
       WHERE org_id = $2 AND external_id = $3
       RETURNING *`,
      [JSON.stringify(actual), orgId, externalId],
    );
    if (rows.length === 0) {
      return { ok: false, notFound: true, errors: ['Not found'] };
    }
    const importRequest = rowToImportRequest(rows[0]);
    // Audit-log BEFORE returning success (ADR 0005). Detail carries
    // the deterministic numbers (cents + 1dp pct) — notes stay on
    // the row.
    await events.record('import_request_actual_reported', {
      orgId,
      actorEmailHash,
      entityType: 'import_request',
      entityId: externalId,
      detail: {
        landedCents: actual.landedCents,
        hasNotes: trimmedNotes.length > 0,
        isSupersession,
        ...(variance !== null ? { variance } : {}),
      },
    });
    return { ok: true, importRequest, actual, variance, isSupersession };
  } catch (err) {
    return failureFromDb(err instanceof Error ? err.message : 'record actual outcome failed');
  }
}

/**
 * The wedge's corpus cut for the Quote Accuracy Ledger. Rows that
 * carry BOTH a reported actual and a scoreable quote, mapped to the
 * ledger-row shape (integer estimateCents — no float round-trip;
 * the calculator prefers estimateCents over the legacy EUR-float
 * snapshot path).
 *
 * Sprint 82 — optional orgId narrows to ONE org's outcomes (the
 * per-org accuracy view on /imports/insights; rides the schema-022
 * partial index). Omitted → platform-wide (the public ledger).
 *
 * @param {{ limit?: number, orgId?: number }} opts
 */
async function listActualOutcomesForLedger({ limit = 10000, orgId } = {}) {
  if (!db.isConfigured()) return [];
  const cap = Math.max(1, Math.min(10000, Number(limit) || 10000));
  const orgScoped = Number.isInteger(orgId);
  try {
    const rows = await db.query(
      `SELECT (actual_outcome->>'landedCents')::bigint  AS landed_cents,
              (landed_quote->>'totalLandedCents')::bigint AS estimate_cents,
              actual_outcome->>'reportedAt'             AS reported_at
         FROM import_requests
        WHERE actual_outcome IS NOT NULL
          AND landed_quote->>'totalLandedCents' IS NOT NULL
          ${orgScoped ? 'AND org_id = $2' : ''}
        LIMIT $1`,
      orgScoped ? [cap, orgId] : [cap],
    );
    return rows.map(/** @param {any} r */ (r) => ({
      landedCents: Number(r.landed_cents),
      estimateCents: Number(r.estimate_cents),
...
/**
 * Sprint 83 — the SLA cut: quoted rows in the rolling window, with
 * the two clock stamps. quoted_at (not the dashboard days toggle)
 * bounds the window — attainment is a commitment track record, not
 * a view. Archived rows stay IN: archiving a slow quote must not
 * launder the SLA. Rides the schema-023 partial index.
 *
 * @param {{ orgId?: number, windowDays?: number }} opts
 */
async function listQuoteTurnaroundsForSla({ orgId, windowDays = 90 } = {}) {
  if (!db.isConfigured()) return [];
  const days = Math.max(1, Math.min(365, Number(windowDays) || 90));
  // Sprint 85 — orgId now optional: omitted → PLATFORM-WIDE (the
  // public /api/sla attainment); integer → one org's slice (the
  // insights cockpit). Same dual-scope posture as the accuracy
  // ledger's corpus reader.
  const orgScoped = Number.isInteger(orgId);
  try {
    const rows = await db.query(
      `SELECT created_at, quoted_at
         FROM import_requests
        WHERE quoted_at IS NOT NULL
          AND quoted_at >= now() - ($1 || ' days')::interval
          ${orgScoped ? 'AND org_id = $2' : ''}`,
      orgScoped ? [String(days), orgId] : [String(days)],
    );
    return rows.map(/** @param {any} r */ (r) => ({
      createdAt: r.created_at instanceof Date ? r.created_at.toISOString() : String(r.created_at),
      quotedAt: r.quoted_at instanceof Date ? r.quoted_at.toISOString() : String(r.quoted_at),
    }));
  } catch (_) {
    // Fail-open — the SLA card degrades to the accruing state.
    return [];
  }
}

/**
 * Sprint 84 — the first-response SLA cut. Rows whose first HUMAN
 * ops action landed in the window, mapped to the generic clock-
 * pair shape the SLA calculator scores ({ createdAt, quotedAt } —
 * "quotedAt" is the calculator's clock-stop field name, here
 * carrying first_ops_action_at). Archived rows stay IN, same as
 * the turnaround cut. Rides the schema-024 partial index.
 *
 * @param {{ orgId?: number, windowDays?: number }} opts
 */
async function listFirstResponsesForSla({ orgId, windowDays = 90 } = {}) {
  if (!db.isConfigured()) return [];
  const days = Math.max(1, Math.min(365, Number(windowDays) || 90));
  // Sprint 85 — same dual-scope posture as the turnaround cut.
  const orgScoped = Number.isInteger(orgId);
  try {
    const rows = await db.query(
      `SELECT created_at, first_ops_action_at
         FROM import_requests
        WHERE first_ops_action_at IS NOT NULL
          AND first_ops_action_at >= now() - ($1 || ' days')::interval
          ${orgScoped ? 'AND org_id = $2' : ''}`,
      orgScoped ? [String(days), orgId] : [String(days)],
    );
    return rows.map(/** @param {any} r */ (r) => ({
      createdAt: r.created_at instanceof Date ? r.created_at.toISOString() : String(r.created_at),
      quotedAt: r.first_ops_action_at instanceof Date
        ? r.first_ops_action_at.toISOString()
        : String(r.first_ops_action_at),
    }));
  } catch (_) {
    // Fail-open — the SLA card degrades to the accruing state.
    return [];
...
// ── Cross-org operator triage (sprint 87 — Track E) ─────────────────

/**
 * One query, the whole book of business: per-org open-work counts
 * ranked by urgency so one operator runs every org from a single
 * screen. Admin-gated at the handler (this is a PLATFORM-STAFF
 * surface — the only reader that may cross org boundaries).
 *
 * Ranking is deterministic and IN SQL (no LLM near it, ADR 0002):
 *   1. slaRisk desc        — unquoted requests past 75% of the 48h
 *                            budget: minutes matter
 *   2. value at stake desc — open quoted € (the sprint-77 lesson:
 *                            weight by money, not row count)
 *   3. awaitingReview desc — the team-side queue
 *
 * Archived rows excluded (they're out of the working set — unlike
 * the SLA attainment cut, this is a WORKLIST, not a track record).
 *
 * @param {{ slaRiskHours: number }} opts
 */
async function aggregateOperatorTriage({ slaRiskHours }) {
  if (!db.isConfigured()) return { ok: false, errors: ['db not configured'] };
  const riskHours = Math.max(1, Math.min(720, Number(slaRiskHours) || 36));
  try {
    const rows = await db.query(
      `SELECT r.org_id,
              o.external_id AS org_external_id,
              o.name        AS org_name,
              COUNT(*) FILTER (WHERE r.status = 'awaiting_review')::int AS awaiting_review,
              COUNT(*) FILTER (WHERE r.status IN ('submitted', 'processing'))::int AS in_flight,
              COUNT(*) FILTER (WHERE r.status = 'quoted')::int AS open_quotes,
              COALESCE(SUM((r.landed_quote->>'totalLandedCents')::bigint)
                FILTER (WHERE r.status = 'quoted'), 0)::bigint AS open_quote_value_cents,
              COUNT(*) FILTER (
                WHERE r.quoted_at IS NULL
                  AND r.status IN ('submitted', 'processing', 'awaiting_review')
                  AND r.created_at < now() - ($1 || ' hours')::interval
              )::int AS sla_risk
         FROM import_requests r
         JOIN organisations o ON o.id = r.org_id
        WHERE r.archived_at IS NULL
        GROUP BY r.org_id, o.external_id, o.name
        ORDER BY sla_risk DESC, open_quote_value_cents DESC, awaiting_review DESC`,
      [String(riskHours)],
    );
    return {
      ok: true,
      rows: rows.map(/** @param {any} r */ (r) => ({
        orgId: Number(r.org_id),
        orgExternalId: String(r.org_external_id || ''),
        ...
        openQuoteValueCents: Number(r.open_quote_value_cents) || 0,
        slaRisk: Number(r.sla_risk) || 0,
      })),
    };
  } catch (err) {
    return failureFromDb(err instanceof Error ? err.message : 'operator triage failed');
  }
}

/**
 * Sprint 90 — one org's open worklist for the triage console
 * drill-down. Read-only, admin-gated at the handler (same
 * cross-org rule as aggregateOperatorTriage). Three buckets, each
 * capped + urgency-ordered:
 *   atRisk   — unquoted past the risk line, oldest first (the
 *              rows about to breach the 48h budget)
 *   awaiting — awaiting_review, oldest first
 *   quoted   — open quotes, value desc (what's at stake)
 *
 * @param {{ orgId: number, slaRiskHours: number, cap?: number }} opts
 */
async function listOperatorTriageDetail({ orgId, slaRiskHours, cap = 10 }) {
  if (!db.isConfigured()) return { ok: false, errors: ['db not configured'] };
  if (!Number.isInteger(orgId)) return { ok: false, errors: ['orgId required'] };
  const riskHours = Math.max(1, Math.min(720, Number(slaRiskHours) || 36));
  const limit = Math.max(1, Math.min(50, Number(cap) || 10));
  const project = /** @param {any} r */ (r) => ({
    externalId: String(r.external_id),
    label: String(r.label || ''),
    status: String(r.status),
    ageHours: Math.round(Number(r.age_hours) * 10) / 10,
  ...
  try {
    const [atRisk, awaiting, quoted] = await Promise.all([
      db.query(
        `SELECT external_id, label, status,
                EXTRACT(EPOCH FROM (now() - created_at)) / 3600 AS age_hours,
                (landed_quote->>'totalLandedCents')::bigint AS landed_cents
      ...
      db.query(
        `SELECT external_id, label, status,
          ...
          WHERE org_id = $1 AND archived_at IS NULL
            AND status = 'quoted'
          ORDER BY (landed_quote->>'totalLandedCents')::bigint DESC NULLS LAST
          LIMIT $2`,
        [orgId, limit],
      ),
    ]);
    return {
      ok: true,
      atRisk: atRisk.map(project),
      awaiting: awaiting.map(project),
      quoted: quoted.map(project),
    };
  } catch (err) {
    return failureFromDb(err instanceof Error ? err.message : 'operator triage detail failed');
  }
}

// ── SLA breach sweep (sprint 97) ─────────────────────────────────────

/**
 * Record newly-breached requests for ONE org: unquoted, pre-quote
 * status, past the org's effective turnaround target, not yet
 * stamped. Each breach becomes exactly one audit-chained,
 * webhook-dispatched import_request_sla_breached event.
 *
 * At-least-once discipline: stamp first (the WHERE re-checks
 * everything, so a concurrent quote landing between read and
 * write drops the row), then record the event; if the event
 * write THROWS, the stamp is REVERTED so the next sweep retries.
 * The stamp can never claim a breach the audit chain doesn't
 * hold (the ADR-0005 posture, adapted to a system actor).
 *
 * @param {{ orgId: number, targetHours: number, cap?: number }} input
 */
async function sweepSlaBreachesForOrg({ orgId, targetHours, cap = 50 }) {
  if (!db.isConfigured()) return { ok: false, errors: ['db not configured'] };
  if (!Number.isInteger(orgId)) return { ok: false, errors: ['orgId required'] };
  const target = (Number.isInteger(targetHours) && targetHours > 0) ? targetHours : 48;
  const limit = Math.max(1, Math.min(200, Number(cap) || 50));
  let stamped;
  try {
    stamped = await db.query(
      `UPDATE import_requests
          SET sla_breach_recorded_at = now()
        WHERE org_id = $1
          AND archived_at IS NULL
          AND quoted_at IS NULL
          AND sla_breach_recorded_at IS NULL
          AND status IN ('submitted', 'processing', 'awaiting_review')
          AND created_at < now() - ($2 || ' hours')::interval
          AND external_id IN (
            SELECT external_id FROM import_requests
             WHERE org_id = $1
               AND archived_at IS NULL
               AND quoted_at IS NULL
               AND sla_breach_recorded_at IS NULL
        ...
        RETURNING external_id, label,
                  EXTRACT(EPOCH FROM (now() - created_at)) / 3600 AS age_hours`,
      [orgId, String(target), limit],
    );
  } catch (err) {
    return failureFromDb(err instanceof Error ? err.message : 'sla breach sweep failed');
  }
  let recorded = 0;
  /** @type {string[]} */
  const reverted = [];
  for (const row of stamped) {
    const externalId = String(row.external_id);
    try {
      await events.record('import_request_sla_breached', {
        orgId,
        actorEmailHash: null, // system detection, not a user action
        entityType: 'import_request',
        entityId: externalId,
        detail: {
          // Sprint 100 — commitment discriminator (legacy events
          // without the field are turnaround).
          commitment: 'quoteTurnaround',
          targetHours: target,
          ageHoursAtDetection: Math.round(Number(row.age_hours) * 10) / 10,
        },
      });
      recorded += 1;
    } catch (_) {
      // Revert the stamp — the next sweep retries, and the stamp
      // never asserts a breach the chain doesn't hold.
      try {
        await db.query(
          `UPDATE import_requests SET sla_breach_recorded_at = NULL
            WHERE org_id = $1 AND external_id = $2`,
          [orgId, externalId],
        );
      } catch (_) { /* revert is best-effort; worst case one duplicate event */ }
      reverted.push(externalId);
    }
  }
  return { ok: true, stamped: stamped.length, recorded, reverted };
}

/**
 * Sprint 100 — the first-response half of breach recording. Same
 * at-least-once + revert-on-event-failure semantics as the
 * turnaround sweep (sprint 97); the predicate is the human-touch
 * clock: first_ops_action_at IS NULL past the org's negotiated
 * first-response target. One shared event type, discriminated by
 * detail.commitment, so the published breach count keeps meaning
 * what /trust/sla says it means.
 *
 * @param {{ orgId: number, targetHours: number, cap?: number }} input
 */
async function sweepFirstResponseBreachesForOrg({ orgId, targetHours, cap = 50 }) {
  if (!db.isConfigured()) return { ok: false, errors: ['db not configured'] };
  if (!Number.isInteger(orgId)) return { ok: false, errors: ['orgId required'] };
  const target = (Number.isInteger(targetHours) && targetHours > 0) ? targetHours : 24;
  const limit = Math.max(1, Math.min(200, Number(cap) || 50));
  let stamped;
  try {
    stamped = await db.query(
      `UPDATE import_requests
          SET fr_breach_recorded_at = now()
        WHERE org_id = $1
          AND archived_at IS NULL
          AND first_ops_action_at IS NULL
          AND fr_breach_recorded_at IS NULL
          AND status IN ('submitted', 'processing', 'awaiting_review')
          AND created_at < now() - ($2 || ' hours')::interval
          AND external_id IN (
            SELECT external_id FROM import_requests
    ...
    return failureFromDb(err instanceof Error ? err.message : 'fr breach sweep failed');
  }
  let recorded = 0;
  /** @type {string[]} */
  const reverted = [];
  for (const row of stamped) {
    ...
    try {
      ...
      try {
        await db.query(
          `UPDATE import_requests SET fr_breach_recorded_at = NULL
            WHERE org_id = $1 AND external_id = $2`,
          [orgId, externalId],
        );
      } catch (_) { /* revert is best-effort */ }
      reverted.push(externalId);
    }
  }
  return { ok: true, stamped: stamped.length, recorded, reverted };
...
/**
 * Sprint 102 — orphaned-intake cut for the rescue sweep. Rows the
 * client created but never processed (the tab died between the
 * create POST and the process POST): status still 'submitted'
 * past a grace period. Cross-org in ONE query — the orchestrator
 * takes orgId per row, so no fan-out is needed. Oldest first
 * (they've waited longest against their SLA clock), capped to
 * bound the runner's wall clock.
 *
 * The 10-minute default grace means the sweep can never race a
 * live tab that is simply slow.
 *
 * @param {{ olderThanMinutes?: number, cap?: number }} opts
 */
async function listOrphanedSubmittedRequests({ olderThanMinutes = 10, cap = 20 } = {}) {
  if (!db.isConfigured()) return [];
  const minutes = Math.max(1, Math.min(1440, Number(olderThanMinutes) || 10));
  const limit = Math.max(1, Math.min(50, Number(cap) || 20));
  try {
    const rows = await db.query(
      `SELECT org_id, external_id, created_by_email_hash,
    ...
    return rows.map(/** @param {any} r */ (r) => ({
      orgId: Number(r.org_id),
      externalId: String(r.external_id),
      createdByEmailHash: r.created_by_email_hash ? String(r.created_by_email_hash) : null,
      ageMinutes: Math.round(Number(r.age_minutes)),
    }));
  } catch (_) {
    // Fail-open — the sweep just finds nothing this tick.
    return [];
  }
}
      ...
      isFailure
        ...
        : `UPDATE import_requests
             SET status = $1,
                 ...
                 quoted_at = CASE WHEN $1 = 'quoted' THEN COALESCE(quoted_at, now()) ELSE quoted_at END,
                 updated_at = now()
           WHERE org_id = $3 AND external_id = $4 RETURNING *`,
    ...
    const rows = await db.query(
      `UPDATE import_requests
         SET status = $1,
             ...
             quoted_at = CASE WHEN $1 = 'quoted' THEN COALESCE(quoted_at, now()) ELSE quoted_at END,
             first_ops_action_at = COALESCE(first_ops_action_at, now()),
             updated_at = now()
...
// Sprint 76 — restore an archived request. The exact mirror of
// archiveImportRequest (archive is never a one-way door — the
// SAP-GTS reversibility posture). Idempotent: restoring a live
// row returns { unchanged: true } rather than erroring, same as
// re-archiving an archived one. The audit event pairs with
// import_request_archived so the timeline reads archive → restore
// as two truthful entries, never a deletion.
/**
 * @param {{ orgId: number, externalId: string, actorEmailHash: string }} input
 */
async function restoreImportRequest({ orgId, externalId, actorEmailHash }) {
  if (!Number.isInteger(orgId) || !externalId || !actorEmailHash) {
    return { ok: false, errors: ['orgId, externalId, actorEmailHash required'] };
  }
  if (!db.isConfigured()) return notConfigured();
  ...
  try {
    const rows = await db.query(
      `UPDATE import_requests SET archived_at = NULL, updated_at = now()
       WHERE org_id = $1 AND external_id = $2 AND archived_at IS NOT NULL
       RETURNING *`,
      [orgId, externalId],
    );
    if (rows.length === 0) return { ok: true, importRequest: beforeRow, unchanged: true };
    const importRequest = rowToImportRequest(rows[0]);
    await events.record('import_request_restored', {
      orgId,
      actorEmailHash,
      entityType: 'import_request',
      entityId: externalId,
    ...
    return failureFromDb(err instanceof Error ? err.message : 'restoreImportRequest failed');
  }
}

// Sprint 76 — bulk restore. The exact mirror of the sprint-74
// bulkArchiveImportRequests: same 50-row cap, same Set dedup,
// same batch-level not-configured fast-fail, same three-outcome
// report (restored / unchanged / failed) with per-row isolation.
// Wraps the per-row restore path so every invariant rides along.
/**
 * @param {{ orgId: number, externalIds: string[], actorEmailHash: string }} input
 */
async function bulkRestoreImportRequests({ orgId, externalIds, actorEmailHash }) {
  if (!Number.isInteger(orgId)) {
    return { ok: false, errors: ['orgId required'] };
  }
  if (!Array.isArray(externalIds) || externalIds.length === 0) {
  ...
  if (externalIds.length > 50) {
    return { ok: false, errors: ['bulk-restore capped at 50 requests per call'] };
  }
  if (!db.isConfigured()) return notConfigured();
  const ids = [...new Set(externalIds.map(String))];
  /** @type {Array<{ externalId: string, unchanged: boolean }>} */
  ...
  for (const externalId of ids) {
    const result = await restoreImportRequest({ orgId, externalId, actorEmailHash });
    if (result.ok) {
      const win = /** @type {any} */ (result);
      succeeded.push({ externalId, unchanged: win.unchanged === true });
    } else {
  ...
  return {
    ok: true,
    restoredCount: succeeded.filter((s) => !s.unchanged).length,
    unchangedCount: succeeded.filter((s) => s.unchanged).length,
    failedCount: failed.length,
    succeeded,
    failed,
...
module.exports = {
  ...
  SUPPLIER_CONCENTRATION_THRESHOLD,
  RATING_TREND_CURRENT_DAYS,
  RATING_TREND_BASELINE_DAYS,
  RATING_TREND_MIN_COUNT,
  RATING_TREND_DROP_THRESHOLD,
  MESSAGE_ROLES,
  MESSAGE_BODY_MAX,
  ...
  bulkArchiveImportRequests,
  bulkRestoreImportRequests,
  ...
  recordImportRequestActual,
  listActualOutcomesForLedger,
  listQuoteTurnaroundsForSla,
  sweepSlaBreachesForOrg,
  sweepFirstResponseBreachesForOrg,
  listOrphanedSubmittedRequests,
  listFirstResponsesForSla,
  aggregateOperatorTriage,
  listOperatorTriageDetail,
  markMessagesRead,
  computeUnreadCount,
  expireStaleQuotes,
