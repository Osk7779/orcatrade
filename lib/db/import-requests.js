 ...
 *
 * @param {{ orgId: number, windowDays?: number, stallThresholdDays?: number, declineSpikeRateMultiplier?: number, supplierConcentrationThreshold?: number }} input
 */
async function aggregateOpsInsights({ orgId, windowDays = 30, stallThresholdDays, declineSpikeRateMultiplier, supplierConcentrationThreshold }) {
  if (!Number.isInteger(orgId)) return { ok: false, errors: ['orgId required'] };
  if (!db.isConfigured()) return notConfigured();
  ...
  try {
    ...
    const isRatingDeclining = (
      ...
      && (ratingTrendBaseline.avg - ratingTrendCurrent.avg) >= RATING_TREND_DROP_THRESHOLD
    );
    ...
    return {
      ...
      insights: {
        ...
        ratingTrend: {
          ...
          dropThreshold: RATING_TREND_DROP_THRESHOLD,
