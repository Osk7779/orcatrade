 ...
 *
 * @param {{ orgId: number, windowDays?: number, stallThresholdDays?: number, declineSpikeRateMultiplier?: number, supplierConcentrationThreshold?: number, ratingTrendDropThreshold?: number }} input
 */
async function aggregateOpsInsights({ orgId, windowDays = 30, stallThresholdDays, declineSpikeRateMultiplier, supplierConcentrationThreshold, ratingTrendDropThreshold }) {
  if (!Number.isInteger(orgId)) return { ok: false, errors: ['orgId required'] };
  if (!db.isConfigured()) return notConfigured();
  ...
  try {
    ...
    const agingQuoteCountRows = await db.query(
      `SELECT COUNT(*)::int AS n
        ...
        WHERE org_id = $1
          ...
          AND status = 'quoted'
          AND updated_at < now() - ($2 || ' days')::interval`,
      [orgId, String(QUOTE_FOLLOWUP_THRESHOLD_DAYS)],
    );
    ...
    if (agingQuoteCount > 0) {
      const agingQuoteListRows = await db.query(
        `SELECT external_id, label, updated_at,
          ...
          ORDER BY updated_at ASC
          LIMIT $3`,
        [orgId, String(QUOTE_FOLLOWUP_THRESHOLD_DAYS), QUOTE_FOLLOWUP_CAP],
      );
    ...
    return {
      ...
      insights: {
        ...
        quoteFollowUp: {
          thresholdDays: QUOTE_FOLLOWUP_THRESHOLD_DAYS,
