 ...
 * }} input
 */
async function listImportRequestsForOrg({ orgId, includeArchived = false, limit = 200, status, createdByEmailHash, declineReason, q }) {
...
async function aggregateSupplierPicks({ orgId, hsPrefix6, windowDays = 90 }) {
  ...
  try {
    ...
    /** @type {Record<string, { count: number, lastPickedAt: string, rationaleCategoryMix: Record<string, number> }>} */
    const byCountry = {};
    for (const r of rows) {
      ...
      if (!byCountry[country]) {
        byCountry[country] = { count: 0, lastPickedAt: at, rationaleCategoryMix: {} };
      }
