 ...
 *
 * @param {{ orgId: number, windowDays?: number, stallThresholdDays?: number }} input
 */
async function aggregateOpsInsights({ orgId, windowDays = 30, stallThresholdDays }) {
  if (!Number.isInteger(orgId)) return { ok: false, errors: ['orgId required'] };
  if (!db.isConfigured()) return notConfigured();
  ...
  try {
    ...
    for (const reason of Object.keys(currentByReason)) {
      ...
      if (currentRate < DECLINE_SPIKE_RATE_MULT * baselineRate) continue;
      spikes.push({
        reason,
        currentCount,
        baselineCount,
    ...
    return {
      ...
      insights: {
        ...
        declineSpike: {
          ...
          rateMultiplier: DECLINE_SPIKE_RATE_MULT,
          spikes,
        },
      },
    };
