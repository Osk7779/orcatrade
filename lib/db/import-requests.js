...
async function listQuoteTurnaroundsForSla({ orgId, windowDays = 90 } = {}) {
  if (!db.isConfigured() || !Number.isInteger(orgId)) return [];
  const days = Math.max(1, Math.min(365, Number(windowDays) || 90));
  try {
    const rows = await db.query(
      `SELECT created_at, quoted_at
        ...
        WHERE org_id = $1
          ...
          AND quoted_at >= now() - ($2 || ' days')::interval`,
      [orgId, String(days)],
    );
    return rows.map(/** @param {any} r */ (r) => ({
  ...
  try {
    const rows = await db.query(
      `SELECT created_at, first_ops_action_at
         FROM import_requests
        WHERE org_id = $1
          AND first_ops_action_at IS NOT NULL
          AND first_ops_action_at >= now() - ($2 || ' days')::interval`,
      [orgId, String(days)],
    );
    return rows.map(/** @param {any} r */ (r) => ({
      createdAt: r.created_at instanceof Date ? r.created_at.toISOString() : String(r.created_at),
