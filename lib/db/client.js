...
async function query(sql, params = []) {
  ...
  try {
    ...
    // The neon HTTP client is a template-tag function that also accepts
    // .query(sql, params) for the regular-string + params shape we use.
    // Call signature: await sql(strings, ...values) OR await sql.query(text, params).
    const result = await client.query(sql, params);
    // The driver returns { rows, rowCount, …} in object form when called
    // via .query(). Normalise to a plain array.
