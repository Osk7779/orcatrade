...
function sqlClient() {
  ...
  const { neon } = require('./neon-driver').loadNeon();
  _sql = neon(poolUrl());
  return _sql;
}
...
async function query(sql, params = []) {
  ...
  try {
    ...
    // @neondatabase/serverless v1.x switched the call signature: the
    // bare `client(text, params)` form was deprecated in favour of
    // `client.query(text, params, options)`. We use `.query()` for
    // forward-compat. v0.x clients also expose `.query` so this works
    // across versions.
    const result = typeof client.query === 'function'
