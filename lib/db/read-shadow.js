...
// Default posture
// ───────────────
// As of the 2026-06-08 audit: PRODUCTION defaults to ENABLED (always-on
// shadowing), test/dev default to DISABLED (preserves existing-test
// hermeticity). The audit flagged opt-in-by-default as an enterprise
// gap — dual-write divergence could go undetected for days. Production
// always-on closes that gap; the env flag becomes an explicit override.
//
// PG reads cost a round-trip each — production-on is acceptable because
// shadowCompare() is only called from specific read paths (saved-plans,
// saved-portfolios, events list) that are not on every request path,
// and the projector filters out noisy fields so divergence-log noise
// stays low.
//
// Env override semantics:
//   ORCATRADE_SHADOW_PG=0     → DISABLED (force off, even in production)
//   ORCATRADE_SHADOW_PG=false → DISABLED
//   ORCATRADE_SHADOW_PG=off   → DISABLED
//   ORCATRADE_SHADOW_PG=no    → DISABLED
//   ORCATRADE_SHADOW_PG=1     → ENABLED (force on, even in test/dev)
//   ORCATRADE_SHADOW_PG=true  → ENABLED
//   (env unset/empty)         → DEFAULT = production-on / non-production-off
//
// Why "projector"
// ───────────────
...
function isEnabled() {
  const flag = process.env.ORCATRADE_SHADOW_PG;
  // Explicit env override always wins.
  if (flag === '0' || flag === 'false' || flag === 'off' || flag === 'no') return false;
  if (flag !== undefined && flag !== '') return true;
  // Default: production-on, non-production-off. Closes the 2026-06-08
  // audit gap (always-on divergence detection in prod) without breaking
  // test hermeticity.
