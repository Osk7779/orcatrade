...
//
// Atomically updates the three sanctions fields (sanctionsLastStatus,
// sanctionsLastScreenedAt, sanctionsLastMatchSummary) AND emits a
// supplier_master_rescreened audit event with the before/after diff
// of just those fields. Used by POST /api/suppliers/<id>/screen.
//
// Why a dedicated function vs reusing updateSupplier:
//   1. The audit event name is distinct — operators reading the
//      timeline need to tell automated screenings from hand edits.
//      A re-screen with the same status as before is still an event;
//      validateForUpdate's no-op short-circuit would suppress it.
//   2. The sanctions fields are NOT editable via the inline edit form
//      (PR #123 explicitly excluded them — see its EditForm doc).
//      Routing them through updateSupplier would weaken that boundary.
//   3. The summary payload may carry several KB of match data;
//      passing it through validateForUpdate's general patch shape
//      conflates two operational flows.
 ...
 *   status: string,                              // one of SANCTIONS_STATUSES
 *   matchSummary: Record<string, unknown>,       // { listSource, matchCount, matches, advisory, ... }
 *   screenedAtIso?: string,                      // override for tests; defaults to now()
 * }} input
 */
async function recordScreeningResult({ orgId, externalId, actorEmailHash, status, matchSummary, screenedAtIso }) {
  if (!Number.isInteger(orgId) || !externalId || !actorEmailHash) {
    return { ok: false, errors: ['orgId, externalId, actorEmailHash required'] };
  }
  if (!SANCTIONS_STATUSES.includes(String(status))) {
    return { ok: false, errors: [`status must be one of: ${SANCTIONS_STATUSES.join(', ')}`] };
  }
  if (!matchSummary || typeof matchSummary !== 'object' || Array.isArray(matchSummary)) {
    return { ok: false, errors: ['matchSummary must be an object'] };
  }
  if (!db.isConfigured()) return notConfigured();

  // Fetch BEFORE so the audit diff is complete — same pattern as
  // updateSupplier / archiveSupplier.
  const before = await getSupplierByExternalId({ orgId, externalId });
  if (!before.ok) return before;
  const beforeSupplier = /** @type {any} */ (before).supplier;

  // Permit caller-supplied screenedAtIso so tests can pin a stable
  // timestamp without freezing the DB clock. Production calls just
  // pass undefined and the DB defaults to now().
  /** @type {string} */
  let screenedAtSql;
  /** @type {any[]} */
  let params;
  if (screenedAtIso) {
    screenedAtSql = '$5';
    params = [orgId, externalId, String(status), JSON.stringify(matchSummary), screenedAtIso];
  } else {
    screenedAtSql = 'now()';
    params = [orgId, externalId, String(status), JSON.stringify(matchSummary)];
  }

  try {
    const rows = await db.query(
      `UPDATE supplier_master
         SET sanctions_last_status = $3,
             sanctions_last_match_summary = $4,
             sanctions_last_screened_at = ${screenedAtSql},
             updated_at = now()
       WHERE org_id = $1 AND external_id = $2 AND archived_at IS NULL
       RETURNING *`,
      params,
    ...
    if (rows.length === 0) return { ok: false, errors: ['not_found'], notFound: true };
    // rowToSupplier is typed as returning Supplier|null, but the
    // RETURNING * above guarantees the row is non-null here. The
    // any-cast keeps tsc happy without adding a runtime null check
    // that would never fire — same pattern used for `before` above.
    const after = /** @type {any} */ (rowToSupplier(rows[0]));

    // Diff shape: only the three sanctions fields. Keeping the diff
    // tight makes the audit timeline readable AND keeps the event
    // payload small (KB savings on multi-thousand-match screenings).
    const beforeSlice = {
      sanctionsLastStatus: beforeSupplier.sanctionsLastStatus ?? null,
      sanctionsLastScreenedAt: beforeSupplier.sanctionsLastScreenedAt ?? null,
      sanctionsLastMatchSummary: beforeSupplier.sanctionsLastMatchSummary ?? null,
    };
    const afterSlice = {
      sanctionsLastStatus: after.sanctionsLastStatus,
      sanctionsLastScreenedAt: after.sanctionsLastScreenedAt,
      sanctionsLastMatchSummary: after.sanctionsLastMatchSummary,
    };

    await events.record('supplier_master_rescreened', {
      orgId,
      actorEmailHash,
      entityType: 'supplier_master',
      entityId: externalId,
    ...
    return failureFromDb(err instanceof Error ? err.message : 'supplier re-screen failed');
  }
}
