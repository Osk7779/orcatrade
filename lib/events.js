...
const ALLOWED_TYPES = new Set([
  ...
  // Sprint 76 — archive's mirror. Restore pairs with archived so
  // the timeline reads archive → restore as two truthful entries.
  'import_request_restored',
  // Sprint 81 — actual landed outcome reported (Track B phase 2).
  // Detail carries the deterministic cents + variance; notes stay
  // on the row.
  'import_request_actual_reported',
  // Sprint 97 — a blown turnaround commitment, recorded as a fact
  // by the hourly sweep (system actor). Exactly one per request.
  ...
  // Sprint 30 — customer rating recorded post-approval. Detail
  // carries { score, hasComment, isSupersession } — the free-text
  // comment stays on the row.
  ...
  // Sprint 42 — per-org operator config mutation. Org-wide policy
  // change (stallThresholdDays today; future knobs as the config
  // surface grows). Written before PATCH returns 200; an audit-
  // chain entry preserves the prior state via the `before` field.
  'operator_config_updated',
  // Sprint 44 — per-org API key lifecycle. Both events name the
  // keyId hash (entityId) so an audit reader can correlate
  // creation with revocation; neither event surfaces the raw
  // key (returned only at create time on the response, never
  // logged).
  'api_key_created',
  'api_key_revoked',
  // Sprint 47 — outbound webhook subscription lifecycle + test
  // delivery. Audit detail surfaces the label + url + eventTypes
  // (NEVER the secret — chain readers can't exfiltrate signing
  // material). _tested is non-blocking: a failed audit write
  // there does NOT 500 the response (test deliveries aren't
  // mutations of substantive state).
  'webhook_subscription_created',
  'webhook_subscription_deleted',
  'webhook_subscription_tested',
  // Sprint 51 — auto-disable + reactivation. Auto-disable fires
  // from inside the cron retry-flush path when the per-sub
  // consecutive-abandonment counter crosses
  // AUTO_DISABLE_THRESHOLD; reactivation is an admin action.
  // Both events name the subscription id (entityId) so audit
  // readers can correlate.
  'webhook_subscription_auto_disabled',
  'webhook_subscription_reactivated',
  // Sprint 59 — signing-secret rotation. The new secret signs
  // subsequent deliveries; the audit detail records the rotator
  // emailHash + timestamp so a future receiver-side incident
  // review can correlate. The raw secret NEVER appears in
  // the chain (same posture as sprint 47 create + sprint 44
  // api-key create).
  'webhook_subscription_secret_rotated',
  // Sprint 55 — internal ops notes (annotations visible only to
  // ops, never to the customer). Audit detail records the noteId
  // + body length (NOT the body itself — same privacy posture as
  // sprint 18 messages).
  'import_request_internal_note_added',
  // Sprint 61 — internal note edit + soft-delete. Both gated to
  // the note's original author (owner-only RBAC). Audit detail
  // records the noteId + new length (edit) / noteId (delete);
  // body NEVER in the chain.
...
async function record(type, payload = {}) {
  ...
  // ── Tertiary: outbound webhook dispatch (Sprint 48) ──
  // Fire-and-forget POSTs to every subscriber that asked for this
  // event type. The dispatcher filters by WEBHOOK_EVENT_TYPES + by
  // per-subscription eventTypes, so the gate here is permissive
  // (only checks "has orgId" + "type registered for dispatch"
  // semantics). Caveat: on Vercel's serverless model the handler
  // returns to the request loop the moment the response is sent;
  // a slow customer endpoint might be cut short by the lambda
  // freezing. Sprint 49+ can add a durable queue. v1 is "best-
  // effort fan-out on the same invocation."
  //
  // The require is deferred so a circular dep (webhooks-dispatch
  // imports events for ALLOWED_TYPES validation at boot) can never
  // form at module-load time.
  Promise.resolve().then(() => {
    const { dispatchEvent } = require('./webhooks-dispatch');
    return dispatchEvent({ event });
  }).catch(() => { /* never propagate to caller */ });
...
function buildPgInsertParams(type, payload) {
  ...
  // Sprint 79 — populate the org_id COLUMN (it existed since the
  // base schema, with events_org_created_idx on it, but the writer
  // never set it — every org-scoped PG read would have been an
  // unindexed jsonb scan). orgId stays IN the payload too (KV
  // parity — readers spread the payload and expect it there).
  // Only plain integers pass: orgId values in this codebase come
  // from organisations.id lookups, so the FK holds; anything else
  // lands NULL rather than failing the insert.
  const orgId = Number.isInteger(safePayload.orgId) ? safePayload.orgId : null;
  return {
    type,
    emailHash,
    orgId,
...
async function recordPg(type, payload) {
  ...
  const { type: t, emailHash, orgId, payloadJson } = buildPgInsertParams(type, payload);
  try {
    await db.query(
      'INSERT INTO events (type, org_id, email_hash, payload) VALUES ($1, $2, $3, $4::jsonb)',
      [t, orgId, emailHash, payloadJson],
    );
    return { written: true };
  } catch (err) {
...
// Sprint 98 — typed event count over the PG spine. Powers the
// published breach ledger on /api/sla (+ the trust pack via the
// single-source reuse). Returns NULL — not 0 — when PG is
// unconfigured or the query fails: 'we recorded zero breaches'
// and 'we cannot read the ledger right now' are different truths,
// and the surface must never render the second as the first.
//
// @param {{ type: string, sinceDays?: number }} opts
async function countEventsFromPg({ type, sinceDays = 90 } = {}) {
  if (!type || typeof type !== 'string') return null;
  let db;
  try { db = require('./db/client'); }
  catch (_) { return null; }
  if (!db.isConfigured()) return null;
  const days = Math.max(1, Math.min(3650, Number(sinceDays) || 90));
  try {
...
// ── Sprint 79 — PG-first promotion for the scoped readers ─────────
//
// listFromPg/listUnified (BG-2.3) covered the FLAT dashboards; the
// three SCOPED readers under the operator wedge (per-entity
// timelines, org activity feed, operator-config history) stayed
// KV-bound — every call a 5000-row scan filtered in-process, and
// every org's audit horizon capped at the KV log's eviction line.
// A licensed counterparty cannot lose audit history: these helpers
// give each scoped reader an indexed, unbounded PG path.
//
// Fallback discipline (same as listUnified):
//   - PG not configured / query throws  → null   → caller uses KV
//   - PG returns [] but KV has rows     → caller uses KV (dual-
//     write only covers events since BG-2.2 — until the KV era
//     ages out, an empty PG cut is not proof of an empty history)
//   - PG returns rows                   → caller uses them
// Reads stay fail-OPEN (sprint-65 distinction: mutations gate
// fail-closed; display surfaces degrade gracefully).
//
// The tamper-evident chain readers are deliberately NOT promoted —
// the _seq/_prevHash/_hash chain is a property of the KV log and
// is verified there (audit-integrity surface).

// Flatten one pg.events row to the KV event shape consumers expect:
// { type, at, ...payload, emailHash? }. Payload spread comes LAST-
// but-one so a payload-carried `at` (the original event stamp)
// overrides the insert timestamp when present.
/** @param {any} r */
function flattenPgEventRow(r) {
  return {
    type: r.type,
    at: r.created_at instanceof Date ? r.created_at.toISOString() : String(r.created_at),
    ...(typeof r.payload === 'string' ? JSON.parse(r.payload) : (r.payload || {})),
...
// Indexed per-entity cut (events_entity_created_idx, schema-021).
// Oldest-first — the timeline contract. Returns null on PG-
// unavailable/error so the caller can distinguish "no PG" from
// "genuinely empty".
async function listForEntityFromPg({ entityType, entityId, limit }) {
  let db;
  try { db = require('./db/client'); }
  catch (_) { return null; }
  if (!db.isConfigured()) return null;
  ...
  try {
    const rows = await db.query(
      `SELECT type, email_hash, payload, created_at
         FROM events
        WHERE payload->>'entityType' = $1
          AND payload->>'entityId' = $2
        ORDER BY created_at ASC
        LIMIT $3`,
      [entityType, entityId, cap],
    );
    return rows.map(flattenPgEventRow);
  } catch (_) {
    return null;
  }
}

// Indexed org cut (events_org_created_idx — populated since the
// sprint-79 writer fix + schema-021 backfill). Newest-first — the
// activity-stream contract. Type allowlist applied IN SQL so the
// LIMIT counts visible rows, not pre-filter rows.
async function listForOrgFromPg({ orgId, types, limit }) {
  let db;
  try { db = require('./db/client'); }
  catch (_) { return null; }
  if (!db.isConfigured()) return null;
...
//
// Sprint 79 — PG-first via the indexed entity cut; the KV path
// below is the fallback (pre-dual-write history + PG outage).
//
// @param {{ entityType: string, entityId: string, limit?: number }} opts
async function listForEntity({ entityType, entityId, limit = 100 } = {}) {
  if (!entityType || !entityId) return [];
  const pgRows = await listForEntityFromPg({ entityType, entityId, limit });
...
const ORG_ACTIVITY_TYPES = new Set([
  ...
  'import_request_status_transition',
  'import_request_archived',
  // Sprint 76 — restore surfaces in the activity feed like its
  // archive sibling (the team should see un-archives too).
  'import_request_restored',
  // Sprint 81 — reported actual outcome. The moment the accuracy
  // loop closes on a request is org-level news.
  'import_request_actual_reported',
  // Sprint 97 — a recorded breach is org-level news by definition.
  'import_request_sla_breached',
  ...
  'import_request_supplier_picked',
  // Sprint 30 — customer rating. Surfaces in the org activity feed
  // so ops sees "5★ rating on ir_xxx" or "2★ — needs follow-up"
  // in real time.
  'import_request_rated',
  // Sprint 55 — internal ops notes. Surface in the activity
  // feed so other ops see context appearing on requests they
  // might pick up next. The activity feed is ops-only on the
  // dashboard (customer dashboards don't render it).
  'import_request_internal_note_added',
  // Sprint 61 — internal note edit + soft-delete. Same
  // activity-feed posture (ops-only) — surface so other ops
  // can spot in-flight revisions to context they may have read
  // earlier.
  'import_request_internal_note_edited',
  'import_request_internal_note_deleted',
...
async function listForOrg({ orgId, limit = 50 } = {}) {
  if (!orgId) return [];
  // Sprint 79 — PG-first via the indexed org cut (allowlist applied
  // in SQL); KV scan below is the fallback.
  const pgRows = await listForOrgFromPg({
    orgId,
    types: Array.from(ORG_ACTIVITY_TYPES),
    limit,
  });
  if (pgRows && pgRows.length > 0) return pgRows;
  const all = await list({ limit: MAX_EVENTS });
...
// Sprint 65 — org-scoped read-only history of operator-config
// PATCHes. Surfaces in the OperatorConfigPanel below the dial
// fields so an admin can see "who twisted which knob and when"
// — enterprise-grade transparency (SAP GTS / Descartes bar)
// on top of the audit-log data ADR-0005 already writes.
//
// Newest-first, capped at limit. Filtered by orgId + type so
// cross-org leakage is impossible; the projection strips the
// chain-hash internals (_seq/_hash/_prevHash) so a compromised
// UI can never see the tamper-evident chain shape (that's for
// audit-integrity readers, not dashboard consumers). Actor is
// projected as the 16-char SHA-256 prefix already stored on
// the event — never the raw email, and never a lookup back to
// a member row (ADR-0008 pseudonymisation posture — the
// audit trail must be readable, but only as the identity the
// system stores, not the identity a viewer wants to see).
async function listOperatorConfigHistory({ orgId, limit = 10 } = {}) {
  if (!orgId) return [];
  const cap = Math.max(1, Math.min(100, Number(limit) || 10));
  // Sprint 79 — PG-first via the same indexed org cut the activity
  // feed uses (single-type allowlist). Past the KV eviction line,
  // this is what keeps the config audit trail unbounded. KV scan
  // below is the fallback (pre-dual-write history + PG outage).
  let sliced;
  const pgRows = await listForOrgFromPg({
    orgId,
    types: ['operator_config_updated'],
    limit: cap,
  });
  if (pgRows && pgRows.length > 0) {
    sliced = pgRows;
  } else {
    const all = await list({ type: 'operator_config_updated', limit: MAX_EVENTS });
    const filtered = all.filter((e) => e && e.orgId === orgId);
    filtered.sort((a, b) => {
      const ta = a && a.at ? Date.parse(a.at) : 0;
      const tb = b && b.at ? Date.parse(b.at) : 0;
      return tb - ta;
    });
    sliced = filtered.slice(0, cap);
  }
  return sliced.map((e) => ({
    at: (e && typeof e.at === 'string') ? e.at : null,
    actorEmailHash: (e && typeof e.actorEmailHash === 'string' && e.actorEmailHash)
      ? e.actorEmailHash
      : null,
    // Only the changed knobs from this PATCH — reading a
    // sequence of entries reconstructs the timeline. The
    // guard on `patched` shape keeps a malformed historical
    // event from crashing the UI: an empty object is safe to
    // render as "no fields changed".
    patched: (e && e.detail && typeof e.detail === 'object'
      && e.detail.patched && typeof e.detail.patched === 'object')
      ? e.detail.patched
      : {},
    // Sprint 72 — before-values for the knobs this PATCH touched.
    // `null` (the whole field) for sprint-42..71 legacy events that
    // predate the capture AND for PATCHes where the pre-mutation
    // read failed (the handler omits the key rather than record a
    // fabricated "was default"). Per-knob `null` INSIDE the object
    // means "was at platform default" — the renderer joins with
    // the defaults it already holds. Array guard mirrors the
    // patched guard: a malformed historical event renders as
    // "no before-values" rather than crashing the UI.
    previous: (e && e.detail && typeof e.detail === 'object'
      && e.detail.previous && typeof e.detail.previous === 'object'
      && !Array.isArray(e.detail.previous))
      ? e.detail.previous
      : null,
    // Sprint 66 — SAP-GTS-style change reason. Only populated when
    // the actor provided one; sprint-42..65 historical events (which
    // predate the reason field) surface as null so the renderer can
    // cleanly hide the row when absent.
    reason: (e && e.detail && typeof e.detail === 'object'
      && typeof e.detail.reason === 'string' && e.detail.reason.trim().length > 0)
      ? e.detail.reason
      : null,
    // Sprint 67 — reset-to-default knob names. Empty [] for sprint-
    // 42..66 legacy events and for PATCHes that only set (didn't
    // reset). Non-empty for reset-only or set+reset PATCHes. The
    // filter guard keeps stray non-string entries out of the
    // renderer if a malformed event ever landed in the log.
    reset: (e && e.detail && typeof e.detail === 'object'
      && Array.isArray(e.detail.reset))
      ? e.detail.reset.filter((k) => typeof k === 'string')
      : [],
    // Sprint 68 — preset name applied (or null for non-preset
    // PATCHes and sprint-42..67 legacy events). The renderer
    // uses this to label the entry as a policy-profile switch
    // ("Applied preset: strict") distinctly from a manual
    // four-knob dial. The projection accepts any non-empty
    // string — an unknown preset name from a malformed event
    // renders as-is rather than being silently dropped,
    // because the audit trail's job is to reflect what was
    // recorded, not to sanitise historical values.
    preset: (e && e.detail && typeof e.detail === 'object'
      && typeof e.detail.preset === 'string' && e.detail.preset.length > 0)
      ? e.detail.preset
      : null,
    // Sprint 73 — the `at` of the entry this PATCH reverted, or
    // null for non-undo PATCHes and sprint-42..72 legacy events.
    // The renderer uses it to label the entry as a reversal
    // ("⎌ undo") distinct from a manual dial or preset switch.
    undoOf: (e && e.detail && typeof e.detail === 'object'
      && typeof e.detail.undoOf === 'string' && e.detail.undoOf.length > 0)
      ? e.detail.undoOf
      : null,
    // Sprint 75 — org-level cadence change as { from, to }, or
    // null for non-cadence PATCHes and legacy events. Both fields
    // must be strings for the object to project — a malformed
    // historical event renders as "no cadence change" rather
    // than crashing the renderer.
    alertCadence: (e && e.detail && typeof e.detail === 'object'
      && e.detail.alertCadence && typeof e.detail.alertCadence === 'object'
      && typeof e.detail.alertCadence.from === 'string'
      && typeof e.detail.alertCadence.to === 'string')
      ? { from: e.detail.alertCadence.from, to: e.detail.alertCadence.to }
      : null,
  }));
}
...
module.exports = {
  ...
  listForEntity,
  listForOrg,
  listOperatorConfigHistory,
  ORG_ACTIVITY_TYPES,
  listFromPg,
  listUnified,
  // Sprint 79 — PG-first scoped readers (exported for test surface).
  flattenPgEventRow,
  listForEntityFromPg,
  listForOrgFromPg,
  countEventsFromPg,
