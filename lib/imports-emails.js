...
function composeOpsInsightsDigest({ orgName, windowDays, insights }) {
  ...
  // Sprint 94 — trust-era sections. Tier-aware per ADR 0021: below
  // the shared honesty gates a line reads the accruing state, never
  // a null%. Null-safe on legacy insights objects that predate the
  // fields (the composer renders what exists, crashes on nothing).
  const slaTurn = (insights && insights.slaQuoteTurnaround) || null;
  const slaFr = (insights && insights.slaFirstResponse) || null;
  const acc = (insights && insights.accuracyLedger) || null;
  const atRisk = (insights && insights.slaAtRisk) || null;
  /** @param {any} a @param {string} name */
  const attainmentLine = (a, name) => {
    if (!a || !Number.isFinite(Number(a.sampleSize))) return null;
    if (a.tier === 'insufficient') {
      return `${name}: accruing (${a.sampleSize} in sample)`;
    }
    return `${name}: ${a.withinTargetPct}% within ${a.targetHours}h · median ${a.medianHours}h${a.tier === 'indicative' ? ' · early sample' : ''}`;
  };
  const accuracyLine = (!acc || !Number.isFinite(Number(acc.sampleSize)))
    ? null
    : (acc.tier === 'insufficient'
      ? `Quote accuracy: accruing (${acc.sampleSize} outcome${acc.sampleSize === 1 ? '' : 's'} reported)`
      : `Quote accuracy: ${acc.within10Pct}% within ±10% · median error ${acc.medianAbsErrorPct}%${acc.tier === 'indicative' ? ' · early sample' : ''}`);
  const atRiskTotal = atRisk
    ? (Number(atRisk.atRiskCount) || 0) + (Number(atRisk.breachedCount) || 0)
    : null;
  const atRiskLine = atRiskTotal === null
    ? null
    : (atRiskTotal === 0
      ? 'SLA at risk right now: none'
      : `SLA at risk RIGHT NOW: ${atRiskTotal}${Number(atRisk.breachedCount) > 0 ? ` (${atRisk.breachedCount} already breached)` : ''}`);
  const commitmentLines = [
    attainmentLine(slaTurn, 'Quote turnaround'),
    attainmentLine(slaFr, 'First response'),
    accuracyLine,
    atRiskLine,
  ].filter((l) => l !== null);
  ...
  if (commitmentLines.length > 0) {
    textLines.push('', 'Service commitments (measured)');
    for (const line of commitmentLines) textLines.push(`  ${line}`);
  }
  if (totalDeclined > 0) {
  ...
  const html = htmlEmailChrome({
    ...
    bodyHtml: [
      ...
      commitmentLines.length > 0
        ? `<h2 style="font-size:14px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;color:${EMAIL_MUTE};margin:22px 0 4px;">Service commitments (measured)</h2>` + htmlDl(commitmentLines.map((line) => {
...
// ── Sprint 39: daily stalled-queue alert ──────────────────────────
//
// Transactional complement to sprint 26's weekly digest, paired with
// the sprint-38 proactive cohort. Stalls (status='awaiting_review'
// with no activity > 7 days) deserve daily attention — the weekly
// digest is too infrequent for action-NOW signals. The cron fan-out
// runs once a morning so ops can act before customers escalate.
//
// Discipline: no LLM in this path (ADR 0002); every number flows
// directly from the aggregateOpsInsights stalledQueue cohort. Per-
// recipient pref-gated via importStalledQueueAlertEmails so a
// vacation-muting admin doesn't get woken up; the cohort card on
// /imports/insights still surfaces the stall list for whoever
// DOES check.

/**
 * Render the stalled-queue alert as { subject, text, html } for
 * Resend. Returns null when the cohort is empty (caller
 * short-circuits — no point sending "0 stalled" noise).
 *
 * The subject + body name the count + the threshold + the top N
 * stallers (oldest first) with one-decimal daysStalled. CTA points
 * at /imports/insights so the StalledQueueCard becomes the action
 * surface.
 *
 * @param {{
 *   orgName?: string,
 ...
 *     count: number,
 *     items: Array<{ externalId: string, label: string, daysStalled: number }>,
 *   }
 * }} args
 * @returns {{ subject: string, text: string, html: string } | null}
 */
function composeStalledQueueAlert({ orgName, stalledQueue }) {
  if (!stalledQueue || !Number.isFinite(stalledQueue.count) || stalledQueue.count <= 0) {
    return null;
  }
  const url = opsInsightsUrl();
  const queueUrl = opsQueueUrl();
  const items = Array.isArray(stalledQueue.items) ? stalledQueue.items : [];
  const threshold = Number(stalledQueue.thresholdDays) || 7;
  const count = Number(stalledQueue.count) || 0;
  const truncated = count > items.length;

  const headlineCount = count === 1 ? `1 request` : `${count} requests`;
  const subject = orgName
    ? `[${orgName}] ${headlineCount} stalled in queue — needs your attention`
    : `${headlineCount} stalled in queue — needs your attention`;

  // ── Text body ────────────────────────────────────────────────
  const opening = count === 1
    ? `1 request has been sitting in awaiting_review for more than ${threshold} days with no activity.`
    : `${count} requests have been sitting in awaiting_review for more than ${threshold} days with no activity.`;
  ...
  for (const item of items) {
    const daysStalled = Number(item.daysStalled).toFixed(1);
    const label = item.label ? `${item.label}  (${item.externalId})` : item.externalId;
    textLines.push(`  ${daysStalled}d  ·  ${label}`);
  }
  if (truncated) {
    textLines.push('', `Showing the ${items.length} oldest; ${count - items.length} more in the queue.`);
  }
  textLines.push(
    '',
    `Open the watch list:`,
  ...
  // ── HTML body ────────────────────────────────────────────────
  // Per-row list as a definition list (label = "Ndays · external",
  // value = the request label). htmlDl is the same primitive used
  // by every other digest/alert so the email chrome stays uniform.
  // htmlDl escapes both label + value INTERNALLY, so we MUST NOT
  // pre-escape here — doing so produces double-encoded entities
  // ("&amp;lt;" instead of "&lt;").
  const dlRows = items.map((item) => ({
    label: `${Number(item.daysStalled).toFixed(1)}d`,
    value: item.label ? `${item.label} · ${item.externalId}` : item.externalId,
  }));
  const listHtml = htmlDl(dlRows);
  const truncatedNote = truncated
    ? `<p style="font-size:12.5px;color:${EMAIL_MUTE};margin:14px 0 0;font-style:italic;">Showing the ${items.length} oldest. ${count - items.length} more in the queue — open the watch list to refine.</p>`
    : '';
  const headline = count === 1
    ? `1 stalled request needs attention.`
    : `${count} stalled requests need attention.`;
  const eyebrow = `Stalled queue · > ${threshold} days`;
  ...
  const html = htmlEmailChrome({
    ...
    bodyHtml: [
      `<h1 style="${HTML_H1}">${esc(headline)}</h1>`,
      `<p style="${HTML_P}">${esc(opening)}</p>`,
      `<h2 style="font-size:14px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;color:${EMAIL_MUTE};margin:18px 0 4px;">Oldest first</h2>`,
      listHtml,
      truncatedNote,
      htmlCta(url, 'Open the watch list'),
      `<p style="font-size:12.5px;color:${EMAIL_MUTE};margin:16px 0 0;">Or jump straight to the queue: <a href="${esc(queueUrl)}" style="color:${EMAIL_BRAND_AQUA};text-decoration:none;font-weight:500;">${esc(queueUrl)}</a></p>`,
    ].join(''),
  });
  return { subject, text, html };
}
...
/**
 * Fire the daily stalled-queue alert to ops admins. Short-circuits
 * when the cohort is empty (returns reason='no-stalls') so the
 * caller can wrap it unconditionally. Pref-gated via
 * importStalledQueueAlertEmails (per-recipient — one admin muting
 * doesn't drop the email for the others). Fail-soft posture matches
 * sendLowRatingAlert + sendOpsInsightsDigest.
 *
 * @param {{
 *   orgIdNumeric: number,
 *   orgName?: string,
 ...
 * }} args
 */
async function sendStalledQueueAlert({ orgIdNumeric, orgName, stalledQueue }) {
  if (!Number.isFinite(orgIdNumeric)) return { ok: false, reason: 'orgIdNumeric required' };
  if (!stalledQueue) return { ok: false, reason: 'stalledQueue required' };
  // Short-circuit on empty: 0-stalled days are healthy days; no email.
  if (!Number.isFinite(stalledQueue.count) || stalledQueue.count <= 0) {
    return { ok: false, reason: 'no-stalls' };
  }
  const resolution = await resolveOpsRecipients(orgIdNumeric);
  if (resolution.recipients.length === 0) {
    log.info('sendStalledQueueAlert skipped — no recipients', {
      orgIdNumeric, fallbackReason: resolution.fallbackReason,
    });
    return { ok: false, reason: 'no-inbox' };
  }
  const filtered = await filterMutedRecipients(resolution.recipients, 'importStalledQueueAlertEmails');
  if (filtered.length === 0) {
    log.info('sendStalledQueueAlert muted by all recipient preferences', {
      orgIdNumeric, total: resolution.recipients.length,
    });
    return { ok: false, reason: 'all-muted' };
  }
  const composed = composeStalledQueueAlert({ orgName, stalledQueue });
  if (!composed) {
    // Defensive — the composer returns null only when count <= 0, which
    // we already short-circuited above. Logged because reaching here
    // means an upstream contract changed.
    log.warn('sendStalledQueueAlert composed null despite count > 0', { orgIdNumeric });
    return { ok: false, reason: 'composer-null' };
  }
  const results = await email.sendMany(filtered, /** @type {any} */ (composed));
  const sent = results.filter((r) => r.result && r.result.ok).length;
  const failed = results.filter((r) => r.result && !r.result.ok).length;
  if (failed > 0) log.warn('sendStalledQueueAlert partial failure', { orgIdNumeric, sent, failed });
  return { ok: sent > 0, sent, failed };
}

// ── Sprint 70: daily aging-quotes alert ───────────────────────────
//
// Transactional complement to the sprint-69 cohort #11 (sixth
// proactive signal). Aging quotes (status='quoted' with no customer
// decision > 5 days) deserve daily attention because the customer's
// window of interest is closing. Sibling to sprint 39's stall alert
// — same architecture, same recipient-resolution flow, same
// short-circuit-on-empty posture, different SQL bucket.
//
// Distinct from sprint 54's weekly quote-acceptance RATE alert
// (that surfaces the population-level trend). Sprint 70 surfaces
// the SPECIFIC quotes to chase. The two are complementary: the
// weekly rate alert says "your conversion is degrading"; this
// daily alert says "here are the quotes to nudge TODAY."
//
// Per-recipient pref-gated via importQuoteFollowUpAlertEmails so a
// single admin's mute doesn't drop the email for the whole ops team.

/**
 * @param {{ orgName?: string, quoteFollowUp: any }} args
 */
function composeQuoteFollowUpAlert({ orgName, quoteFollowUp }) {
  if (!quoteFollowUp || !Number.isFinite(quoteFollowUp.count) || quoteFollowUp.count <= 0) {
    return null;
  }
  const url = opsInsightsUrl();
  const queueUrl = opsQueueUrl();
  const items = Array.isArray(quoteFollowUp.items) ? quoteFollowUp.items : [];
  const threshold = Number(quoteFollowUp.thresholdDays) || 5;
  const count = Number(quoteFollowUp.count) || 0;
  const truncated = count > items.length;

  const headlineCount = count === 1 ? `1 quote` : `${count} quotes`;
  const subject = orgName
    ? `[${orgName}] ${headlineCount} awaiting customer decision — nudge today`
    : `${headlineCount} awaiting customer decision — nudge today`;

  // ── Text body ────────────────────────────────────────────────
  const opening = count === 1
    ? `1 quote has been sitting in 'quoted' for more than ${threshold} days with no customer decision.`
    : `${count} quotes have been sitting in 'quoted' for more than ${threshold} days with no customer decision.`;

  const textLines = [
    opening,
    `Oldest first — nudge or re-quote:`,
    ``,
  ];
  for (const item of items) {
    const daysPending = Number(item.daysPending).toFixed(1);
    const label = item.label ? `${item.label}  (${item.externalId})` : item.externalId;
    textLines.push(`  ${daysPending}d  ·  ${label}`);
  }
  if (truncated) {
    textLines.push('', `Showing the ${items.length} oldest; ${count - items.length} more aging.`);
  }
  textLines.push(
    '',
    `Open the watch list:`,
  ...
  // ── HTML body ────────────────────────────────────────────────
  // htmlDl escapes label + value INTERNALLY — do NOT pre-escape here.
  // Same lesson learnt in sprint 55 (double-encoding bug) — the
  // primitive owns escaping so callers stay clean.
  const dlRows = items.map(/** @param {any} item */ (item) => ({
    label: `${Number(item.daysPending).toFixed(1)}d`,
    value: item.label ? `${item.label} · ${item.externalId}` : item.externalId,
  }));
  const listHtml = htmlDl(dlRows);
  const truncatedNote = truncated
    ? `<p style="font-size:12.5px;color:${EMAIL_MUTE};margin:14px 0 0;font-style:italic;">Showing the ${items.length} oldest. ${count - items.length} more aging — open the watch list to chase the tail.</p>`
    : '';
  const headline = count === 1
    ? `1 quote is waiting on the customer.`
    : `${count} quotes are waiting on the customer.`;
  const eyebrow = `Aging quotes · > ${threshold} days`;
  ...
  const html = htmlEmailChrome({
    ...
    bodyHtml: [
      `<h1 style="${HTML_H1}">${esc(headline)}</h1>`,
      `<p style="${HTML_P}">${esc(opening)}</p>`,
      `<h2 style="font-size:14px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;color:${EMAIL_MUTE};margin:18px 0 4px;">Oldest first — nudge or re-quote</h2>`,
      listHtml,
      truncatedNote,
      htmlCta(url, 'Open the watch list'),
      `<p style="font-size:12.5px;color:${EMAIL_MUTE};margin:16px 0 0;">Or jump straight to the queue: <a href="${esc(queueUrl)}" style="color:${EMAIL_BRAND_AQUA};text-decoration:none;font-weight:500;">${esc(queueUrl)}</a></p>`,
...
/**
 * Fire the daily aging-quotes alert to ops admins. Short-circuits
 * on empty cohort (returns reason='no-aging'). Pref-gated per
 * recipient via importQuoteFollowUpAlertEmails. Fail-soft posture
 * matches sendStalledQueueAlert.
 *
 * @param {{
 *   orgIdNumeric: number,
 *   orgName?: string,
 ...
 * }} args
 */
async function sendQuoteFollowUpAlert({ orgIdNumeric, orgName, quoteFollowUp }) {
  if (!Number.isFinite(orgIdNumeric)) return { ok: false, reason: 'orgIdNumeric required' };
  if (!quoteFollowUp) return { ok: false, reason: 'quoteFollowUp required' };
  if (!Number.isFinite(quoteFollowUp.count) || quoteFollowUp.count <= 0) {
    return { ok: false, reason: 'no-aging' };
  }
  const resolution = await resolveOpsRecipients(orgIdNumeric);
  if (resolution.recipients.length === 0) {
    log.info('sendQuoteFollowUpAlert skipped — no recipients', {
      orgIdNumeric, fallbackReason: resolution.fallbackReason,
    });
    return { ok: false, reason: 'no-inbox' };
  }
  const filtered = await filterMutedRecipients(resolution.recipients, 'importQuoteFollowUpAlertEmails');
  if (filtered.length === 0) {
    log.info('sendQuoteFollowUpAlert muted by all recipient preferences', {
      orgIdNumeric, total: resolution.recipients.length,
    });
    return { ok: false, reason: 'all-muted' };
  }
  const composed = composeQuoteFollowUpAlert({ orgName, quoteFollowUp });
  if (!composed) {
    log.warn('sendQuoteFollowUpAlert composed null despite count > 0', { orgIdNumeric });
    return { ok: false, reason: 'composer-null' };
  }
  const results = await email.sendMany(filtered, /** @type {any} */ (composed));
  const sent = results.filter((r) => r.result && r.result.ok).length;
  const failed = results.filter((r) => r.result && !r.result.ok).length;
  if (failed > 0) log.warn('sendQuoteFollowUpAlert partial failure', { orgIdNumeric, sent, failed });
  return { ok: sent > 0, sent, failed };
}

// ── Sprint 78: weekly expired-quotes alert ────────────────────────
//
// Trifecta for the sprint-77 cohort #12: quotes that hit expiry
// undecided in the last 30 days, value-weighted. WEEKLY cadence —
// unlike the daily aging-quotes nudge (a same-day chase), expiry
// is a retrospective loss report; a Tuesday-morning batch of
// re-quote candidates beats seven drip emails about the same rows.
// Deliberately NOT gated by the sprint-75 alertCadence — weekly is
// precisely what cadence-weekly orgs asked for.
//
// Discipline: no LLM in this path (ADR 0002); every number flows
// from aggregateOpsInsights. The € figures arrive pre-summed in
// integer cents (ADR 0004) — this composer only formats.
// Per-recipient pref-gated via importExpiredQuotesAlertEmails.

/**
 * @param {{ orgName?: string, expiredQuotes: any }} args
 */
function composeExpiredQuotesAlert({ orgName, expiredQuotes }) {
  if (!expiredQuotes || !Number.isFinite(expiredQuotes.count) || expiredQuotes.count <= 0) {
    return null;
  }
  const url = opsInsightsUrl();
  const items = Array.isArray(expiredQuotes.items) ? expiredQuotes.items : [];
  const windowDays = Number(expiredQuotes.windowDays) || 30;
  const count = Number(expiredQuotes.count) || 0;
  const totalEur = eurFromCents(expiredQuotes.totalLandedCents);
  const truncated = count > items.length;

  // Per-item € — null-guarded BEFORE eurFromCents (which coerces
  // null to €0). A historical row without a landed quote must
  // read "—", never €0: a real zero and a missing quote are
  // different truths.
  /** @param {any} item */
  const itemEur = (item) => (
    (item.landedCents === null || item.landedCents === undefined)
      ? '—'
      : eurFromCents(item.landedCents)
  );

  const headlineCount = count === 1 ? '1 quote' : `${count} quotes`;
  const subject = orgName
    ? `[${orgName}] ${totalEur} in quotes expired unanswered — re-quote candidates`
    : `${totalEur} in quotes expired unanswered — re-quote candidates`;

  // ── Text body ────────────────────────────────────────────────
  const opening = count === 1
    ? `1 quote worth ${totalEur} hit expiry with no customer decision in the last ${windowDays} days.`
    : `${count} quotes worth ${totalEur} in total hit expiry with no customer decision in the last ${windowDays} days.`;
  ...
  for (const item of items) {
    const label = item.label ? `${item.label}  (${item.externalId})` : item.externalId;
    const expiredDate = String(item.expiredAt || '').slice(0, 10);
    textLines.push(`  ${itemEur(item)}  ·  ${label}  ·  expired ${expiredDate}`);
  }
  if (truncated) {
    textLines.push('', `Showing the ${items.length} most recent; ${count - items.length} more expired in the window.`);
  }
  textLines.push(
    '',
    `Open the watch list:`,
  ...
  const dlRows = items.map(/** @param {any} item */ (item) => ({
    label: itemEur(item),
    value: item.label
      ? `${item.label} · ${item.externalId} · expired ${String(item.expiredAt || '').slice(0, 10)}`
      : `${item.externalId} · expired ${String(item.expiredAt || '').slice(0, 10)}`,
  }));
  const listHtml = htmlDl(dlRows);
  const truncatedNote = truncated
    ? `<p style="font-size:12.5px;color:${EMAIL_MUTE};margin:14px 0 0;font-style:italic;">Showing the ${items.length} most recent. ${count - items.length} more expired in the window — open the watch list for the full set.</p>`
    : '';
  const headline = count === 1
    ? `${totalEur} walked away unanswered.`
    : `${totalEur} walked away unanswered.`;
  const eyebrow = `Expired quotes · last ${windowDays} days`;
  ...
  const html = htmlEmailChrome({
    ...
    bodyHtml: [
      `<h1 style="${HTML_H1}">${esc(headline)}</h1>`,
      `<p style="${HTML_P}">${esc(opening)}</p>`,
      `<h2 style="font-size:14px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;color:${EMAIL_MUTE};margin:18px 0 4px;">Most recent first — re-quote candidates (${esc(headlineCount)})</h2>`,
      listHtml,
      truncatedNote,
      htmlCta(url, 'Open the watch list'),
    ].join(''),
...
/**
 * Fire the weekly expired-quotes alert to ops admins. Short-
 * circuits on empty cohort (returns reason='no-expired'). Pref-
 * gated per recipient via importExpiredQuotesAlertEmails. Fail-
 * soft posture matches sendQuoteFollowUpAlert.
 *
 * @param {{
 *   orgIdNumeric: number,
 *   orgName?: string,
 ...
 * }} args
 */
async function sendExpiredQuotesAlert({ orgIdNumeric, orgName, expiredQuotes }) {
  if (!Number.isFinite(orgIdNumeric)) return { ok: false, reason: 'orgIdNumeric required' };
  if (!expiredQuotes) return { ok: false, reason: 'expiredQuotes required' };
  if (!Number.isFinite(expiredQuotes.count) || expiredQuotes.count <= 0) {
    return { ok: false, reason: 'no-expired' };
  }
  const resolution = await resolveOpsRecipients(orgIdNumeric);
  if (resolution.recipients.length === 0) {
    log.info('sendExpiredQuotesAlert skipped — no recipients', {
      orgIdNumeric, fallbackReason: resolution.fallbackReason,
    });
    return { ok: false, reason: 'no-inbox' };
  }
  const filtered = await filterMutedRecipients(resolution.recipients, 'importExpiredQuotesAlertEmails');
  if (filtered.length === 0) {
    log.info('sendExpiredQuotesAlert muted by all recipient preferences', {
      orgIdNumeric, total: resolution.recipients.length,
    });
    return { ok: false, reason: 'all-muted' };
  }
  const composed = composeExpiredQuotesAlert({ orgName, expiredQuotes });
  if (!composed) {
    log.warn('sendExpiredQuotesAlert composed null despite count > 0', { orgIdNumeric });
    return { ok: false, reason: 'composer-null' };
  }
  const results = await email.sendMany(filtered, /** @type {any} */ (composed));
  const sent = results.filter((r) => r.result && r.result.ok).length;
  const failed = results.filter((r) => r.result && !r.result.ok).length;
  if (failed > 0) log.warn('sendExpiredQuotesAlert partial failure', { orgIdNumeric, sent, failed });
  return { ok: sent > 0, sent, failed };
}

// ── Sprint 92: daily SLA-at-risk alert ────────────────────────────
//
// Trifecta for the sprint-91 cohort #13: unquoted requests past
// 75% of the org's NEGOTIATED turnaround target — the breach-
// prevention alert. DAILY at 08:15 UTC (between the stall and
// spike alerts, same morning sweep) because minutes matter here
// more than on any other cohort: every listed row still has
// runway, and the breached ones have a customer actively waiting.
//
// Discipline: no LLM (ADR 0002); every number flows from the
// aggregateOpsInsights slaAtRisk cohort — hoursRemaining arrives
// server-derived, this composer only renders its sign. Honours
// the sprint-75 alertCadence at the cron layer (an org that chose
// weekly-only keeps that choice; the cockpit still shows the
// card) + per-recipient prefs via importSlaAtRiskAlertEmails.

/**
 * @param {{ orgName?: string, slaAtRisk: any }} args
 */
function composeSlaAtRiskAlert({ orgName, slaAtRisk }) {
  if (!slaAtRisk || typeof slaAtRisk !== 'object') return null;
  const atRiskCount = Number(slaAtRisk.atRiskCount) || 0;
  const breachedCount = Number(slaAtRisk.breachedCount) || 0;
  const total = atRiskCount + breachedCount;
  if (total <= 0) return null;
  const url = opsInsightsUrl();
  const items = Array.isArray(slaAtRisk.items) ? slaAtRisk.items : [];
  const target = Number(slaAtRisk.targetHours) || 48;
  const riskLine = Number(slaAtRisk.riskThresholdHours) || 36;
  const truncated = total > items.length;

  const headlineCount = total === 1 ? '1 request' : `${total} requests`;
  const breachSuffix = breachedCount > 0
    ? ` — ${breachedCount} already breached`
    : '';
  const subject = orgName
    ? `[${orgName}] ${headlineCount} at SLA risk${breachSuffix}`
    : `${headlineCount} at SLA risk${breachSuffix}`;

  // Per-item clock rendering — the SIGN of the server-derived
  // hoursRemaining, never recomputed here.
  /** @param {any} item */
  const clockOf = (item) => {
    const rem = Number(item.hoursRemaining);
    if (!Number.isFinite(rem)) return '—';
    return rem < 0 ? `${Math.abs(rem).toFixed(1)}h OVER` : `${rem.toFixed(1)}h left`;
  };

  // ── Text body ────────────────────────────────────────────────
  const opening = total === 1
    ? `1 request is unquoted past ${riskLine}h of your ${target}h turnaround target.`
    : `${total} requests are unquoted past ${riskLine}h of your ${target}h turnaround target.`;
  const breachLine = breachedCount > 0
    ? `${breachedCount} of them ${breachedCount === 1 ? 'is' : 'are'} already past the full target — those customers are still waiting.`
    : `None have breached yet — every one of these is recoverable if quoted today.`;
  ...
  for (const item of items) {
    const label = item.label ? `${item.label}  (${item.externalId})` : item.externalId;
    textLines.push(`  ${clockOf(item)}  ·  ${label}  ·  ${String(item.status || '').replace(/_/g, ' ')}`);
  }
  if (truncated) {
    textLines.push('', `Showing the ${items.length} oldest; ${total - items.length} more on the cockpit.`);
  }
  textLines.push(
    '',
    `Open the cockpit:`,
  ...
  // ── HTML body ────────────────────────────────────────────────
  // htmlDl escapes label + value INTERNALLY — do NOT pre-escape
  // (the sprint-55 double-encoding lesson).
  const dlRows = items.map(/** @param {any} item */ (item) => ({
    label: clockOf(item),
    value: item.label
      ? `${item.label} · ${item.externalId} · ${String(item.status || '').replace(/_/g, ' ')}`
      : `${item.externalId} · ${String(item.status || '').replace(/_/g, ' ')}`,
  }));
  const listHtml = htmlDl(dlRows);
  const truncatedNote = truncated
    ? `<p style="font-size:12.5px;color:${EMAIL_MUTE};margin:14px 0 0;font-style:italic;">Showing the ${items.length} oldest. ${total - items.length} more on the cockpit.</p>`
    : '';
  const headline = breachedCount > 0
    ? `${total === 1 ? '1 request is' : `${total} requests are`} at SLA risk — ${breachedCount} already breached.`
    : `${total === 1 ? '1 request is' : `${total} requests are`} at SLA risk.`;
  const eyebrow = `SLA at risk · > ${riskLine}h of ${target}h`;

  const html = htmlEmailChrome({
    eyebrow,
    bodyHtml: [
      `<h1 style="${HTML_H1}">${esc(headline)}</h1>`,
      `<p style="${HTML_P}">${esc(opening)} ${esc(breachLine)}</p>`,
      `<h2 style="font-size:14px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;color:${EMAIL_MUTE};margin:18px 0 4px;">Oldest first — quote or escalate</h2>`,
      listHtml,
      truncatedNote,
      htmlCta(url, 'Open the cockpit'),
    ].join(''),
  });
  return { subject, text, html };
}
...
/**
 * Fire the daily SLA-at-risk alert to ops admins. Short-circuits
 * on an empty cohort (reason='no-at-risk'). Pref-gated per
 * recipient via importSlaAtRiskAlertEmails. Fail-soft posture
 * matches the sibling daily alerts.
 *
 * @param {{
 *   orgIdNumeric: number,
 *   orgName?: string,
 ...
 * }} args
 */
async function sendSlaAtRiskAlert({ orgIdNumeric, orgName, slaAtRisk }) {
  if (!Number.isFinite(orgIdNumeric)) return { ok: false, reason: 'orgIdNumeric required' };
  if (!slaAtRisk) return { ok: false, reason: 'slaAtRisk required' };
  const total = (Number(slaAtRisk.atRiskCount) || 0) + (Number(slaAtRisk.breachedCount) || 0);
  if (total <= 0) {
    return { ok: false, reason: 'no-at-risk' };
  }
  const resolution = await resolveOpsRecipients(orgIdNumeric);
  if (resolution.recipients.length === 0) {
    log.info('sendSlaAtRiskAlert skipped — no recipients', {
      orgIdNumeric, fallbackReason: resolution.fallbackReason,
    });
    return { ok: false, reason: 'no-inbox' };
  }
  const filtered = await filterMutedRecipients(resolution.recipients, 'importSlaAtRiskAlertEmails');
  if (filtered.length === 0) {
    log.info('sendSlaAtRiskAlert muted by all recipient preferences', {
      orgIdNumeric, total: resolution.recipients.length,
    });
    return { ok: false, reason: 'all-muted' };
  }
  const composed = composeSlaAtRiskAlert({ orgName, slaAtRisk });
  if (!composed) {
    log.warn('sendSlaAtRiskAlert composed null despite total > 0', { orgIdNumeric });
    return { ok: false, reason: 'composer-null' };
  }
  const results = await email.sendMany(filtered, /** @type {any} */ (composed));
  const sent = results.filter((r) => r.result && r.result.ok).length;
  const failed = results.filter((r) => r.result && !r.result.ok).length;
  if (failed > 0) log.warn('sendSlaAtRiskAlert partial failure', { orgIdNumeric, sent, failed });
  return { ok: sent > 0, sent, failed };
}

// ── Sprint 41: daily decline-spike alert ──────────────────────────
//
// Transactional complement to sprint 26's weekly digest, paired with
// the sprint-40 second proactive cohort. Decline-reason spikes are
// TRENDS (current 7-day rate >= 2x the 30-day baseline OR first-time
// reasons hitting the count floor) — but a trend that catches a
// genuine supplier-compliance / regulatory-shift issue deserves
// daily attention, not weekly.
//
// Discipline: no LLM in this path (ADR 0002); every number flows
// directly from the aggregateOpsInsights declineSpike cohort.
// Per-recipient pref-gated via importDeclineSpikeAlertEmails so a
// vacation-muting admin doesn't get woken up; the cohort card on
// /imports/insights still surfaces the spike for whoever DOES check.

// Internal DECLINE_REASON_COPY map renders user-facing reason labels.
// Exposed via module.exports already (see customer-rejected composer).

/**
 * Render the decline-spike alert as { subject, text, html }. Returns
 * null when the cohort is empty (caller short-circuits — no point
 * sending "0 spikes" noise).
 *
 * @param {{
 *   orgName?: string,
 *   declineSpike: {
 ...
 * @returns {{ subject: string, text: string, html: string } | null}
 */
function composeDeclineSpikeAlert({ orgName, declineSpike }) {
  if (!declineSpike || !Array.isArray(declineSpike.spikes) || declineSpike.spikes.length === 0) {
    return null;
  }
  const url = opsInsightsUrl();
  const queueUrl = opsQueueUrl();
  const spikes = declineSpike.spikes;
  const currentDays = Number(declineSpike.currentDays) || 7;
  const baselineDays = Number(declineSpike.baselineDays) || 30;
  const rateMultiplier = Number(declineSpike.rateMultiplier) || 2;
  const count = spikes.length;

  // Subject names the count + the action. "1 reason / 2 reasons"
  // singular branch fits the inbox preview.
  const headlineCount = count === 1 ? `1 decline reason` : `${count} decline reasons`;
  const subject = orgName
    ? `[${orgName}] ${headlineCount} spiking — investigate the trend`
    : `${headlineCount} spiking — investigate the trend`;

  // Enum→prose label converter. The client-side DECLINE_REASON_LABELS
  // map lives in app-shell/lib/api.ts (TS), and DECLINE_REASON_COPY
  // here returns full sentences (headline + nudge — too long for an
  // inline list). A small inline converter keeps this composer
  // self-contained: snake_case → Sentence case.
  /** @param {string} reason */
  ...
  /** @param {{ ratio: number | null, currentCount: number, baselineCount: number }} spike */
  function spikeBadge(spike) {
    if (spike.ratio === null) return 'NEW (no prior occurrence)';
    return `${Number(spike.ratio).toFixed(1)}× vs ${baselineDays}d baseline`;
  }

  // ── Text body ───────────────────────────────────────────────
  const opening = count === 1
    ? `1 decline reason is accelerating vs the prior ${baselineDays} days. Investigate the trend before it becomes a churn signal.`
    : `${count} decline reasons are accelerating vs the prior ${baselineDays} days. Investigate the trends before they become churn signals.`;

  const textLines = [opening, ``, `Last ${currentDays} days vs ${baselineDays}-day baseline:`, ``];
  for (const spike of spikes) {
    const label = reasonLabel(spike.reason);
    const badge = spikeBadge(spike);
    const detail = spike.ratio === null
      ? `(${spike.currentCount} in ${currentDays}d · no prior occurrence)`
      : `(${spike.currentCount} in ${currentDays}d vs ${spike.baselineCount} across ${baselineDays}d)`;
    textLines.push(`  • ${label}  —  ${badge}`);
  ...
  // ── HTML body ───────────────────────────────────────────────
  // htmlDl escapes label + value INTERNALLY (sprint 39 caught the
  // double-encoding bug). We pass RAW strings here.
  const dlRows = spikes.map((spike) => ({
    label: spikeBadge(spike),
    value: `${reasonLabel(spike.reason)} (${spike.currentCount} in ${currentDays}d${
      spike.ratio === null ? '' : ` vs ${spike.baselineCount} in ${baselineDays}d`
    })`,
  }));
  const listHtml = htmlDl(dlRows);
  const headline = count === 1
    ? `1 decline reason is spiking.`
    : `${count} decline reasons are spiking.`;
  const eyebrow = `Decline-reason spike · ≥ ${rateMultiplier}× the ${baselineDays}d baseline`;
  ...
  const html = htmlEmailChrome({
    ...
    bodyHtml: [
      `<h1 style="${HTML_H1}">${esc(headline)}</h1>`,
      `<p style="${HTML_P}">${esc(opening)}</p>`,
      `<h2 style="font-size:14px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;color:${EMAIL_MUTE};margin:18px 0 4px;">Last ${currentDays} days vs ${baselineDays}-day baseline</h2>`,
      listHtml,
      htmlCta(url, 'Open the cohort'),
      `<p style="font-size:12.5px;color:${EMAIL_MUTE};margin:16px 0 0;">Or jump into the queue: <a href="${esc(queueUrl)}" style="color:${EMAIL_BRAND_AQUA};text-decoration:none;font-weight:500;">${esc(queueUrl)}</a></p>`,
    ].join(''),
  });
  return { subject, text, html };
...
/**
 * Fire the daily decline-spike alert to ops admins. Short-circuits
 * with reason='no-spikes' when the cohort is empty (so the caller
 * can wrap it unconditionally). Pref-gated per-recipient via
 * importDeclineSpikeAlertEmails. Fail-soft posture matches
 * sendStalledQueueAlert + sendLowRatingAlert.
 *
 * @param {{
 *   orgIdNumeric: number,
 *   orgName?: string,
 ...
 * }} args
 */
async function sendDeclineSpikeAlert({ orgIdNumeric, orgName, declineSpike }) {
  if (!Number.isFinite(orgIdNumeric)) return { ok: false, reason: 'orgIdNumeric required' };
  if (!declineSpike) return { ok: false, reason: 'declineSpike required' };
  if (!Array.isArray(declineSpike.spikes) || declineSpike.spikes.length === 0) {
    return { ok: false, reason: 'no-spikes' };
  }
  const resolution = await resolveOpsRecipients(orgIdNumeric);
  if (resolution.recipients.length === 0) {
    log.info('sendDeclineSpikeAlert skipped — no recipients', {
      orgIdNumeric, fallbackReason: resolution.fallbackReason,
    });
    return { ok: false, reason: 'no-inbox' };
  }
  const filtered = await filterMutedRecipients(resolution.recipients, 'importDeclineSpikeAlertEmails');
  if (filtered.length === 0) {
    log.info('sendDeclineSpikeAlert muted by all recipient preferences', {
      orgIdNumeric, total: resolution.recipients.length,
    });
    return { ok: false, reason: 'all-muted' };
  }
  const composed = composeDeclineSpikeAlert({ orgName, declineSpike });
  if (!composed) {
    log.warn('sendDeclineSpikeAlert composed null despite non-empty spikes', { orgIdNumeric });
    return { ok: false, reason: 'composer-null' };
  }
  const results = await email.sendMany(filtered, /** @type {any} */ (composed));
  const sent = results.filter((r) => r.result && r.result.ok).length;
  const failed = results.filter((r) => r.result && !r.result.ok).length;
  if (failed > 0) log.warn('sendDeclineSpikeAlert partial failure', { orgIdNumeric, sent, failed });
  return { ok: sent > 0, sent, failed };
}

// ── Sprint 54: weekly quote-acceptance rate alert ─────────────────
//
// Matches the sprint-53 third proactive cohort (cohort #8). The
// quote-acceptance signal is SLOW (30d window vs 60d baseline) so
// the cadence is weekly Wed-morning, not daily — daily would surface
// the same data three or four times before it materially moves.
//
// Discipline: no LLM in this path (ADR 0002). Every number flows
// directly from the aggregateOpsInsights quoteAcceptance cohort.
// Per-recipient pref-gated via importQuoteAcceptanceAlertEmails so
// a vacation-muting admin doesn't get woken up; the cohort card on
// /imports/insights still surfaces the signal for whoever DOES check.

/**
 * Render the quote-acceptance alert as { subject, text, html }.
 * Returns null when the cohort is NOT degraded (caller short-
 * circuits — no point sending "rate is fine" noise).
 *
 * @param {{
 *   orgName?: string,
 *   quoteAcceptance: {
 ...
 * @returns {{ subject: string, text: string, html: string } | null}
 */
function composeQuoteAcceptanceAlert({ orgName, quoteAcceptance }) {
  // Strict boolean check (NOT truthy) — a non-bool isDegraded
  // means upstream contract drift (sprint-53 cohort shape
  // change); fail closed rather than emit a partially-undefined
  // email.
  if (!quoteAcceptance || quoteAcceptance.isDegraded !== true) return null;
  const url = opsInsightsUrl();
  const queueUrl = opsQueueUrl();
  const cur = Math.round((Number(quoteAcceptance.currentRate) || 0) * 100);
  const base = Math.round((Number(quoteAcceptance.baselineRate) || 0) * 100);
  const deltaPp = Number.isFinite(quoteAcceptance.delta)
    ? Math.round(Number(quoteAcceptance.delta) * 100)
    : null;
  const currentDays = Number(quoteAcceptance.currentDays) || 30;
  const baselineDays = Number(quoteAcceptance.baselineDays) || 60;
  const thresholdPct = Math.round(Number(quoteAcceptance.degradationThreshold || 0.75) * 100);

  const subject = orgName
    ? `[${orgName}] Quote acceptance ${cur}% (was ${base}%) — investigate`
    : `Quote acceptance ${cur}% (was ${base}%) — investigate`;

  const deltaWord = deltaPp !== null && deltaPp > 0 ? '+' : '';
  const deltaString = deltaPp !== null ? `${deltaWord}${deltaPp}pp` : '—';

  // ── Text body ────────────────────────────────────────────────
  const opening = `Over the last ${currentDays} days, ${cur}% of requests that exited "quoted" went to "customer_approved" — vs ${base}% across the prior ${baselineDays} days. That's a ${deltaString} swing.`;
  const textLines = [
    opening,
    ``,
    `Current  (${currentDays}d)`,
    `  Approved: ${quoteAcceptance.currentApproved} of ${quoteAcceptance.currentQuoted} decisions`,
    `  Rate:     ${cur}%`,
    ``,
    `Baseline (${baselineDays}d prior)`,
    `  Approved: ${quoteAcceptance.baselineApproved} of ${quoteAcceptance.baselineQuoted} decisions`,
    `  Rate:     ${base}%`,
    ``,
    `Triggered because the current rate fell below ${thresholdPct}% of the prior baseline AND there were >= ${quoteAcceptance.minCount} decisions to compare against.`,
    ``,
    `Open the cockpit:`,
    `  ${url}`,
    ``,
    `Jump to the decline breakdown to see what's driving rejections:`,
    `  ${url}`,
    ``,
    `Or the queue:`,
    `  ${queueUrl}`,
  ...
  // ── HTML body ────────────────────────────────────────────────
  // htmlDl escapes label + value INTERNALLY (sprint 39 lesson:
  // pre-escaping double-encodes). Pass RAW strings.
  const currentRows = htmlDl([
    { label: 'Approved', value: `${quoteAcceptance.currentApproved} of ${quoteAcceptance.currentQuoted}` },
    { label: 'Rate', value: `${cur}%` },
  ]);
  const baselineRows = htmlDl([
    { label: 'Approved', value: `${quoteAcceptance.baselineApproved} of ${quoteAcceptance.baselineQuoted}` },
    { label: 'Rate', value: `${base}%` },
  ]);
  const headline = `Quote-acceptance rate dropped to ${cur}%.`;
  const eyebrow = `Quote-acceptance · ${deltaString} vs ${baselineDays}d baseline`;
  const html = htmlEmailChrome({
    eyebrow,
    bodyHtml: [
      `<h1 style="${HTML_H1}">${esc(headline)}</h1>`,
      `<p style="${HTML_P}">${esc(opening)}</p>`,
      `<h2 style="font-size:14px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;color:${EMAIL_MUTE};margin:18px 0 4px;">Current (${currentDays}d)</h2>`,
      currentRows,
      `<h2 style="font-size:14px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;color:${EMAIL_MUTE};margin:18px 0 4px;">Baseline (${baselineDays}d prior)</h2>`,
      baselineRows,
      `<p style="font-size:12.5px;color:${EMAIL_MUTE};margin:14px 0 4px;font-style:italic;">Triggered because current rate fell below ${thresholdPct}% of baseline AND there were &ge; ${quoteAcceptance.minCount} decisions to compare against.</p>`,
      htmlCta(url, 'Open the cockpit'),
      `<p style="font-size:12.5px;color:${EMAIL_MUTE};margin:16px 0 0;">Or jump straight to the queue: <a href="${esc(queueUrl)}" style="color:${EMAIL_BRAND_AQUA};text-decoration:none;font-weight:500;">${esc(queueUrl)}</a></p>`,
    ].join(''),
  });
...
/**
 * Fire the weekly quote-acceptance alert to ops admins.
 * Short-circuits with reason='not-degraded' when the cohort is
 * healthy (caller fires unconditionally). Pref-gated per-recipient
 * via importQuoteAcceptanceAlertEmails. Fail-soft posture matches
 * sendDeclineSpikeAlert + sendStalledQueueAlert.
 *
 * @param {{
 *   orgIdNumeric: number,
 *   orgName?: string,
 ...
 * }} args
 */
async function sendQuoteAcceptanceAlert({ orgIdNumeric, orgName, quoteAcceptance }) {
  if (!Number.isFinite(orgIdNumeric)) return { ok: false, reason: 'orgIdNumeric required' };
  if (!quoteAcceptance) return { ok: false, reason: 'quoteAcceptance required' };
  if (!quoteAcceptance.isDegraded) {
    return { ok: false, reason: 'not-degraded' };
  }
  const resolution = await resolveOpsRecipients(orgIdNumeric);
  if (resolution.recipients.length === 0) {
    log.info('sendQuoteAcceptanceAlert skipped — no recipients', {
      orgIdNumeric, fallbackReason: resolution.fallbackReason,
    });
    return { ok: false, reason: 'no-inbox' };
  }
  const filtered = await filterMutedRecipients(resolution.recipients, 'importQuoteAcceptanceAlertEmails');
  if (filtered.length === 0) {
    log.info('sendQuoteAcceptanceAlert muted by all recipient preferences', {
      orgIdNumeric, total: resolution.recipients.length,
    });
    return { ok: false, reason: 'all-muted' };
  }
  const composed = composeQuoteAcceptanceAlert({ orgName, quoteAcceptance });
  if (!composed) {
    log.warn('sendQuoteAcceptanceAlert composed null despite isDegraded', { orgIdNumeric });
    return { ok: false, reason: 'composer-null' };
  }
  const results = await email.sendMany(filtered, /** @type {any} */ (composed));
  const sent = results.filter((r) => r.result && r.result.ok).length;
  const failed = results.filter((r) => r.result && !r.result.ok).length;
  if (failed > 0) log.warn('sendQuoteAcceptanceAlert partial failure', { orgIdNumeric, sent, failed });
  return { ok: sent > 0, sent, failed };
}

// ── Sprint 58: weekly supplier-concentration alert ─────────────────
//
// Matches the sprint-57 fourth proactive cohort (cohort #9). The
// supplier-concentration signal is a RISK state — not urgent like
// stalls but more important than a slow trend. Weekly Thu-morning
// cadence lands the signal between Mon's batch / Wed's quote-
// acceptance and the weekend so ops has time to diversify before
// the next tariff/sanctions window.
//
// Discipline: no LLM in this path (ADR 0002). Every number flows
// directly from the aggregateOpsInsights supplierConcentration
// cohort. Per-recipient pref-gated via
// importSupplierConcentrationAlertEmails. Strict-boolean fail-
// closed guard on isConcentrated (sprint-54 lesson applied).

/**
 * Render the supplier-concentration alert as
 * { subject, text, html }. Returns null when the cohort is NOT
 * concentrated (caller short-circuits — no point sending
 * "concentration is fine" noise) OR when isConcentrated isn't a
 * strict true (defensive — non-bool from upstream contract drift).
 *
 * @param {{
 *   orgName?: string,
 ...
 * @returns {{ subject: string, text: string, html: string } | null}
 */
function composeSupplierConcentrationAlert({ orgName, supplierConcentration }) {
  // Strict boolean check (NOT truthy) — sprint-54 lesson applied:
  // a non-bool isConcentrated means upstream contract drift,
  // fail closed rather than emit a partially-undefined email.
  if (!supplierConcentration || supplierConcentration.isConcentrated !== true) return null;
  if (!supplierConcentration.topCountry || supplierConcentration.topCountryShare === null) {
    // Defensive: isConcentrated=true should imply both topCountry
  ...
  const url = opsInsightsUrl();
  const queueUrl = opsQueueUrl();
  const sharePct = Math.round(Number(supplierConcentration.topCountryShare) * 100);
  const thresholdPct = Math.round(Number(supplierConcentration.threshold || 0.75) * 100);
  const windowDays = Number(supplierConcentration.windowDays) || 30;
  const topCountry = String(supplierConcentration.topCountry);
  const topCount = Number(supplierConcentration.topCountryCount) || 0;
  const total = Number(supplierConcentration.totalPicks) || 0;

  const subject = orgName
    ? `[${orgName}] Supplier concentration ${sharePct}% to ${topCountry} — sourcing risk`
    : `Supplier concentration ${sharePct}% to ${topCountry} — sourcing risk`;

  // ── Text body ────────────────────────────────────────────────
  const opening = `Of the ${total} supplier picks in the last ${windowDays} days, ${sharePct}% (${topCount}) went to ${topCountry}. A tariff shift or sanctions update on that origin would take a large share of your sourcing offline.`;
  const textLines = [
    opening,
    ``,
    `Window`,
    ...
    `Top country`,
    `  ${topCountry}`,
    `  ${topCount} of ${total} picks (${sharePct}%)`,
    ``,
    `Triggered because one country crossed ${thresholdPct}% of picks AND there were >= ${supplierConcentration.minCount} picks to compare against.`,
    ``,
    `Open the cockpit:`,
    `  ${url}`,
    ``,
    `Or jump to the queue to plan diversification:`,
    `  ${queueUrl}`,
    ``,
    `— OrcaTrade Operations`,
  ];
  ...
  // htmlDl escapes label + value INTERNALLY (sprint 39 lesson).
  // Pass RAW strings.
  const windowRows = htmlDl([
    { label: 'Window', value: `Last ${windowDays} days` },
    { label: 'Total picks', value: String(total) },
  ]);
  const topRows = htmlDl([
    { label: 'Country', value: topCountry },
    { label: 'Picks', value: `${topCount} of ${total}` },
    { label: 'Share', value: `${sharePct}%` },
  ]);
  const headline = `Supplier concentration: ${sharePct}% to ${topCountry}.`;
  const eyebrow = `Sourcing risk · concentration crossed ${thresholdPct}%`;
  const html = htmlEmailChrome({
    eyebrow,
    bodyHtml: [
      `<h1 style="${HTML_H1}">${esc(headline)}</h1>`,
      `<p style="${HTML_P}">${esc(opening)}</p>`,
      `<h2 style="font-size:14px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;color:${EMAIL_MUTE};margin:18px 0 4px;">Window</h2>`,
      windowRows,
      `<h2 style="font-size:14px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;color:${EMAIL_MUTE};margin:18px 0 4px;">Top country</h2>`,
      topRows,
      `<p style="font-size:12.5px;color:${EMAIL_MUTE};margin:14px 0 4px;font-style:italic;">Triggered because one country crossed ${thresholdPct}% of picks AND there were &ge; ${supplierConcentration.minCount} picks to compare against.</p>`,
      htmlCta(url, 'Open the cockpit'),
      `<p style="font-size:12.5px;color:${EMAIL_MUTE};margin:16px 0 0;">Or jump straight to the queue: <a href="${esc(queueUrl)}" style="color:${EMAIL_BRAND_AQUA};text-decoration:none;font-weight:500;">${esc(queueUrl)}</a></p>`,
    ].join(''),
  });
...
/**
 * Fire the weekly supplier-concentration alert to ops admins.
 * Short-circuits with reason='not-concentrated' when the cohort is
 * healthy (caller fires unconditionally). Pref-gated per-recipient
 * via importSupplierConcentrationAlertEmails. Fail-soft posture
 * matches sendQuoteAcceptanceAlert + sendDeclineSpikeAlert +
 ...
 * }} args
 */
async function sendSupplierConcentrationAlert({ orgIdNumeric, orgName, supplierConcentration }) {
  if (!Number.isFinite(orgIdNumeric)) return { ok: false, reason: 'orgIdNumeric required' };
  if (!supplierConcentration) return { ok: false, reason: 'supplierConcentration required' };
  if (!supplierConcentration.isConcentrated) {
    return { ok: false, reason: 'not-concentrated' };
  }
  const resolution = await resolveOpsRecipients(orgIdNumeric);
  if (resolution.recipients.length === 0) {
    log.info('sendSupplierConcentrationAlert skipped — no recipients', {
      orgIdNumeric, fallbackReason: resolution.fallbackReason,
    });
    return { ok: false, reason: 'no-inbox' };
  }
  const filtered = await filterMutedRecipients(resolution.recipients, 'importSupplierConcentrationAlertEmails');
  if (filtered.length === 0) {
    log.info('sendSupplierConcentrationAlert muted by all recipient preferences', {
      orgIdNumeric, total: resolution.recipients.length,
    });
    return { ok: false, reason: 'all-muted' };
  }
  const composed = composeSupplierConcentrationAlert({ orgName, supplierConcentration });
  if (!composed) {
    log.warn('sendSupplierConcentrationAlert composed null despite isConcentrated', { orgIdNumeric });
    return { ok: false, reason: 'composer-null' };
  }
  const results = await email.sendMany(filtered, /** @type {any} */ (composed));
  const sent = results.filter((r) => r.result && r.result.ok).length;
  const failed = results.filter((r) => r.result && !r.result.ok).length;
  if (failed > 0) log.warn('sendSupplierConcentrationAlert partial failure', { orgIdNumeric, sent, failed });
  return { ok: sent > 0, sent, failed };
}

// ── Sprint 63: weekly rating-trend alert ──────────────────────────
//
// Matches the sprint-62 fifth proactive cohort (cohort #10). The
// rating-trend signal is the slow-cook complement to sprint-33's
// per-event 1-2★ alerts — a streak of 3★ ratings (none low enough
// for sprint 33) quietly drops the avg from 4.5 to 3.5 over a
// week. Weekly Fri-morning cadence lands the signal end-of-week
// so ops can plan investigation Mon AM with the week's data in
// hand.
//
// Discipline: no LLM in this path (ADR 0002). Every number flows
// directly from the aggregateOpsInsights ratingTrend cohort.
// Per-recipient pref-gated via importRatingTrendAlertEmails.
// Strict-boolean fail-closed guard on isDeclining (sprint-54
// lesson re-applied).

/**
 * Render the rating-trend alert as { subject, text, html }.
 * Returns null when the cohort is NOT declining (caller short-
 * circuits — no point sending "rating is steady" noise) OR when
 * isDeclining isn't a strict true (defensive — non-bool from
 ...
 * @returns {{ subject: string, text: string, html: string } | null}
 */
function composeRatingTrendAlert({ orgName, ratingTrend }) {
  // Strict boolean check (NOT truthy) — sprint-54 lesson.
  if (!ratingTrend || ratingTrend.isDeclining !== true) return null;
  // Defensive: isDeclining=true should imply currentAvg +
  // baselineAvg are non-null. If they're not, upstream is
  // broken; fail closed.
  if (ratingTrend.currentAvg === null || ratingTrend.baselineAvg === null) return null;
  const url = opsInsightsUrl();
  const queueUrl = opsQueueUrl();
  const currentAvg = Number(ratingTrend.currentAvg).toFixed(1);
  const baselineAvg = Number(ratingTrend.baselineAvg).toFixed(1);
  const deltaStars = Number.isFinite(ratingTrend.delta)
    ? Number(ratingTrend.delta).toFixed(1)
    : null;
  const currentDays = Number(ratingTrend.currentDays) || 7;
  const baselineDays = Number(ratingTrend.baselineDays) || 30;
  const dropThreshold = Number(ratingTrend.dropThreshold || 0.5).toFixed(1);

  const subject = orgName
    ? `[${orgName}] Rating ${currentAvg}★ (was ${baselineAvg}★) — investigate drift`
    : `Rating ${currentAvg}★ (was ${baselineAvg}★) — investigate drift`;

  // ── Text body ────────────────────────────────────────────────
  const opening = `Over the last ${currentDays} days, the avg customer rating is ${currentAvg}★ across ${ratingTrend.currentCount} ratings — down from ${baselineAvg}★ over the prior ${baselineDays} days (${ratingTrend.baselineCount} ratings). That's a -${deltaStars || '?'}★ drift.`;
  const textLines = [
    opening,
    ``,
    `Current  (${currentDays}d)`,
    `  Avg: ${currentAvg}★`,
    `  N:   ${ratingTrend.currentCount} ratings`,
    ``,
    `Baseline (${baselineDays}d prior)`,
    `  Avg: ${baselineAvg}★`,
    `  N:   ${ratingTrend.baselineCount} ratings`,
    ``,
    `Triggered because the drop crossed ${dropThreshold}★ AND there are >= ${ratingTrend.minCount} ratings in the current window. Sprint-33 per-event alerts only fire on 1-2★; this is the slow-cook signal.`,
    ``,
    `Open the cockpit:`,
    `  ${url}`,
    ``,
    `Or open recent customer-approved requests to read the comments:`,
    `  ${queueUrl}`,
    ``,
    `— OrcaTrade Operations`,
  ];
  ...
  // Pass RAW strings.
  const currentRows = htmlDl([
    { label: 'Avg', value: `${currentAvg}★` },
    { label: 'N', value: `${ratingTrend.currentCount} ratings` },
  ]);
  const baselineRows = htmlDl([
    { label: 'Avg', value: `${baselineAvg}★` },
    { label: 'N', value: `${ratingTrend.baselineCount} ratings` },
  ]);
  const headline = `Avg rating dropped to ${currentAvg}★.`;
  const eyebrow = `Rating trend · -${deltaStars || '?'}★ vs ${baselineDays}d baseline`;
  const html = htmlEmailChrome({
    eyebrow,
    bodyHtml: [
      `<h1 style="${HTML_H1}">${esc(headline)}</h1>`,
      ...
      `<h2 style="font-size:14px;font-weight:600;letter-spacing:0.04em;text-transform:uppercase;color:${EMAIL_MUTE};margin:18px 0 4px;">Baseline (${baselineDays}d prior)</h2>`,
      baselineRows,
      `<p style="font-size:12.5px;color:${EMAIL_MUTE};margin:14px 0 4px;font-style:italic;">Triggered because the drop crossed ${dropThreshold}★ AND there are &ge; ${ratingTrend.minCount} ratings in the current window. Sprint-33 per-event alerts only fire on 1-2★; this is the slow-cook signal.</p>`,
      htmlCta(url, 'Open the cockpit'),
      `<p style="font-size:12.5px;color:${EMAIL_MUTE};margin:16px 0 0;">Or jump straight to the queue: <a href="${esc(queueUrl)}" style="color:${EMAIL_BRAND_AQUA};text-decoration:none;font-weight:500;">${esc(queueUrl)}</a></p>`,
    ].join(''),
  });
...
/**
 * Fire the weekly rating-trend alert to ops admins. Short-
 * circuits with reason='not-declining' when the cohort is
 * healthy (caller fires unconditionally). Pref-gated per-
 * recipient via importRatingTrendAlertEmails. Fail-soft posture
 * matches sendSupplierConcentrationAlert + sendDeclineSpikeAlert
 * + sendStalledQueueAlert.
 *
 * @param {{
 *   orgIdNumeric: number,
 ...
 * }} args
 */
async function sendRatingTrendAlert({ orgIdNumeric, orgName, ratingTrend }) {
  if (!Number.isFinite(orgIdNumeric)) return { ok: false, reason: 'orgIdNumeric required' };
  if (!ratingTrend) return { ok: false, reason: 'ratingTrend required' };
  if (!ratingTrend.isDeclining) {
    return { ok: false, reason: 'not-declining' };
  }
  const resolution = await resolveOpsRecipients(orgIdNumeric);
  if (resolution.recipients.length === 0) {
    log.info('sendRatingTrendAlert skipped — no recipients', {
      orgIdNumeric, fallbackReason: resolution.fallbackReason,
    });
    return { ok: false, reason: 'no-inbox' };
  }
  const filtered = await filterMutedRecipients(resolution.recipients, 'importRatingTrendAlertEmails');
  if (filtered.length === 0) {
    log.info('sendRatingTrendAlert muted by all recipient preferences', {
      orgIdNumeric, total: resolution.recipients.length,
    });
    return { ok: false, reason: 'all-muted' };
  }
  const composed = composeRatingTrendAlert({ orgName, ratingTrend });
  if (!composed) {
    log.warn('sendRatingTrendAlert composed null despite isDeclining', { orgIdNumeric });
    return { ok: false, reason: 'composer-null' };
  }
  const results = await email.sendMany(filtered, /** @type {any} */ (composed));
  const sent = results.filter((r) => r.result && r.result.ok).length;
  const failed = results.filter((r) => r.result && !r.result.ok).length;
  if (failed > 0) log.warn('sendRatingTrendAlert partial failure', { orgIdNumeric, sent, failed });
  return { ok: sent > 0, sent, failed };
}
...
module.exports = {
  ...
  composeQuoteFollowUpAlert,
  composeExpiredQuotesAlert,
  composeSlaAtRiskAlert,
  ...
  sendLowRatingAlert,
  sendStalledQueueAlert,
  sendQuoteFollowUpAlert,
  sendExpiredQuotesAlert,
  sendSlaAtRiskAlert,
