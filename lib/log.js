...
function emit(levelName, message, extras, baseContext) {
  ...
  // Sprint BG-4.2: forward warn + error to Sentry when SENTRY_DSN is set.
  // Strictly fire-and-forget — the structured log line above is the
  // always-available primary record; Sentry is a side channel for
  // visibility + alerting. Forwarding lower levels (info/debug) would
  // exceed any reasonable plan's quota; warn+error captures the
  // signal worth paging on.
  if (levelName === 'warn' || levelName === 'error') {
    forwardToSentry(levelName, payload);
  }
}

// Lazy + safe Sentry forwarder. Returns immediately; the network call
// happens in a microtask. Any error inside the forwarder is silently
// swallowed — telemetry must NEVER break a request.
//
// P0.7: if `extras.err` is an Error, route through captureException so
// the stack trace + frames land in Sentry's Issues view properly.
// Otherwise fall through to captureMessage (the historical path).
function forwardToSentry(level, payload) {
  try {
    const sentry = require('./sentry');
    if (!sentry.isConfigured()) return;
    // Strip the redundant ts/level/msg from extras (they're already
    // promoted to the Sentry event's top-level fields).
    const { ts: _ts, level: _level, msg, ...extras } = payload;
    const errCandidate = extras.err;
    if (errCandidate instanceof Error) {
      // Preserve everything else as extras but rename the Error so
      // captureException can format it properly. The full message
      // (e.g. "handler threw") goes to a `summary` extra so the
      // human-readable lead-in is still visible alongside the
      // exception type/value.
      const { err: _err, ...rest } = extras;
      sentry.captureException(errCandidate, {
        extras: { summary: msg, ...rest },
        level,
      }).catch(() => {});
      return;
    }
    sentry.captureMessage({ level, message: msg, extras }).catch(() => {});
  } catch (_) { /* sentry module unloadable / require cycle / etc. */ }
}
