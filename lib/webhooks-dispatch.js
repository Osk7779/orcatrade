...
async function dispatchEvent({ event, fetchImpl, nowMs }) {
  ...
  // Sprint 51 — successful deliveries reset the per-sub
  // abandonment counter. A single success after a streak of
  // failures rebuilds the runway before auto-disable would fire.
  await Promise.all(
    results
      .filter((r) => r.ok)
      .map((r) => webhooks.resetAbandonmentCounter(r.subId).catch(() => null)),
  );
...
async function flushRetriesForSubscription({ subId, fetchImpl, nowMs }) {
  ...
  for (const due of dues) {
    ...
    if (result.ok) {
      ...
      // Sprint 51 — reset the abandonment counter on success.
      // Fail-soft.
      await webhooks.resetAbandonmentCounter(subId).catch(() => null);
      succeeded += 1;
    } else {
      ...
      if (reschedule.abandoned) {
        abandoned += 1;
        // Sprint 51 — bump the per-sub consecutive-abandonment
        // counter. Auto-disable fires inside bumpAbandonmentCounter
        // when the threshold is crossed; an audit event marks the
        // transition so admins reviewing the chain see WHEN + WHY
        // the sub was disabled. Fail-soft.
        const bump = await webhooks.bumpAbandonmentCounter({
          subId,
          lastError: result.error || (result.timedOut ? 'timeout' : `non-2xx: ${result.status}`),
        }).catch(() => null);
        if (bump && bump.autoDisabled && !bump.noOp) {
          // Defer the events require so the boot-time circular dep
          // (events.js → webhooks-dispatch.js → events.js) stays
          // resolved.
          try {
            const events = require('./events');
            await events.record('webhook_subscription_auto_disabled', {
              orgId: bump.subscription.orgIdNumeric,
              entityType: 'webhook_subscription',
              entityId: subId,
              actorEmailHash: null,  // automated, not user-driven
              detail: {
                label: bump.subscription.label,
                url: bump.subscription.url,
                reason: bump.subscription.autoDisabledReason,
              },
            });
          } catch (_) { /* audit best-effort here — cron retries the next tick */ }
        }
      }
    }
  }
