...
//
// Two destinations are written in parallel:
//   1. lib/log.js — structured stdout (Vercel captures, downstream-routable
//      to Sentry / Axiom in Track 4.2)
//   2. lib/events.js — durable KV append (queried by /dashboard/ai/ in
//      Sprint BG-6.5). Fire-and-forget; KV failure must not break the
//      request, so the promise is `.catch()`-ed silently and the
//      structured log is the always-available primary record.
function recordAnthropicCall({ agent, promptVersion, promptHash, model, requestId, response, latencyMs, tier, userTier, email, emailHash }) {
  try {
    const usage = response && response.usage;
    const tokens = summariseTokens(usage);
    const costCents = computeCost(model, usage);
    // Per-tenant rollup (consumed by /dashboard/ai/'s byTenant panel).
    // The pseudonym is computed inline so the raw email NEVER enters the
    // ai_call event payload at all — KV stores only the hash, and the
    // chain projection already excludes 'email'. Anonymous / unauth'd
    // calls drop the pseudonym entirely, which buckets them under
    // '(anonymous)' in the dashboard aggregator.
    //
    // Callers may pass either `email` (we hash it here) or a pre-computed
    // `emailHash` directly; the explicit hash wins to keep the value
    // stable across re-tries that may not re-supply the raw email.
    const effectiveEmailHash = emailHash || (email ? hash.emailHash(email) : null);
    const payload = {
      agent,
    ...
    if (effectiveEmailHash) payload.emailHash = effectiveEmailHash;
    log.info('anthropic call', { event: 'ai_call', ...payload });

    // Durable record for /dashboard/ai/ — lazy-required to avoid a
    // require cycle with handlers that import cost-telemetry early in
    // module load. The events log is capped + TTL'd so this can't
    // unbounded-grow.
    try {
      const events = require('../events');
      // Fire and forget; KV unavailable → just lose the row for the
      // dashboard. The structured log line still happened.
      events.record('ai_call', payload).catch(() => {});
    } catch (_) { /* events module unloadable in some test envs */ }

    // Apex P1.7 — per-tenant spend cap. Bump the user's cumulative
    // monthly cents-spend counter so the next call's pre-flight gate
    // (gating.checkAgentSpend) can detect a runaway. Lazy-required
    // for the same require-cycle reason as events.js. Fire-and-forget
    // by contract (lib/ai/spend-cap.js's recordSpend never throws).
    if (effectiveEmailHash) {
      try {
        const spendCap = require('./spend-cap');
        spendCap.recordSpend({ email, emailHash: effectiveEmailHash, costCents }).catch(() => {});
      } catch (_) { /* spend-cap module unloadable in some test envs */ }
    }
  } catch (err) {
