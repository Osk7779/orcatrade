...
function recordAnthropicCall({ agent, promptVersion, promptHash, model, requestId, response, latencyMs, tier, userTier, email }) {
  try {
    ...
    // calls drop emailHash entirely, which buckets them under
    // '(anonymous)' in the dashboard aggregator.
    const emailHash = email ? hash.emailHash(email) : null;
    const payload = {
      agent,
