...
const WEBHOOK_EVENT_TYPES = Object.freeze([
  ...
  // Sprint 76 — archive's mirror joins the dispatch surface for
  // symmetry: a subscriber tracking archive lifecycle needs both
  // halves or its external state drifts on every restore.
  'import_request_restored',
  // Sprint 81 — actual outcome reported. ERP/finance subscribers
  // reconcile quoted-vs-actual the moment the loop closes.
  'import_request_actual_reported',
  // Sprint 97 — SLA breach recorded. A subscriber tracking the
  // commitment (procurement systems, escalation bots) hears the
  // moment the promise breaks, not at the weekly report.
  ...
  // Sprint 72 — config governance joins the dispatch surface.
  // operator_config_updated lets external systems (SIEM pipelines,
  // GRC tooling, Slack bridges) observe knob changes the moment
  // they land — the SAP-GTS "config audit export" pattern, but
  // push instead of batch. Strictly opt-in: every subscription
  // stores an explicit eventTypes array, so no existing endpoint
  // receives the new type until its owner edits the subscription.
  // The dispatched detail carries patched/reset/previous/reason/
  // preset — before-values included (sprint 72) so a subscriber
  // sees the full old→new diff without querying back.
...
// Sprint 59 — rotate the signing secret on an existing subscription.
// Mints a fresh whsec_<64 hex>, replaces the stored secret, stamps
// secretRotatedAt + rotatedByEmailHash for audit correlation.
// Returns the NEW raw secret ONCE — same reveal-once posture as
// create. Cross-org isolated like deleteWebhook (notFound shape on
// mismatch, never "this exists but isn't yours").
//
// v1: no grace period. The new secret signs ALL deliveries from
// the moment this returns; the customer's receiver must verify
// against the new value. A future sprint can add overlap (dual
// signing during a grace window) — the rotatedAt timestamp is
// the anchor that change would build on.
//
// @param {{ orgIdNumeric: number, id: string, actorEmailHash?: string }} input
async function rotateSecret({ orgIdNumeric, id, actorEmailHash }) {
  if (!Number.isFinite(orgIdNumeric)) return { ok: false, errors: ['orgIdNumeric required'] };
  if (typeof id !== 'string' || !id) return { ok: false, errors: ['id required'] };
  let stored = null;
  try {
    stored = await kv.get(subKey(id));
  } catch (_) {
    return { ok: false, errors: ['kv read failed'] };
  }
  ...
  const updated = {
    ...
    rotatedByEmailHash: actorEmailHash || null,
  };
  try {
  ...
  return { ok: true, subscription: updated, secret: newSecret };
}
