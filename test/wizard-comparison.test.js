...
test('start/legacy/app.js declares renderComparison function', () => {
...
test('start/legacy/app.js wires .compare-btn click handlers', async () => {
  const js = fs.readFileSync(path.join(ROOT, 'start/legacy/app.js'), 'utf8');
...
test('start/legacy/app.js renders Compare button per non-user-pick row', () => {
  const js = fs.readFileSync(path.join(ROOT, 'start/legacy/app.js'), 'utf8');
  assert.match(js, /class="compare-btn"[\s\S]*?data-compare-origin/);
  assert.match(js, /e\.isUserChoice[\s\S]*?\?[\s\S]*?''/); // skip user pick
});

test('start/legacy/app.js emits comparison-panel placeholder', () => {
  const js = fs.readFileSync(path.join(ROOT, 'start/legacy/app.js'), 'utf8');
